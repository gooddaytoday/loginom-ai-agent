...
const candidate: Loginom.Candidate = {
  revision: 0, url: "http://example.test/app/?a=b", username: "User",
  apiKey: { operation: "replace", value: "private-api-key" }, password: { operation: "empty" },
}
async function fixture(runtime?: ConnectionRuntime) {
  ...
  const service = await connectionService(store, runtime ?? {
    async check() {},
    async prepare(record) { return { async close() { closed.push(record.generation) } } },
  })
  return { service, store, closed, async close() { await service.close(); await rm(directory, { recursive: true, force: true }) } }
}
...
test("safe views expose defaults and presence flags only; a successful save survives restart", async () => {
  ...
  try {
    expect(await f.service.api.read()).toMatchObject({ revision: 0, username: "user", hasApiKey: false, hasPassword: false, folder: "/user", state: "unconfigured" })
    await save(f.service)
    expect(await f.service.api.read()).toMatchObject({ generation: 1, revision: 1, username: "User", folder: "/User", hasApiKey: true, hasPassword: false, state: "ready" })
...
test("cancelling a pending switch keeps the active identity and invalidates stale windows", async () => {
  ...
  try {
    ...
    expect(await f.service.api.cancelPending({ revision: 2 })).toMatchObject({ state: "ready", username: "User", generation: 1, revision: 3 })
    run.release()
    await f.service.settled()
...
test("failed authentication returns a bounded error and never saves candidate secrets", async () => {
  const f = await fixture({ async check() { throw new Error("private-api-key password") }, async prepare() { throw new Error("unreachable") } })
  try {
...
test("validation is one-use, revision-bound and cannot silently replace another window's save", async () => {
  ...
  try {
    ...
    await f.service.settled()
    await expect(f.service.api.save({ validationId: validation.validationId, revision: 1 })).rejects.toThrow("LOGINOM_VALIDATION_EXPIRED")
    await expect(f.service.api.check(candidate)).rejects.toThrow("LOGINOM_REVISION_CONFLICT")
    await save(f.service, { ...candidate, revision: 1, apiKey: { operation: "preserve" }, password: { operation: "replace", value: "new-password" } })
    await save(f.service, { ...candidate, revision: 2, apiKey: { operation: "preserve" }, password: { operation: "preserve" } })
...
for (const username of ["", ".", "..", "a/b", "a\\b", "a\nb"]) {
  test(`rejects unsafe username ${JSON.stringify(username)}`, async () => {
    const f = await fixture()
    try { await expect(f.service.api.check({ ...candidate, username })).rejects.toThrow("LOGINOM_USERNAME_INVALID") }
    finally { await f.close() }
  })
}
for (const url of ["file:///tmp/private", "https://user:password@example.test/", "not-a-url"]) {
  test(`rejects invalid Loginom URL`, async () => {
    const f = await fixture()
    try { await expect(f.service.api.check({ ...candidate, url })).rejects.toThrow("LOGINOM_URL_INVALID") }
    finally { await f.close() }
  })
}

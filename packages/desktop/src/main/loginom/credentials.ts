...
export type ProtectedSecrets = { encrypted: string } | Secrets
...
export function credentials(platform: NodeJS.Platform, protection?: Protection) {
  ...
  return {
    ...
    decode(value: ProtectedSecrets): Secrets {
      ...
      if (!decoded || typeof decoded !== "object" || !("apiKey" in decoded) || !("password" in decoded)
        || typeof decoded.apiKey !== "string" || typeof decoded.password !== "string") {

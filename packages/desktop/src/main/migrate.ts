...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "com.loginom.aiagent.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `loginom-ai-agent.settings.dat` is special: it maps to the `loginom-ai-agent.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"loginom-ai-agent.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // loginom-ai-agent.settings.dat → the electron settings store ("loginom-ai-agent.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "loginom-ai-agent.global.dat").
  const storeName = filename === "loginom-ai-agent.settings.dat" ? "loginom-ai-agent.settings" : filename

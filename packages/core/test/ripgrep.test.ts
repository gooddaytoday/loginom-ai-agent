...
import { RelativePath } from "@loginom-ai-agent/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".loginom-ai-agent")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".loginom-ai-agent", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".loginom-ai-agent/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".loginom-ai-agent/config"))

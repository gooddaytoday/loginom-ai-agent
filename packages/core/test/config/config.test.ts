...
import { ConfigProvider } from "@loginom-ai-agent/core/config/provider"
...
import { ConfigMigrateV1 } from "@loginom-ai-agent/core/v1/config/migrate"
...
import { Policy } from "@loginom-ai-agent/core/policy"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "loginom-ai-agent.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "loginom-ai-agent.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "loginom-ai-agent.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "loginom-ai-agent.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "loginom-ai-agent.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "loginom-ai-agent.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "loginom-ai-agent.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .loginom-ai-agent configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".loginom-ai-agent"), { recursive: true })
            await fs.mkdir(path.join(directory, ".loginom-ai-agent"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "loginom-ai-agent.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "loginom-ai-agent.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "loginom-ai-agent.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "loginom-ai-agent.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "loginom-ai-agent.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".loginom-ai-agent", "loginom-ai-agent.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".loginom-ai-agent", "loginom-ai-agent.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".loginom-ai-agent")),
              AbsolutePath.make(path.join(directory, ".loginom-ai-agent")),
            ])

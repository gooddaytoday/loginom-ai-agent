...
import { ConfigExternalPlugin } from "@loginom-ai-agent/core/config/plugin/external"
...
import { PluginHost } from "@loginom-ai-agent/core/plugin/host"
...
describe("ConfigExternalPlugin", () => {
  it.live("resolves and loads a configured Promise plugin with options", () =>
    Effect.gen(function* () {
      ...
      const document = path.join(import.meta.dir, "loginom-ai-agent.json")
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Document({
                  type: "document",
                  path: path.join(import.meta.dir, "loginom-ai-agent.json"),
                  info: decode({
                    plugins: [
                      {

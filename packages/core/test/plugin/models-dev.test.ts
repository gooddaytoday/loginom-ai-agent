...
import { Integration } from "@loginom-ai-agent/core/integration"
...
import { ModelsDevPlugin } from "@loginom-ai-agent/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.LOGINOM_AI_AGENT_MODELS_PATH,
          disabled: Flag.LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH,
        }
        Flag.LOGINOM_AI_AGENT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.LOGINOM_AI_AGENT_MODELS_PATH = previous.path
          Flag.LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})

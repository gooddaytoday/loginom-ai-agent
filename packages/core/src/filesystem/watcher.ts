...
import { FileSystemWatcher } from "@loginom-ai-agent/schema/filesystem-watcher"
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof LOGINOM_AI_AGENT_LIBC === "undefined" ? undefined : LOGINOM_AI_AGENT_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@loginom-ai-agent/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER)) {

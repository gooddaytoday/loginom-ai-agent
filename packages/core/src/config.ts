...
export class Service extends Context.Service<Service, Interface>()("@loginom-ai-agent/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["loginom-ai-agent.json", "loginom-ai-agent.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".loginom-ai-agent", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".loginom-ai-agent")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".loginom-ai-agent").toReversed()
    ...
    // global config, project files, then `.loginom-ai-agent` files.

...
export const LOGINOM_AI_AGENT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const LOGINOM_AI_AGENT_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(LOGINOM_AI_AGENT_MODE_KEY, LOGINOM_AI_AGENT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(LOGINOM_AI_AGENT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(LOGINOM_AI_AGENT_MODE_KEY, stack.at(-1)?.mode ?? LOGINOM_AI_AGENT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? LOGINOM_AI_AGENT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(LOGINOM_AI_AGENT_MODE_KEY, undefined)

...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("loginom-ai-agent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("loginom-ai-agent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("loginom-ai-agent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("loginom-ai-agent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("loginom-ai-agent://open-project")).toBeUndefined()
    expect(parseDeepLink("loginom-ai-agent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "loginom-ai-agent://open-project?directory=/a",
      "loginom-ai-agent://other?directory=/b",
      "loginom-ai-agent://open-project?directory=/c",
    ])
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("loginom-ai-agent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("loginom-ai-agent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "loginom-ai-agent://new-session?directory=/a",
      "loginom-ai-agent://open-project?directory=/b",
      "loginom-ai-agent://new-session?directory=/c&prompt=ship%20it",
    ])

...
export async function startChromeTrace(page: Page, name: string) {
  const directory = process.env.LOGINOM_AI_AGENT_PERFORMANCE_TRACE_DIR
  if (!directory) return

  const selectors = process.env.LOGINOM_AI_AGENT_PERFORMANCE_SELECTOR_TRACE === "1"
...
export async function prepareChromeTrace(
  ...
  const run = process.env.LOGINOM_AI_AGENT_PERFORMANCE_RUN_ID ?? "manual"

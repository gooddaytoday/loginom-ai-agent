...
const directory = "C:/Loginom AI Agent/NewSessionPanelCorner"
...
test("matches the rounded panel corners to the dark new-session background", async ({ page }, testInfo) => {
  ...
  await page.addInitScript(
    ({ directory, draftID, server }) => {
      ...
      localStorage.setItem("loginom-ai-agent-theme-id", "oc-2")
      localStorage.setItem("loginom-ai-agent-color-scheme", "dark")
      ...
      localStorage.setItem(
        "loginom-ai-agent.window.browser.dat:tabs",

...
async function configureServers(page: Page, tabs: { type: "session"; server: string; sessionId: string }[] = []) {
  await page.addInitScript(
    ({ serverB, tabs }) => {
      ...
      localStorage.setItem("loginom-ai-agent.global.dat:server", JSON.stringify({ list: [serverB] }))
      localStorage.setItem("loginom-ai-agent.window.browser.dat:tabs", JSON.stringify(tabs))
    },
    { serverB, tabs },
  )
}

...
- Copy Solid store arrays into plain arrays before passing them to Desktop APIs: store proxies cannot cross Electron's contextBridge/IPC (`An object could not be cloned`). Recovery acknowledgement must send a plain ID list. A durable save held by recovery keeps the existing recovery notice and form open; it is not a connection failure. Installed regression: `packages/desktop/test/loginom/recovery-ui.mjs` with an isolated durable journal.

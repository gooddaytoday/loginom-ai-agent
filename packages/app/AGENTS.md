...
- Backend (from `packages/agent`): `bun run ./src/index.ts serve --port 4096`
...
- Loginom settings use `settings-loginom-state.ts`: check never saves; save validates, waits for immediate activation, then closes. Deferred activation is announced separately and monitored after closing. The key mask is a placeholder based only on the stored-key flag; never submit it as a credential. Close, Escape and the overlay share a dialog close guard for dirty drafts. Keep the general reload action absent; offer refresh only for a revision conflict. Regression: `test-browser/settings-loginom.test.ts` and Desktop `test/loginom/settings-ux.mjs` / `settings-live.mjs`.
- Copy Solid store arrays into plain arrays before passing them to Desktop APIs: store proxies cannot cross Electron's contextBridge/IPC (`An object could not be cloned`). Recovery acknowledgement must send a plain ID list. `form.recover()`, ключи `loginom.recoveryRequired` / `loginom.acknowledgeRecovery` и `acknowledgeRecovery` в API остаются, чтобы кнопку можно было вернуть. Сама кнопка и предупреждение в `settings-loginom.tsx` скрыты: мягкий режим не предлагает невыполнимое подтверждение. `packages/desktop/test/loginom/recovery-ui.mjs` сохранён, но не применяется, пока кнопка скрыта. Рендер-тест кнопки в Bun не поднимается: провайдеры Solid компилируются классическим JSX и ищут `React`.

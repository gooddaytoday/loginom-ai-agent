...
export async function loginPage(page, candidate) {
  ...
  if (!(await avatar.isVisible())) {
    ...
    await avatar.waitFor({ state: 'visible', timeout: 30_000 }).catch(() => { throw Error('LOGINOM_LOGIN_REJECTED'); });
  }
...
export async function checkKnowledge(endpoint, apiKey) {
  ...
  } catch { throw Error('LOGINOM_KNOWLEDGE_AUTH_FAILED'); }
...
export async function loginBrowser({ browserPath, profile, candidate, headless = false }) {
  ...
  }).catch(() => { throw Error('LOGINOM_BROWSER_START_FAILED'); });
  try {

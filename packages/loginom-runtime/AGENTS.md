...
- Delivery navigation IDs are `operation_id:nav<navigationStep>` and stay monotonic across resumes. A failure before upload with every gesture settled (a SUCCEEDED click or a matched no-effect refusal) records `artifact_delivery_preupload_checkpoint` and returns `cleanup_complete: true` with `next_step: dock_artifact_delivery_resume`; the resume keeps the admitted attachment and requires the same document. A lost reply or unmatched refusal leaves navigation uncertain: no resume, `inspection_required: true`. A settled NOT_APPLIED checkpoint with effect_possible false is not unsettled work.

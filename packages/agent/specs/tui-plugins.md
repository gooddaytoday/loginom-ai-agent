...
- Author package entrypoint is `@loginom-ai-agent/plugin/tui`.
...
- Import types from `@loginom-ai-agent/plugin/tui`.
- `@loginom-ai-agent/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@loginom-ai-agent/plugin/tui"
...
- For local scope, path is `<worktree>/.loginom-ai-agent` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.loginom-ai-agent`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.loginom-ai-agent`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.loginom-ai-agent` dir or `LOGINOM_AI_AGENT_CONFIG_DIR`, OpenCode installs `@loginom-ai-agent/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@loginom-ai-agent/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@loginom-ai-agent/plugin/tui`.
...
- Local plugins persist installed themes under the local `.loginom-ai-agent/themes` area near the plugin config source.
...
- `--pure` / `LOGINOM_AI_AGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.loginom-ai-agent/plugins/tui-smoke.tsx`
- Local vim plugin: `.loginom-ai-agent/plugins/tui-vim.tsx`
- Local smoke config: `.loginom-ai-agent/tui.json`
- Local smoke theme: `.loginom-ai-agent/plugins/smoke-theme.json`

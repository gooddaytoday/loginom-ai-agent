...
import { MAX_STEPS_PROMPT } from "@loginom-ai-agent/core/session/runner/max-steps"
...
export class Service extends Context.Service<Service, Interface>()("@loginom-ai-agent/SessionPrompt") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      const session = yield* sessions.get(input.sessionID).pipe(Effect.orDie)
      if (input.model) yield* getModel(input.model.providerID, input.model.modelID, input.sessionID)
      yield* revert.cleanup(session)

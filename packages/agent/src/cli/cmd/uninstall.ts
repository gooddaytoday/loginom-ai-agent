...
export const UninstallCommand = {
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Loginom AI Agent")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".loginom-ai-agent")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Loginom AI Agent!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".loginom-ai-agent/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".loginom-ai-agent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".loginom-ai-agent/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".loginom-ai-agent"))
    ) {
      continue
    }

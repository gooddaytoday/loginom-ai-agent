...
import { ConfigMCPV1 } from "@loginom-ai-agent/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in loginom-ai-agent.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .loginom-ai-agent/ subdirectory too)
  const candidates = [path.join(baseDir, "loginom-ai-agent.json"), path.join(baseDir, "loginom-ai-agent.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".loginom-ai-agent", "loginom-ai-agent.json"), path.join(baseDir, ".loginom-ai-agent", "loginom-ai-agent.jsonc"))
  }
  ...
  // Default to loginom-ai-agent.json if none exist
  return candidates[0]
}

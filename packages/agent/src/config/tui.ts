...
import { isRecord } from "@loginom-ai-agent/tui/util/record"
...
import { TuiKeybind } from "@loginom-ai-agent/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@loginom-ai-agent/core/installation/version"
import { makeRuntime } from "@loginom-ai-agent/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@loginom-ai-agent/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@loginom-ai-agent/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@loginom-ai-agent/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old loginom-ai-agent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.loginom-ai-agent`
  // folders between cwd and home, and LOGINOM_AI_AGENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.LOGINOM_AI_AGENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit LOGINOM_AI_AGENT_TUI_CONFIG override, if set.
  if (Flag.LOGINOM_AI_AGENT_TUI_CONFIG) {
    const configFile = Flag.LOGINOM_AI_AGENT_TUI_CONFIG
  ...
  // 4. `.loginom-ai-agent` directories (and LOGINOM_AI_AGENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".loginom-ai-agent") || dir === Flag.LOGINOM_AI_AGENT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".loginom-ai-agent") && dir !== Flag.LOGINOM_AI_AGENT_CONFIG_DIR) continue

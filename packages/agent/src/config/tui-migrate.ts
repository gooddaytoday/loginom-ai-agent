...
const TUI_SCHEMA_URL = "urn:loginom-ai-agent:tui"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from loginom-ai-agent.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "loginom-ai-agent"),
    ...(await Filesystem.findUp(["loginom-ai-agent.json", "loginom-ai-agent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "loginom-ai-agent"))
  }
  if (Flag.LOGINOM_AI_AGENT_CONFIG) files.push(Flag.LOGINOM_AI_AGENT_CONFIG)

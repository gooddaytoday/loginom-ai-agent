...
import { serviceUse } from "@loginom-ai-agent/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@loginom-ai-agent/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@loginom-ai-agent/core/v1/config/console-state"
...
import { RemoteAuthError } from "@loginom-ai-agent/core/v1/config/error"
import { ConfigPermissionV1 } from "@loginom-ai-agent/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@loginom-ai-agent/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["loginom-ai-agent.jsonc", "loginom-ai-agent.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "urn:loginom-ai-agent:config"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.LOGINOM_AI_AGENT_CONFIG && !Flag.LOGINOM_AI_AGENT_CONFIG_DIR && !Flag.LOGINOM_AI_AGENT_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "urn:loginom-ai-agent:config" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "loginom-ai-agent.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "loginom-ai-agent.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "urn:loginom-ai-agent:config"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LOGINOM_AI_AGENT_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/loginom-ai-agent`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "urn:loginom-ai-agent:config"
        ...
        if (Flag.LOGINOM_AI_AGENT_CONFIG) {
          yield* merge(Flag.LOGINOM_AI_AGENT_CONFIG, yield* loadFile(Flag.LOGINOM_AI_AGENT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.LOGINOM_AI_AGENT_CONFIG })
        }

        if (!Flag.LOGINOM_AI_AGENT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("loginom-ai-agent", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.LOGINOM_AI_AGENT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from LOGINOM_AI_AGENT_CONFIG_DIR", { path: Flag.LOGINOM_AI_AGENT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".loginom-ai-agent") || dir === Flag.LOGINOM_AI_AGENT_CONFIG_DIR) {
            for (const file of ["loginom-ai-agent.json", "loginom-ai-agent.jsonc"]) {
          ...
          // Auto-discovered plugins under `.loginom-ai-agent/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.LOGINOM_AI_AGENT_CONFIG_CONTENT) {
          const source = "LOGINOM_AI_AGENT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LOGINOM_AI_AGENT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from LOGINOM_AI_AGENT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LOGINOM_AI_AGENT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LOGINOM_AI_AGENT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LOGINOM_AI_AGENT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LOGINOM_AI_AGENT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("LOGINOM_AI_AGENT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LOGINOM_AI_AGENT_DISABLE_PRUNE) {

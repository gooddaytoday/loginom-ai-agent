...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.LOGINOM_AI_AGENT_TEST ?? "")'), {
      env: {
        LOGINOM_AI_AGENT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "LOGINOM_AI_AGENT_TEST_SHELL"], {
      shell: true,
      env: {
        LOGINOM_AI_AGENT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("LOGINOM_AI_AGENT_TEST_SHELL=ok")
  })

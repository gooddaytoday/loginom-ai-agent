...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "loginom-ai-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "loginom-ai-agent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "loginom-ai-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "loginom-ai-agent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["loginom-ai-agent.json", "loginom-ai-agent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "loginom-ai-agent.json"),
        path.join(project, "loginom-ai-agent.jsonc"),
      ])
    })
  })

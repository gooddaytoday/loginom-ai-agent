...
import { EventSequenceTable } from "@loginom-ai-agent/core/event/sql"
...
import { WorkspaceV2 } from "@loginom-ai-agent/core/workspace"
import { WorkspaceTable } from "@loginom-ai-agent/core/control-plane/workspace.sql"
...
const originalEnv = {
  LOGINOM_AI_AGENT_AUTH_CONTENT: process.env.LOGINOM_AI_AGENT_AUTH_CONTENT,
  LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES: process.env.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.LOGINOM_AI_AGENT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.LOGINOM_AI_AGENT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.LOGINOM_AI_AGENT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES).toBe("true")

...
import { HttpRecorder } from "@loginom-ai-agent/http-recorder"
import { HttpRecorderInternal } from "@loginom-ai-agent/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@loginom-ai-agent/llm"
import { RequestExecutor } from "@loginom-ai-agent/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.LOGINOM_AI_AGENT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("LOGINOM_AI_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("LOGINOM_AI_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    canRecord: () => Boolean(process.env.LOGINOM_AI_AGENT_RECORD_CONSOLE_TOKEN && process.env.LOGINOM_AI_AGENT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        env: ["LOGINOM_AI_AGENT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.LOGINOM_AI_AGENT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.LOGINOM_AI_AGENT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.LOGINOM_AI_AGENT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("LOGINOM_AI_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("LOGINOM_AI_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("LOGINOM_AI_AGENT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set LOGINOM_AI_AGENT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."

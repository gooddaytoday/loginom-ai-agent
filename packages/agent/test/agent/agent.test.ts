...
it.instance("plan agent denies edits except .loginom-ai-agent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".loginom-ai-agent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".loginom-ai-agent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.LOGINOM_AI_AGENT_TEST_HOME
      process.env.LOGINOM_AI_AGENT_TEST_HOME = test.directory

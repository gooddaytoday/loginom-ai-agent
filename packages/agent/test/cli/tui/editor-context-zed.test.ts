...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@loginom-ai-agent/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.LOGINOM_AI_AGENT_ZED_DB
  process.env.LOGINOM_AI_AGENT_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.LOGINOM_AI_AGENT_ZED_DB
    else process.env.LOGINOM_AI_AGENT_ZED_DB = previous
    home.mockRestore()
  }
})

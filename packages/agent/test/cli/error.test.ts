...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/loginom-ai-agent.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/loginom-ai-agent.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/loginom-ai-agent.jsonc", dir: ".loginom-ai-agent", suggestion: "opencode" },
        expected:
          'Directory ".loginom-ai-agent" in /tmp/loginom-ai-agent.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/loginom-ai-agent.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/loginom-ai-agent.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Or check your config (loginom-ai-agent.json) provider/model names",
    ].join("\n")

...
const original = {
  LOGINOM_AI_AGENT_SERVER_PASSWORD: Flag.LOGINOM_AI_AGENT_SERVER_PASSWORD,
  LOGINOM_AI_AGENT_SERVER_USERNAME: Flag.LOGINOM_AI_AGENT_SERVER_USERNAME,
  envPassword: process.env.LOGINOM_AI_AGENT_SERVER_PASSWORD,
  envUsername: process.env.LOGINOM_AI_AGENT_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.LOGINOM_AI_AGENT_SERVER_PASSWORD
  else process.env.LOGINOM_AI_AGENT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.LOGINOM_AI_AGENT_SERVER_USERNAME
  else process.env.LOGINOM_AI_AGENT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.LOGINOM_AI_AGENT_SERVER_PASSWORD = auth.password
  Flag.LOGINOM_AI_AGENT_SERVER_USERNAME = auth.username
  process.env.LOGINOM_AI_AGENT_SERVER_PASSWORD = auth.password
  process.env.LOGINOM_AI_AGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.LOGINOM_AI_AGENT_SERVER_PASSWORD = undefined
  Flag.LOGINOM_AI_AGENT_SERVER_USERNAME = auth.username
  delete process.env.LOGINOM_AI_AGENT_SERVER_PASSWORD
  process.env.LOGINOM_AI_AGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS
    process.env.LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS
      else process.env.LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS = previous
    }
  })

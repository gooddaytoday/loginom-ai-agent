...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const originalWorkspaces = Flag.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES
    Flag.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.promise(() => resetDatabase())
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES = originalWorkspaces
        await resetDatabase()
      }),
    )
  }),
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.LOGINOM_AI_AGENT_WORKSPACE_ID
      Flag.LOGINOM_AI_AGENT_WORKSPACE_ID = WorkspaceV2.ID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.LOGINOM_AI_AGENT_WORKSPACE_ID = originalWorkspaceID
        }),
      )

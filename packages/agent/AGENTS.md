...
- OAuth errors in `src/plugin/openai/codex.ts` expose only status and allowlisted error codes. Model and OAuth traffic use the system proxy translation from `docs/superpowers/specs/2026-09-23-system-proxy-v2.md`. Node WebSockets for OpenAI Responses must pass `HttpsProxyAgent` because `ws` ignores proxy environment variables.

...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `loginom-ai-agent.json`을 사용하십시오:
...
OpenCode를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `loginom-ai-agent.json` 파일에 다음을 추가하십시오.
...
`loginom-ai-agent.json`을 통해 사용자 정의:
...
4. 프로젝트 디렉토리에 `loginom-ai-agent.json` 파일을 만들거나 업데이트하십시오.

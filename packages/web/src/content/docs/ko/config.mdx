...
2. **Global config**(`~/.config/opencode/loginom-ai-agent.json`) - 사용자 기본 설정
3. **Custom config**(`LOGINOM_AI_AGENT_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `loginom-ai-agent.json`) - 프로젝트별 설정
5. **`.loginom-ai-agent` directories** - agents, commands, plugins
6. **Inline config**(`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - 런타임 override

즉, 프로젝트 config는 전역 기본값을 override할 수 있고, 전역 config는 조직의 Remote 기본값을 override할 수 있습니다.

:::note
`.loginom-ai-agent` 및 `~/.config/opencode` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
전역 OpenCode config는 `~/.config/opencode/loginom-ai-agent.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.
...
프로젝트 루트에 `loginom-ai-agent.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
`LOGINOM_AI_AGENT_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`LOGINOM_AI_AGENT_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.loginom-ai-agent` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.loginom-ai-agent` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.
...
`LOGINOM_AI_AGENT_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.

`loginom-ai-agent.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
...
`~/.config/opencode/agents/` 또는 `.loginom-ai-agent/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
`~/.config/opencode/commands/` 또는 `.loginom-ai-agent/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.loginom-ai-agent/plugins/` 또는 `~/.config/opencode/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.

...
- 프로젝트 구성: `.loginom-ai-agent/skills/<name>/SKILL.md`
...
그것은 `skills/*/SKILL.md`에 있는 어떤 어울리는 `.loginom-ai-agent/` 및 어떤 어울리는 `.claude/skills/*/SKILL.md` 또는 `.agents/skills/*/SKILL.md`를 방법 적재합니다.
...
이처럼 `.loginom-ai-agent/skills/git-release/SKILL.md` 만들기:
...
기술 에이전트가 `loginom-ai-agent.json`의 패턴 기반 권한을 사용하여 액세스 할 수있는 제어 :
...
** 내장 에이전트 ** (`loginom-ai-agent.json`에서):

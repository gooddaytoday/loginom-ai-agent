...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc`（或 `~/.config/opencode/loginom-ai-agent.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
如果你的 `loginom-ai-agent.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。

#### 檢查環境變數

如果你在環境中設定了 `LOGINOM_AI_AGENT_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `LOGINOM_AI_AGENT_PORT`（或選擇一個空閒連接埠）並重新啟動。
...
- `loginom-ai-agent.settings.dat`（桌面預設伺服器 URL）

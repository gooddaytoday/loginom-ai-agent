...
設定 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `LOGINOM_AI_AGENT_SERVER_USERNAME` 來覆蓋它。這適用於 `opencode serve` 和 `opencode web`。

...
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本驗證密碼（預設使用 `LOGINOM_AI_AGENT_SERVER_PASSWORD`）                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本驗證使用者名稱（預設使用 `LOGINOM_AI_AGENT_SERVER_USERNAME` 或 `opencode`） |
...
此指令啟動一個 HTTP 伺服器，提供對 OpenCode 功能的 API 存取，無需 TUI 介面。設定 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 OpenCode。設定 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
| 變數                                  | 類型    | 說明                                        |
| ------------------------------------- | ------- | ------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | 自動分享工作階段                            |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可執行檔的路徑          |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | 設定檔路徑                                  |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | TUI 設定檔路徑                              |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | 設定目錄路徑                                |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | 內嵌 JSON 設定內容                          |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | 停用自動更新檢查                            |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | 停用舊資料清理                              |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | 停用自動終端機標題更新                      |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | 內嵌 JSON 權限設定                          |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | 停用預設外掛程式                            |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | 停用 LSP 伺服器自動下載                     |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | 啟用實驗性模型                              |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | 停用自動上下文壓縮                          |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | 停用讀取 `.claude`（提示詞 + 技能）         |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 停用讀取 `~/.claude/CLAUDE.md`              |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 停用載入 `.claude/skills`                   |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | 停用從遠端來源擷取模型                      |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | 用於測試目的的模擬 VCS 供應商               |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | 用戶端識別碼（預設為 `cli`）                |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | 啟用 Exa 網路搜尋工具                       |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | 為 `serve`/`web` 啟用基本認證               |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `opencode`） |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | 自訂模型設定擷取 URL                        |
...
| 變數                                            | 類型    | 說明                            |
| ----------------------------------------------- | ------- | ------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | 啟用受總開關控制的實驗性功能    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 啟用圖示探索                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 停用 TUI 中的選取即複製         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 指令的預設逾時時間（毫秒） |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 回應的最大輸出 Token 數     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | 啟用整個目錄的檔案監看器        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | 啟用 oxfmt 格式化器             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | 啟用實驗性 LSP 工具             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 停用檔案監看器                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | 啟用實驗性 Exa 功能             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | 為 python 檔案啟用 TY LSP       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | 啟用計畫模式                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | 啟用背景子代理任務              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | 啟用實驗性事件系統              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | 啟用原生 LLM 請求路徑           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | 啟用平行 Web 搜尋執行           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | 啟用 Scout 子代理               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | 啟用工作區支援                  |

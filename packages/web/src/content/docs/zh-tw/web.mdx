...
:::caution
如果未設定 `LOGINOM_AI_AGENT_SERVER_PASSWORD`，伺服器將沒有安全保護。本地使用沒有問題，但在網路存取時應當設定密碼。
:::
...
要保護伺服器存取，可以透過 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 環境變數設定密碼：

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

使用者名稱預設為 `opencode`，可以透過 `LOGINOM_AI_AGENT_SERVER_USERNAME` 進行更改。
...
你也可以在 `loginom-ai-agent.json` 設定檔中設定伺服器選項：

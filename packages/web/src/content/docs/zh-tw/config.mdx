...
2. **全域設定**（`~/.config/opencode/loginom-ai-agent.json`）- 使用者偏好
3. **自訂設定**（`LOGINOM_AI_AGENT_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `loginom-ai-agent.json`）- 專案特定設定
5. **`.loginom-ai-agent` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`LOGINOM_AI_AGENT_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.loginom-ai-agent` 和 `~/.config/opencode` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
將全域 OpenCode 設定放在 `~/.config/opencode/loginom-ai-agent.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
在專案根目錄中新增 `loginom-ai-agent.json`。專案設定在標準設定檔中具有最高優先級——它會覆寫全域設定和遠端設定。
...
使用 `LOGINOM_AI_AGENT_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `LOGINOM_AI_AGENT_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.loginom-ai-agent` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.loginom-ai-agent` 目錄之後載入，因此**可以覆寫**它們的設定。
...
使用 `LOGINOM_AI_AGENT_TUI_CONFIG` 指向自訂 TUI 設定檔。

`loginom-ai-agent.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
您還可以使用 `~/.config/opencode/agents/` 或 `.loginom-ai-agent/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
您還可以使用 `~/.config/opencode/commands/` 或 `.loginom-ai-agent/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.loginom-ai-agent/plugins/` 或 `~/.config/opencode/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。

...
:::note
該工具僅在設定 `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true`（或 `LOGINOM_AI_AGENT_EXPERIMENTAL=true`）時可用。
:::
...
:::note
該工具僅在使用 OpenCode 供應商時，或當 `LOGINOM_AI_AGENT_ENABLE_EXA` 環境變數設定為任意真值（例如 `true` 或 `1`）時可用。

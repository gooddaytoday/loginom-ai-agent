...
2. 使用以下方法之一**設定身分驗證**：
   ...
   如需專案級別或持久化的設定，請使用 `loginom-ai-agent.json`：
...
:::note[合規說明]
OpenCode 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 OpenCode 僅使用您自己的 GitLab 託管實例，請在 `loginom-ai-agent.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
透過 `loginom-ai-agent.json` 進行自訂設定：
...
4. 在專案目錄中建立或更新 `loginom-ai-agent.json` 檔案：

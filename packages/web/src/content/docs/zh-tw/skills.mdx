...
- 專案設定：`.loginom-ai-agent/skills/<name>/SKILL.md`
...
在此過程中，它會載入 `.loginom-ai-agent/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。
...
建立 `.loginom-ai-agent/skills/git-release/SKILL.md`，內容如下：
...
在 `loginom-ai-agent.json` 中使用基於模式的權限來控制代理可以存取哪些技能：
...
**內建代理**（在 `loginom-ai-agent.json` 中）：

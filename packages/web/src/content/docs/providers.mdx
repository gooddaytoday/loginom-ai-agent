...
```json title="loginom-ai-agent.json" {6}
{
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `loginom-ai-agent.json`:
...
```json title="loginom-ai-agent.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
`loginom-ai-agent.json` file. It is also recommended to disable session sharing.
...
Customize through `loginom-ai-agent.json`:
...
```jsonc title="~/.config/opencode/loginom-ai-agent.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
```json title="loginom-ai-agent.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="loginom-ai-agent.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
```json title="loginom-ai-agent.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
The model catalog is provided automatically. A minimal `loginom-ai-agent.json` is all that's needed:
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in loginom-ai-agent.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `loginom-ai-agent.json` file in your project directory:

   ```json title="loginom-ai-agent.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="loginom-ai-agent.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {

...
2. **الإعدادات العامة** (`~/.config/opencode/loginom-ai-agent.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`LOGINOM_AI_AGENT_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`loginom-ai-agent.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.loginom-ai-agent`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.loginom-ai-agent` و`~/.config/opencode` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
ضع إعدادات OpenCode العامة في `~/.config/opencode/loginom-ai-agent.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `loginom-ai-agent.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `LOGINOM_AI_AGENT_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `LOGINOM_AI_AGENT_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.loginom-ai-agent`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.loginom-ai-agent`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/opencode/agents/` أو `.loginom-ai-agent/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/opencode/commands/` أو `.loginom-ai-agent/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.loginom-ai-agent/plugins/` أو `~/.config/opencode/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.

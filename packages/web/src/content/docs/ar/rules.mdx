...
يمكنك تحديد ملفات تعليمات مخصصة في `loginom-ai-agent.json` أو في الملف العام `~/.config/opencode/loginom-ai-agent.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام loginom-ai-agent.json

النهج الموصى به هو استخدام الحقل `instructions` في `loginom-ai-agent.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `loginom-ai-agent.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::

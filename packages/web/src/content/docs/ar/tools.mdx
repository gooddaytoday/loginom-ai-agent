...
:::note
هذه الأداة متاحة فقط عند ضبط `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (أو `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `LOGINOM_AI_AGENT_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

...
:::caution
Si `LOGINOM_AI_AGENT_SERVER_PASSWORD` n'est pas défini, le serveur ne sera pas sécurisé. C'est bien pour une utilisation locale mais doit être défini pour l'accès au réseau.
:::
...
Pour protéger l'accès, définissez un mot de passe à l'aide de la variable d'environnement `LOGINOM_AI_AGENT_SERVER_PASSWORD` :

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Le nom d'utilisateur par défaut est `opencode` mais peut être modifié avec `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Vous pouvez également configurer les paramètres du serveur dans votre fichier de configuration `loginom-ai-agent.json` :

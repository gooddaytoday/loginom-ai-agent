...
:::note
Cet outil n'est disponible que lorsque `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ou `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `LOGINOM_AI_AGENT_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

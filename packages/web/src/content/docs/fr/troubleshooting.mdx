...
- **macOS/Linux** : `~/.config/opencode/loginom-ai-agent.jsonc` (ou `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
- **Plugins de projet** (uniquement si vous utilisez une configuration par projet)
  - `<your-project>/.loginom-ai-agent/plugins/`
...
Si votre `loginom-ai-agent.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `LOGINOM_AI_AGENT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `LOGINOM_AI_AGENT_PORT` (ou choisissez un port libre) et redémarrez.
...
- `loginom-ai-agent.settings.dat` (serveur de bureau par défaut URL)

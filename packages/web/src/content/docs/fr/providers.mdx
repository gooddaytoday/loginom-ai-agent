...
Pour une configuration spécifique au projet ou persistante, utilisez `loginom-ai-agent.json` :
...
Fichier `loginom-ai-agent.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `loginom-ai-agent.json` :
...
4. Créez ou mettez à jour votre fichier `loginom-ai-agent.json` dans le répertoire de votre projet :

...
```jsonc title="loginom-ai-agent.jsonc"
{
...
2. **Configuration globale** (`~/.config/opencode/loginom-ai-agent.json`) - préférences utilisateur
3. **Configuration personnalisée** (`LOGINOM_AI_AGENT_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`loginom-ai-agent.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.loginom-ai-agent`** - agents, commandes, plugins
6. **Configuration en ligne** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.loginom-ai-agent` et `~/.config/opencode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
```json title="loginom-ai-agent.json"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale OpenCode dans `~/.config/opencode/loginom-ai-agent.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `loginom-ai-agent.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `LOGINOM_AI_AGENT_CONFIG`.

```bash
export LOGINOM_AI_AGENT_CONFIG=/path/to/my/custom-config.json
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `LOGINOM_AI_AGENT_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.loginom-ai-agent`, et devrait suivre la même structure.

```bash
export LOGINOM_AI_AGENT_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.loginom-ai-agent`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `LOGINOM_AI_AGENT_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `loginom-ai-agent.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.loginom-ai-agent/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.loginom-ai-agent/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.loginom-ai-agent/plugins/` ou `~/.config/opencode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
{
  "$schema": "https://opencode.ai/config.json",
  "model": "{env:LOGINOM_AI_AGENT_MODEL}",
  "provider": {
    "anthropic": {
      "models": {},
      "options": {

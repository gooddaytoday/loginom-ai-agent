...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `LOGINOM_AI_AGENT_SERVER_USERNAME` ou `opencode`) |
...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCode sans l'interface TUI. Définissez `LOGINOM_AI_AGENT_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCode via une interface Web. Définissez `LOGINOM_AI_AGENT_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `LOGINOM_AI_AGENT_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | chaîne  | Chemin d'accès au fichier de configuration TUI                                    |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `LOGINOM_AI_AGENT_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `LOGINOM_AI_AGENT_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                       | Type    | Description                                                     |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | booléen | Activer les fonctionnalités expérimentales du drapeau global    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | booléen | Activer TY LSP pour les fichiers python                         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | booléen | Activer les tâches de sous-agents en arrière-plan               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | booléen | Activer le système d'événements expérimental                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | booléen | Activer le chemin de requête LLM natif                          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | booléen | Activer l'exécution parallèle de la recherche web               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | booléen | Activer le sous-agent Scout                                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | booléen | Activer la prise en charge des espaces de travail               |

...
```bash
export LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `loginom-ai-agent.json` ou dans le `~/.config/opencode/loginom-ai-agent.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de loginom-ai-agent.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `loginom-ai-agent.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `loginom-ai-agent.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::

...
```bash
npm install @loginom-ai-agent/sdk
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `loginom-ai-agent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
```typescript
import type { Session, Message, Part } from "@loginom-ai-agent/sdk"
```

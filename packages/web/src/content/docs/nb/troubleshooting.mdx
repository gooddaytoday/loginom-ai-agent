...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (eller `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Hvis `loginom-ai-agent.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `LOGINOM_AI_AGENT_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `LOGINOM_AI_AGENT_PORT` (eller velg en ledig port) og start på nytt.
...
- `loginom-ai-agent.settings.dat` (stasjonær standardserver URL)

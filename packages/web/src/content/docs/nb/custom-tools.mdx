...
- Lokalt ved å plassere dem i `.loginom-ai-agent/tools/`-katalogen til prosjektet ditt.
...
```ts title=".loginom-ai-agent/tools/database.ts" {1}
...
```ts title=".loginom-ai-agent/tools/math.ts"
import { tool } from "@loginom-ai-agent/plugin"
...
```ts title=".loginom-ai-agent/tools/bash.ts"
import { tool } from "@loginom-ai-agent/plugin"
...
```ts title=".loginom-ai-agent/tools/project.ts" {8}
import { tool } from "@loginom-ai-agent/plugin"
...
```python title=".loginom-ai-agent/tools/add.py"
import sys
...
```ts title=".loginom-ai-agent/tools/python-add.ts" {10}
import { tool } from "@loginom-ai-agent/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".loginom-ai-agent/tools/add.py")

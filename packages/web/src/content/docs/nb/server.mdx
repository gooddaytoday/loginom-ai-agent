...
Sett `LOGINOM_AI_AGENT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `LOGINOM_AI_AGENT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencode serve` og `opencode web`.

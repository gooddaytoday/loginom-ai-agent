...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Økt ID for å fortsette                                                                             |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren økten ved fortsettelse (bruk med `--continue` eller `--session`)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `LOGINOM_AI_AGENT_SERVER_USERNAME` eller `opencode`) |
...
Dette starter en HTTP-server som gir API tilgang til OpenCode-funksjonalitet uten TUI-grensesnittet. Sett `LOGINOM_AI_AGENT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenCode via et nettgrensesnitt. Sett `LOGINOM_AI_AGENT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
| ------------------------------------- | ------ | --------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolsk | Del økter automatisk                                                  |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | streng | Bane til Git Bash-kjørbar på Windows                                  |
| `LOGINOM_AI_AGENT_CONFIG`                     | streng | Bane til konfigurasjonsfil                                            |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | streng | Bane til TUI-konfigurasjonsfil                                        |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | streng | Bane til konfigurasjonskatalog                                        |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | streng | Innebygd json-konfigurasjonsinnhold                                   |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolsk | Deaktiver automatiske oppdateringskontroller                          |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolsk | Deaktiver beskjæring av gamle data                                    |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolsk | Deaktiver automatiske terminaltitteloppdateringer                     |
| `LOGINOM_AI_AGENT_PERMISSION`                 | streng | Innebygd json-tillatelseskonfigurasjon                                |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolsk | Deaktiver standard plugins                                            |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolsk | Deaktiver automatiske LSP servernedlastinger                          |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolsk | Aktiver eksperimentelle modeller                                      |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolsk | Deaktiver automatisk kontekstkomprimering                             |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolsk | Deaktiver lesing fra `.claude` (spørsmål + ferdigheter)               |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolsk | Deaktiver lesing `~/.claude/CLAUDE.md`                                |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolsk | Deaktiver innlasting av `.claude/skills`                              |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolsk | Deaktiver henting av modeller fra eksterne kilder                     |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | streng | Falsk VCS-leverandør for testformål                                   |
| `LOGINOM_AI_AGENT_CLIENT`                     | streng | Klientidentifikator (standard til `cli`)                              |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolsk | Aktiver Exa-nettsøkeverktøy                                           |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | streng | Aktiver grunnleggende autentisering for `serve`/`web`                 |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | streng | Egendefinert URL for henting av modellkonfigurasjon                   |
...
| Variabel                                        | Type   | Beskrivelse                                           |
| ----------------------------------------------- | ------ | ----------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolsk | Aktiver eksperimentelle funksjoner under samleflagget |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolsk | Aktiver ikonoppdagelse                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolsk | Deaktiver kopi ved valg i TUI                         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | tall   | Standard tidsavbrudd for bash-kommandoer i ms         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | tall   | Maks Output Tokens for LLM-svar                       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolsk | Aktiver filovervåker for hele dir                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolsk | Aktiver oxfmt formatter                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolsk | Aktiver eksperimentelt LSP-verktøy                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolsk | Deaktiver filovervåking                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolsk | Aktiver eksperimentelle Exa-funksjoner                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolsk | Aktiver TY LSP for python-filer                       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolsk | Aktiver planmodus                                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolsk | Aktiver bakgrunnsoppgaver for subagenter              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolsk | Aktiver eksperimentelt hendelsessystem                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolsk | Aktiver innebygd LLM-forespørselsvei                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolsk | Aktiver parallell kjøring av websøk                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolsk | Aktiver Scout-subagent                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolsk | Aktiver arbeidsområde-støtte                          |

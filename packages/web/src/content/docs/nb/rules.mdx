...
Du kan spesifisere egendefinerte instruksjonsfiler i din `loginom-ai-agent.json` eller den globale `~/.config/opencode/loginom-ai-agent.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker loginom-ai-agent.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `loginom-ai-agent.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `loginom-ai-agent.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::

...
2. **Global konfigurasjon** (`~/.config/opencode/loginom-ai-agent.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`LOGINOM_AI_AGENT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`loginom-ai-agent.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.loginom-ai-agent` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.loginom-ai-agent` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/opencode/loginom-ai-agent.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `loginom-ai-agent.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `LOGINOM_AI_AGENT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `LOGINOM_AI_AGENT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.loginom-ai-agent` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.loginom-ai-agent`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.loginom-ai-agent/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.loginom-ai-agent/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.loginom-ai-agent/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.

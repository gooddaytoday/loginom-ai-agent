...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `loginom-ai-agent.json` konfigurasjonsfilen:

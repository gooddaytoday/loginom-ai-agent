...
:::note
Dette verktøyet er bare tilgjengelig når `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (eller `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `LOGINOM_AI_AGENT_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

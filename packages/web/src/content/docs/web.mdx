...
:::caution
If `LOGINOM_AI_AGENT_SERVER_PASSWORD` is not set, the server will be unsecured. This is fine for local use but should be set for network access.
:::
...
To protect access, set a password using the `LOGINOM_AI_AGENT_SERVER_PASSWORD` environment variable:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

The username defaults to `opencode` but can be changed with `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
You can also configure server settings in your `loginom-ai-agent.json` config file:

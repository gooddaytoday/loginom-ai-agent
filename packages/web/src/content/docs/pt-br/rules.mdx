...
Você pode especificar arquivos de instrução personalizados no seu `loginom-ai-agent.json` ou no global `~/.config/opencode/loginom-ai-agent.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
...
### Usando loginom-ai-agent.json

A abordagem recomendada é usar o campo `instructions` em `loginom-ai-agent.json`:
...
:::tip
Para monorepos ou projetos com padrões compartilhados, usar `loginom-ai-agent.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::

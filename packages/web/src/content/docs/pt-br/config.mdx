...
2. **Configuração global** (`~/.config/opencode/loginom-ai-agent.json`) - preferências do usuário
3. **Configuração personalizada** (`LOGINOM_AI_AGENT_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`loginom-ai-agent.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.loginom-ai-agent`** - agentes, comandos, plugins
6. **Configuração inline** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.loginom-ai-agent` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/opencode/loginom-ai-agent.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `loginom-ai-agent.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `LOGINOM_AI_AGENT_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `LOGINOM_AI_AGENT_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.loginom-ai-agent`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.loginom-ai-agent`, então ele **pode substituir** suas configurações.
...
Use `LOGINOM_AI_AGENT_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `loginom-ai-agent.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.loginom-ai-agent/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.loginom-ai-agent/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.loginom-ai-agent/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.

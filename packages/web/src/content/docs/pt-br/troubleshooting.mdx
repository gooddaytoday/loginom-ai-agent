...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Se seu `loginom-ai-agent.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `LOGINOM_AI_AGENT_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `LOGINOM_AI_AGENT_PORT` (ou escolha uma porta livre) e reinicie.
...
- `loginom-ai-agent.settings.dat` (URL do servidor padrão do desktop)

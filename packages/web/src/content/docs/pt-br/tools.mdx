...
:::note
Esta ferramenta está disponível apenas quando `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ou `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `LOGINOM_AI_AGENT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

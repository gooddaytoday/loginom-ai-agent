...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `LOGINOM_AI_AGENT_SERVER_USERNAME` ou `opencode`) |
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `LOGINOM_AI_AGENT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `LOGINOM_AI_AGENT_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |

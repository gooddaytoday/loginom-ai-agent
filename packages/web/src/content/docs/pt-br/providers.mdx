...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `loginom-ai-agent.json`:
...
arquivo `loginom-ai-agent.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `loginom-ai-agent.json`:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no loginom-ai-agent.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `loginom-ai-agent.json` no diretório do seu projeto:

...
:::caution
Se `LOGINOM_AI_AGENT_SERVER_PASSWORD` não estiver definido, o servidor ficará sem segurança. Isso é aceitável para uso local, mas deve ser configurado para acesso à rede.
:::
...
Para proteger o acesso, defina uma senha usando a variável de ambiente `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

O nome de usuário padrão é `opencode`, mas pode ser alterado com `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Você também pode configurar as configurações do servidor no seu arquivo de configuração `loginom-ai-agent.json`:

...
:::note
You can disable automatic LSP server downloads by setting the `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
```json title="loginom-ai-agent.json" {5-8}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="loginom-ai-agent.json" {5-13}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "custom-lsp": {

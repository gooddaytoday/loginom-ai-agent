...
2. **Config globale** (`~/.config/opencode/loginom-ai-agent.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `LOGINOM_AI_AGENT_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`loginom-ai-agent.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.loginom-ai-agent`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `LOGINOM_AI_AGENT_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.loginom-ai-agent` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/opencode/loginom-ai-agent.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `loginom-ai-agent.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `LOGINOM_AI_AGENT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `LOGINOM_AI_AGENT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.loginom-ai-agent` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.loginom-ai-agent`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `LOGINOM_AI_AGENT_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `loginom-ai-agent.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.loginom-ai-agent/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.loginom-ai-agent/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.loginom-ai-agent/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.

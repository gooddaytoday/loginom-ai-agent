...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `LOGINOM_AI_AGENT_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `LOGINOM_AI_AGENT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `LOGINOM_AI_AGENT_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |

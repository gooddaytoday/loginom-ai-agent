...
:::note
Questo strumento e' disponibile solo quando `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (oppure `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `LOGINOM_AI_AGENT_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

...
Puoi specificare file di istruzioni personalizzati nel tuo `loginom-ai-agent.json` o nel globale `~/.config/opencode/loginom-ai-agent.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
### Usa loginom-ai-agent.json

L'approccio consigliato e' usare il campo `instructions` in `loginom-ai-agent.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `loginom-ai-agent.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::

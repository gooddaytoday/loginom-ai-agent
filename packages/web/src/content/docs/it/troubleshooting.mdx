...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Se il tuo `loginom-ai-agent.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `LOGINOM_AI_AGENT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `LOGINOM_AI_AGENT_PORT` (o scegli una porta libera) e riavvia.

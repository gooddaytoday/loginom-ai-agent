...
:::caution
Se `LOGINOM_AI_AGENT_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Puoi anche configurare le impostazioni del server nel file di config `loginom-ai-agent.json`:

...
Puede especificar archivos de instrucciones personalizados en su `loginom-ai-agent.json` o el `~/.config/opencode/loginom-ai-agent.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
### Usar loginom-ai-agent.json

El enfoque recomendado es utilizar el campo `instructions` en `loginom-ai-agent.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `loginom-ai-agent.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::

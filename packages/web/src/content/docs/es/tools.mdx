...
:::note
Esta herramienta solo está disponible cuando `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (o `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `LOGINOM_AI_AGENT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

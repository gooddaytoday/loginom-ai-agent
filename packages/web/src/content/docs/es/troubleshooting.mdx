...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (o `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Si su `loginom-ai-agent.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LOGINOM_AI_AGENT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LOGINOM_AI_AGENT_PORT` (o elija un puerto libre) y reinicie.
...
- `loginom-ai-agent.settings.dat` (URL del servidor predeterminado de escritorio)

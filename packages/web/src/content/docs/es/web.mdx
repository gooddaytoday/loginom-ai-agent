...
:::caution
Si no se establece `LOGINOM_AI_AGENT_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `loginom-ai-agent.json`:

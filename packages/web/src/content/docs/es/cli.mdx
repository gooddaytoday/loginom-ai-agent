...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | ID de sesión para continuar                                                               |
| <nobr><code>{"--fork"}</code></nobr>     |       | Bifurcar la sesión al continuar (usar con `--continue` o `--session`)                     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `LOGINOM_AI_AGENT_SERVER_USERNAME` u `opencode`) |
...
Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad opencode sin la interfaz TUI. Configure `LOGINOM_AI_AGENT_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Esto inicia un servidor HTTP y abre un navegador web para acceder a OpenCode a través de una interfaz web. Configure `LOGINOM_AI_AGENT_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
| ------------------------------------- | -------- | ------------------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | booleano | Compartir sesiones automáticamente                                              |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | cadena   | Ruta al ejecutable de Git Bash en Windows                                       |
| `LOGINOM_AI_AGENT_CONFIG`                     | cadena   | Ruta al archivo de configuración                                                |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | cadena   | Ruta al directorio de configuración                                             |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | cadena   | Contenido de configuración json en línea                                        |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | booleano | Deshabilitar las comprobaciones automáticas de actualizaciones                  |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | booleano | Deshabilitar la poda de datos antiguos                                          |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | booleano | Deshabilitar las actualizaciones automáticas de títulos de terminal             |
| `LOGINOM_AI_AGENT_PERMISSION`                 | cadena   | Configuración de permisos json incorporados                                     |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | booleano | Deshabilitar complementos predeterminados                                       |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | booleano | Deshabilitar las descargas automáticas del servidor LSP                         |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | booleano | Habilitar modelos experimentales                                                |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | booleano | Deshabilitar la compactación automática de contexto                             |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | booleano | Deshabilitar la lectura desde `.claude` (mensaje + habilidades)                 |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | booleano | Desactivar lectura `~/.claude/CLAUDE.md`                                        |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | booleano | Deshabilitar la carga `.claude/skills`                                          |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | booleano | Deshabilitar la recuperación de modelos desde fuentes remotas                   |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | cadena   | Proveedor de VCS falso para fines de prueba                                     |
| `LOGINOM_AI_AGENT_CLIENT`                     | cadena   | Identificador de cliente (por defecto `cli`)                                    |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | booleano | Habilitar las herramientas de búsqueda web de Exa                               |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | cadena   | Habilite la autenticación básica para `serve`/`web`                             |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | cadena   | URL personalizada para buscar la configuración de modelos                       |
...
| Variable                                        | Type     | Description                                                |
| ----------------------------------------------- | -------- | ---------------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | booleano | Habilitar funciones experimentales de la bandera global    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | booleano | Habilitar descubrimiento de íconos                         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booleano | Deshabilitar copia al seleccionar en TUI                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | número   | Tiempo de espera predeterminado para comandos bash en ms   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | número   | Tokens de salida máximos para respuestas LLM               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | booleano | Habilite el observador de archivos para todo el directorio |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | booleano | Habilitar el formateador oxfmt                             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | booleano | Habilitar herramienta experimental LSP                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booleano | Deshabilitar el observador de archivos                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | booleano | Habilitar funciones experimentales de Exa                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | booleano | Habilitar Habilitar TY LSP para archivos python            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | booleano | Habilitar modo de plan                                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | booleano | Habilitar tareas de subagentes en segundo plano            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | booleano | Habilitar sistema de eventos experimental                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | booleano | Habilitar ruta nativa de solicitud LLM                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | booleano | Habilitar ejecución paralela de búsqueda web               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | booleano | Habilitar subagente Scout                                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | booleano | Habilitar soporte de espacios de trabajo                   |

...
2. **Configuración global** (`~/.config/opencode/loginom-ai-agent.json`) - preferencias del usuario
3. **Configuración personalizada** (`LOGINOM_AI_AGENT_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`loginom-ai-agent.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.loginom-ai-agent`** - agentes, comandos, complementos
6. **Configuración en línea** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.loginom-ai-agent` y `~/.config/opencode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global OpenCode en `~/.config/opencode/loginom-ai-agent.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `loginom-ai-agent.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `LOGINOM_AI_AGENT_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `LOGINOM_AI_AGENT_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.loginom-ai-agent`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.loginom-ai-agent`, por lo que **puede anular** su configuración.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.loginom-ai-agent/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.loginom-ai-agent/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.loginom-ai-agent/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.

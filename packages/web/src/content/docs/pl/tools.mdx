...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (lub `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCode lub gdy zmienna środowiskowa `LOGINOM_AI_AGENT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

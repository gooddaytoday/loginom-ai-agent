...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (lub `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Jeśli Twój `loginom-ai-agent.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `LOGINOM_AI_AGENT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `LOGINOM_AI_AGENT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
- `loginom-ai-agent.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)

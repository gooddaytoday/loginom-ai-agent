...
:::caution
Jeśli `LOGINOM_AI_AGENT_SERVER_PASSWORD` nie jest ustawione, serwer będzie niezabezpieczony. Jest to dobre rozwiązanie do użytku lokalnego, ale powinno być ustawione na dostęp do sieci.
:::
...
Aby chronić dostęp, ustaw hasło za pomocą zmiennej środowiskowej `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Domyślna nazwa użytkownika to `opencode`, ale można ją zmienić za pomocą `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Możesz także skonfigurować ustawienia serwera w pliku konfiguracyjnym `loginom-ai-agent.json`:

...
Możesz określić niestandardowe pliki instrukcji w swoim `loginom-ai-agent.json` lub globalnym `~/.config/opencode/loginom-ai-agent.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie loginom-ai-agent.json

Zalecanym podejściem jest użycie pola `instructions` w `loginom-ai-agent.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `loginom-ai-agent.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::

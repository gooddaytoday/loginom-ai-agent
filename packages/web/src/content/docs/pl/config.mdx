...
2. **Konfiguracja globalna** (`~/.config/opencode/loginom-ai-agent.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `LOGINOM_AI_AGENT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`loginom-ai-agent.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.loginom-ai-agent`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `LOGINOM_AI_AGENT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.loginom-ai-agent` i `~/.config/opencode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/opencode/loginom-ai-agent.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `loginom-ai-agent.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `LOGINOM_AI_AGENT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `LOGINOM_AI_AGENT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.loginom-ai-agent` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.loginom-ai-agent`, więc **może być łączony** z ich zawartością.
...
Użyj `LOGINOM_AI_AGENT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `loginom-ai-agent.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencode/agents/` lub `.loginom-ai-agent/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.loginom-ai-agent/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.loginom-ai-agent/plugins/` lub `~/.config/opencode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.

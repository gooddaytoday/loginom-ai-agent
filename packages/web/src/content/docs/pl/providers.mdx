...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `loginom-ai-agent.json`:
...
`loginom-ai-agent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
4. Utwórz lub zaktualizuj plik `loginom-ai-agent.json` w katalogu projektu:

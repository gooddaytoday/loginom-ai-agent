...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `LOGINOM_AI_AGENT_SERVER_USERNAME` lub `opencode`) |
...
Uruchamia to serwer HTTP, który zapewnia dostęp do API OpenCode bez interfejsu TUI. Ustaw `LOGINOM_AI_AGENT_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Uruchamia to serwer HTTP i udostępnia OpenCode przez interfejs przeglądarkowy. Ustaw `LOGINOM_AI_AGENT_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
| Zmienna                               | Typ     | Opis                                                       |
| ------------------------------------- | ------- | ---------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Automatycznie udostępniaj sesje                            |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Ścieżka do pliku wykonywalnego Git Bash w systemie Windows |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Ścieżka do pliku konfiguracyjnego                          |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | Ścieżka do pliku konfiguracyjnego TUI                      |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Ścieżka do katalogu konfiguracyjnego                       |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Treść konfiguracji JSON (inline)                           |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Wyłącz automatyczne sprawdzanie aktualizacji               |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Wyłącz czyszczenie starych wyników (pruning)               |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Wyłącz automatyczne ustawianie tytułu terminala            |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Konfiguracja uprawnień w JSON (inline)                     |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Wyłącz domyślne wtyczki                                    |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Wyłącz automatyczne pobieranie serwerów LSP                |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Włącz modele eksperymentalne                               |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Wyłącz automatyczne kompaktowanie kontekstu                |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Wyłącz integrację z `.claude` (prompt + skills)            |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Wyłącz czytanie `~/.claude/CLAUDE.md`                      |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Wyłącz ładowanie `.claude/skills`                          |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Wyłącz pobieranie modeli ze źródeł zewnętrznych            |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Fałszywy dostawca VCS do celów testowych                   |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Identyfikator klienta (domyślnie `cli`)                    |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Włącz narzędzie wyszukiwania internetowego Exa             |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Włącz uwierzytelnianie podstawowe dla `serve`/`web`        |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `opencode`)    |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | Niestandardowy adres URL do pobierania konfiguracji modeli |
...
| Zmienna                                         | Typ     | Opis                                                |
| ----------------------------------------------- | ------- | --------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Włącz funkcje eksperymentalne objęte flagą zbiorczą |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Włącz wykrywanie ikon                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Wyłącz kopiowanie przy zaznaczaniu w TUI            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Domyślny limit czasu dla narzędzia bash w ms        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksymalne tokeny wyjściowe dla LLM                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Włącz obserwatora plików dla całego katalogu        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Włącz formater oxfmt                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Włącz eksperymentalne narzędzie LSP                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Wyłącz obserwatora plików                           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Włącz funkcje eksperymentalne Exa                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Włącz TY LSP dla plików python                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Włącz tryb planowania                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Włącz zadania subagentów w tle                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Włącz eksperymentalny system zdarzeń                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Włącz natywną ścieżkę żądań LLM                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Włącz równoległe wykonywanie wyszukiwania web       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Włącz subagenta Scout                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Włącz obsługę obszarów roboczych                    |

...
2. **Genel yapılandırma** (`~/.config/opencode/loginom-ai-agent.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`LOGINOM_AI_AGENT_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `loginom-ai-agent.json`) - projeye özgü ayarlar
5. **`.loginom-ai-agent` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.loginom-ai-agent` ve `~/.config/opencode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global opencode yapılandırmanızı `~/.config/opencode/loginom-ai-agent.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `loginom-ai-agent.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`LOGINOM_AI_AGENT_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`LOGINOM_AI_AGENT_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.loginom-ai-agent` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.loginom-ai-agent` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
Agent'ları `~/.config/opencode/agents/` veya `.loginom-ai-agent/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/opencode/commands/` veya `.loginom-ai-agent/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.loginom-ai-agent/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.

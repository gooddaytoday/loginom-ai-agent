...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (veya `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc` yapistirin
...
`loginom-ai-agent.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `LOGINOM_AI_AGENT_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `LOGINOM_AI_AGENT_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
- `loginom-ai-agent.settings.dat` (desktop varsayilan sunucu URL'si)

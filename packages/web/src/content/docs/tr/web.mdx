...
:::caution
`LOGINOM_AI_AGENT_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::
...
Erişimi korumak için `LOGINOM_AI_AGENT_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `LOGINOM_AI_AGENT_SERVER_USERNAME` ile değiştirilebilir.
...
Sunucu ayarlarını `loginom-ai-agent.json` yapılandırma dosyanızda da yapılandırabilirsiniz:

...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `loginom-ai-agent.json` kullanın:
...
`loginom-ai-agent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`loginom-ai-agent.json` üzerinden özelleştirin:
...
4. Proje dizininizde `loginom-ai-agent.json` dosyanızı oluşturun veya güncelleyin:

...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `loginom-ai-agent.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `LOGINOM_AI_AGENT_TUI_CONFIG` kullanın.

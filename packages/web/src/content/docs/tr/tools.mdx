...
:::note
Bu araç yalnızca `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (veya `LOGINOM_AI_AGENT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `LOGINOM_AI_AGENT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

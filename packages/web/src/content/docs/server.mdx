...
Set `LOGINOM_AI_AGENT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `LOGINOM_AI_AGENT_SERVER_USERNAME` to override it. This applies to both `opencode serve` and `opencode web`.

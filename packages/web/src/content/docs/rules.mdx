...
You can specify custom instruction files in your `loginom-ai-agent.json` or the global `~/.config/opencode/loginom-ai-agent.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
### Using loginom-ai-agent.json

The recommended approach is to use the `instructions` field in `loginom-ai-agent.json`:
...
:::tip
For monorepos or projects with shared standards, using `loginom-ai-agent.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::

...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (or `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
If your `loginom-ai-agent.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `LOGINOM_AI_AGENT_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `LOGINOM_AI_AGENT_PORT` (or pick a free port) and restart.

...
```json title="loginom-ai-agent.json" {4}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
This tool is only available when `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (or `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode or OpenCode Go provider, or when either the `LOGINOM_AI_AGENT_ENABLE_EXA` or `LOGINOM_AI_AGENT_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching OpenCode:

```bash
LOGINOM_AI_AGENT_ENABLE_EXA=1 opencode
# or
LOGINOM_AI_AGENT_ENABLE_PARALLEL=1 opencode
```

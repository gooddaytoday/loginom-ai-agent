...
Du kannst zusaetzliche Anweisungsdateien in deiner `loginom-ai-agent.json` oder in `~/.config/opencode/loginom-ai-agent.json` angeben.
...
### Mit loginom-ai-agent.json

Empfohlen ist das `instructions`-Feld in der `loginom-ai-agent.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `loginom-ai-agent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

...
:::note
Dieses Tool ist nur verfuegbar, wenn `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (oder `LOGINOM_AI_AGENT_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `LOGINOM_AI_AGENT_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

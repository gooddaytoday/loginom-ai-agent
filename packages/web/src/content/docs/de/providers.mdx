...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `loginom-ai-agent.json`:
...
`loginom-ai-agent.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Anpassen über `loginom-ai-agent.json`:
...
4. Erstellen oder aktualisieren Sie Ihre `loginom-ai-agent.json`-Datei in Ihrem Projektverzeichnis:

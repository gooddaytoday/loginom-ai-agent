...
2. **Globale Konfiguration** (`~/.config/opencode/loginom-ai-agent.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`LOGINOM_AI_AGENT_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`loginom-ai-agent.json` im Projekt) – projektspezifische Einstellungen
5. **`.loginom-ai-agent` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.loginom-ai-agent` und `~/.config/opencode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/opencode/loginom-ai-agent.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `loginom-ai-agent.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `LOGINOM_AI_AGENT_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `LOGINOM_AI_AGENT_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.loginom-ai-agent` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.loginom-ai-agent` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.loginom-ai-agent/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.loginom-ai-agent/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.loginom-ai-agent/plugins/` oder `~/.config/opencode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.

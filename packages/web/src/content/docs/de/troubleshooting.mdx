...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (oder `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Wenn deine `loginom-ai-agent.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `LOGINOM_AI_AGENT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `LOGINOM_AI_AGENT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
- `loginom-ai-agent.settings.dat` (desktop default server URL)

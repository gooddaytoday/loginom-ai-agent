...
:::caution
Wenn `LOGINOM_AI_AGENT_SERVER_PASSWORD` nicht gesetzt ist, ist der Server ungesichert. Dies ist für den lokalen Gebrauch in Ordnung, sollte aber für den Netzwerkzugriff eingestellt werden.
:::
...
Um den Zugriff zu schützen, legen Sie mithilfe der Umgebungsvariablen `LOGINOM_AI_AGENT_SERVER_PASSWORD` ein Passwort fest:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Der Benutzername ist standardmäßig `opencode`, kann aber mit `LOGINOM_AI_AGENT_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `loginom-ai-agent.json`-Konfigurationsdatei konfigurieren:

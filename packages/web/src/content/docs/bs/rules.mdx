...
Mozete navesti prilagodene datoteke uputstava u `loginom-ai-agent.json` ili globalnom `~/.config/opencode/loginom-ai-agent.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje loginom-ai-agent.json

Preporuceni pristup je da koristite `instructions` polje u `loginom-ai-agent.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `loginom-ai-agent.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::

...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `loginom-ai-agent.json`:
...
`loginom-ai-agent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `loginom-ai-agent.json`:
...
4. Kreirajte ili ažurirajte svoju `loginom-ai-agent.json` datoteku u direktoriju projekta:

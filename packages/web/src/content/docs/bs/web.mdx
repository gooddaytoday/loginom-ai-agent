...
:::caution
Ako `LOGINOM_AI_AGENT_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `LOGINOM_AI_AGENT_SERVER_PASSWORD`:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Korisničko ime podrazumevano je `opencode`, ali se može promeniti sa `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `loginom-ai-agent.json` konfiguracijskom fajlu:

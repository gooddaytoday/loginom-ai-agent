...
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `LOGINOM_AI_AGENT_SERVER_USERNAME` ili `opencode`) |
...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `LOGINOM_AI_AGENT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `LOGINOM_AI_AGENT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| Varijabla                             | Tip     | Opis                                                               |
| ------------------------------------- | ------- | ------------------------------------------------------------------ |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | Putanja do TUI konfiguracijskog fajla                              |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                       | Tip     | Opis                                                    |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Omogući eksperimentalne funkcije pod zbirnom zastavicom |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući TY LSP za python datoteke                       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Omogući pozadinske zadatke subagenata                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Omogući eksperimentalni sistem događaja                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Omogući nativnu putanju LLM zahtjeva                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Omogući paralelno izvršavanje web pretrage              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Omogući Scout subagenta                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Omogući podršku za radne prostore                       |

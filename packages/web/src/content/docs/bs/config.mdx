...
2. **Globalna konfiguracija** (`~/.config/opencode/loginom-ai-agent.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LOGINOM_AI_AGENT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`loginom-ai-agent.json` u projektu) - postavke specifične za projekat
5. **`.loginom-ai-agent` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.loginom-ai-agent` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/opencode/loginom-ai-agent.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `loginom-ai-agent.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LOGINOM_AI_AGENT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LOGINOM_AI_AGENT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.loginom-ai-agent` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.loginom-ai-agent`, tako da **može nadjačati** njihove postavke.
...
Koristite `LOGINOM_AI_AGENT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `loginom-ai-agent.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.loginom-ai-agent/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.loginom-ai-agent/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.loginom-ai-agent/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.

...
- **macOS/Linux**: `~/.config/opencode/loginom-ai-agent.jsonc` (ili `~/.config/opencode/loginom-ai-agent.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Ako vaš `loginom-ai-agent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LOGINOM_AI_AGENT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LOGINOM_AI_AGENT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
- `loginom-ai-agent.settings.dat` (URL zadanog servera za desktop)

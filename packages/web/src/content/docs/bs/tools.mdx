...
:::note
Ovaj alat je dostupan samo kada je `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ili `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `LOGINOM_AI_AGENT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

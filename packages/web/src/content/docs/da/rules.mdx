...
Du kan spesifisere egendefinerte instruksjonsfiler i din `loginom-ai-agent.json` eller den globale `~/.config/opencode/loginom-ai-agent.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af loginom-ai-agent.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `loginom-ai-agent.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `loginom-ai-agent.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::

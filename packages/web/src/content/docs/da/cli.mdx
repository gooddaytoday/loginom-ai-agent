...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                      |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `LOGINOM_AI_AGENT_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `LOGINOM_AI_AGENT_SERVER_USERNAME` eller `opencode`) |
...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `LOGINOM_AI_AGENT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `LOGINOM_AI_AGENT_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                          |
| ----------------------------------------------- | ------- | ---------------------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Aktiver eksperimentelle funktioner under samleflaget |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                                |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI                        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Aktiver baggrundsopgaver for subagenter              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Aktiver eksperimentelt hændelsessystem               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Aktiver native LLM-anmodningssti                     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Aktiver parallel udførelse af websøgning             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Aktiver Scout-subagent                               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Aktiver workspace-understøttelse                     |

...
2. **Global config** (`~/.config/opencode/loginom-ai-agent.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`LOGINOM_AI_AGENT_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`loginom-ai-agent.json` i projekt) - projektspecifikke indstillinger
5. **`.loginom-ai-agent` mapper** - agent, kommandoer, plugins
6. **Inline config** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.loginom-ai-agent` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/opencode/loginom-ai-agent.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `loginom-ai-agent.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `LOGINOM_AI_AGENT_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `LOGINOM_AI_AGENT_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.loginom-ai-agent` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.loginom-ai-agent` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.loginom-ai-agent/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.loginom-ai-agent/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.loginom-ai-agent/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.

...
:::caution
Hvis `LOGINOM_AI_AGENT_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `LOGINOM_AI_AGENT_SERVER_PASSWORD` miljøvariablen:

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

Brugernavnet er som standard `opencode`, men kan ændres med `LOGINOM_AI_AGENT_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `loginom-ai-agent.json`-konfigurationsfil:

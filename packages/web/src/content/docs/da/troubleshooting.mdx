...
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
Hvis `loginom-ai-agent.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LOGINOM_AI_AGENT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LOGINOM_AI_AGENT_PORT` (eller vælg en ledig port) og start på ny.
...
- `loginom-ai-agent.settings.dat` (skrivebordsstandardserver URL)

...
:::note
Dette verktøyet er bare tilgængelig når `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (eller `LOGINOM_AI_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `LOGINOM_AI_AGENT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

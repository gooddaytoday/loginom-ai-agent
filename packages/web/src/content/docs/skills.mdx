...
It loads any matching `skills/*/SKILL.md` in `.loginom-ai-agent/` and any matching `.claude/skills/*/SKILL.md` or `.agents/skills/*/SKILL.md` along the way.
...
Create `.loginom-ai-agent/skills/git-release/SKILL.md` like this:
...
Control which skills agents can access using pattern-based permissions in `loginom-ai-agent.json`:
...
**For built-in agents** (in `loginom-ai-agent.json`):

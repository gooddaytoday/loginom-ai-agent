...
2. **Global config** (`~/.config/opencode/loginom-ai-agent.json`) - user preferences
3. **Custom config** (`LOGINOM_AI_AGENT_CONFIG` env var) - custom overrides
4. **Project config** (`loginom-ai-agent.json` in project) - project-specific settings
5. **`.loginom-ai-agent` directories** - agents, commands, plugins
6. **Inline config** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.loginom-ai-agent` and `~/.config/opencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCode config in `~/.config/opencode/loginom-ai-agent.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `loginom-ai-agent.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `LOGINOM_AI_AGENT_CONFIG` environment variable.
...
Specify a custom config directory using the `LOGINOM_AI_AGENT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.loginom-ai-agent` directory, and should
...
The custom directory is loaded after the global config and `.loginom-ai-agent` directories, so it **can override** their settings.
...
Drop an `loginom-ai-agent.json` or `loginom-ai-agent.jsonc` file in the system managed config directory:
...
The plist keys map directly to `loginom-ai-agent.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.
...
Use `LOGINOM_AI_AGENT_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `loginom-ai-agent.json` are deprecated and automatically migrated when possible.
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.loginom-ai-agent/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.loginom-ai-agent/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.loginom-ai-agent/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.

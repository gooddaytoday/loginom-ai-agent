...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.loginom-ai-agent/agent` based on the prompt)                                                                                                                         |
...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `LOGINOM_AI_AGENT_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `LOGINOM_AI_AGENT_SERVER_USERNAME` or `opencode`) |
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `LOGINOM_AI_AGENT_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `LOGINOM_AI_AGENT_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | Path to config file                               |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | Path to config directory                          |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | Inlined json permissions config                   |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `LOGINOM_AI_AGENT_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `LOGINOM_AI_AGENT_ENABLE_PARALLEL`            | boolean | Enable Parallel web search tools                  |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |

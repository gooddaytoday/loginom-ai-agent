...
- 项目配置：`.loginom-ai-agent/skills/<name>/SKILL.md`
...
在此过程中，它会加载 `.loginom-ai-agent/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。
...
创建 `.loginom-ai-agent/skills/git-release/SKILL.md`，内容如下：
...
在 `loginom-ai-agent.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
...
**内置代理**（在 `loginom-ai-agent.json` 中）：

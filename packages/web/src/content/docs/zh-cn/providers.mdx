...
2. 使用以下方法之一**配置身份验证**：
   ...
   ***
   ...
   如需项目级别或持久化的配置，请使用 `loginom-ai-agent.json`：
...
:::note[合规说明]
OpenCode 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 OpenCode 仅使用你自己的 GitLab 托管实例，请在 `loginom-ai-agent.json` 文件中添加以下内容。同时建议禁用会话共享。
...
通过 `loginom-ai-agent.json` 进行自定义配置：
...
4. 在项目目录中创建或更新 `loginom-ai-agent.json` 文件：

...
- **macOS/Linux**（旧版安装）: `~/.local/share/opencode/loginom-ai-agent.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\loginom-ai-agent.jsonc`
...
如果你的 `loginom-ai-agent.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。

#### 检查环境变量

如果你在环境中设置了 `LOGINOM_AI_AGENT_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `LOGINOM_AI_AGENT_PORT`（或选择一个空闲端口）并重新启动。
...
- `loginom-ai-agent.settings.dat`（桌面默认服务器 URL）

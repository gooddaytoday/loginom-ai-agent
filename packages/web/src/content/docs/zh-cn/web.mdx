...
:::caution
如果未设置 `LOGINOM_AI_AGENT_SERVER_PASSWORD`，服务器将没有安全保护。本地使用没有问题，但在网络访问时应当设置密码。
:::
...
要保护服务器访问，可以通过 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 环境变量设置密码：

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

用户名默认为 `opencode`，可以通过 `LOGINOM_AI_AGENT_SERVER_USERNAME` 进行更改。
...
你也可以在 `loginom-ai-agent.json` 配置文件中设置服务器选项：

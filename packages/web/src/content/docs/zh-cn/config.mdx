...
2. **全局配置**（`~/.config/opencode/loginom-ai-agent.json`）- 用户偏好
3. **自定义配置**（`LOGINOM_AI_AGENT_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `loginom-ai-agent.json`）- 项目特定设置
5. **`.loginom-ai-agent` 目录** - 代理、命令、插件
6. **内联配置**（`LOGINOM_AI_AGENT_CONFIG_CONTENT` 环境变量）- 运行时覆盖

这意味着项目配置可以覆盖全局默认值，全局配置可以覆盖远程组织默认值。

:::note
`.loginom-ai-agent` 和 `~/.config/opencode` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
将全局 OpenCode 配置放在 `~/.config/opencode/loginom-ai-agent.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
在项目根目录中添加 `loginom-ai-agent.json`。项目配置在标准配置文件中具有最高优先级——它会覆盖全局配置和远程配置。
...
使用 `LOGINOM_AI_AGENT_CONFIG` 环境变量指定自定义配置文件路径。
...
使用 `LOGINOM_AI_AGENT_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.loginom-ai-agent` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
自定义目录在全局配置和 `.loginom-ai-agent` 目录之后加载，因此**可以覆盖**它们的设置。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.loginom-ai-agent/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
您还可以使用 `~/.config/opencode/commands/` 或 `.loginom-ai-agent/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.loginom-ai-agent/plugins/` 或 `~/.config/opencode/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。

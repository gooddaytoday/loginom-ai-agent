...
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本认证密码（默认使用 `LOGINOM_AI_AGENT_SERVER_PASSWORD`）                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `LOGINOM_AI_AGENT_SERVER_USERNAME` 或 `opencode`） |
...
此命令启动一个 HTTP 服务器，提供对 OpenCode 功能的 API 访问，无需 TUI 界面。设置 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenCode。设置 `LOGINOM_AI_AGENT_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
| 变量                                  | 类型    | 描述                                    |
| ------------------------------------- | ------- | --------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | boolean | 自动分享会话                            |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | string  | Windows 上 Git Bash 可执行文件的路径    |
| `LOGINOM_AI_AGENT_CONFIG`                     | string  | 配置文件路径                            |
| `LOGINOM_AI_AGENT_TUI_CONFIG`                 | string  | TUI 配置文件路径                        |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | string  | 配置目录路径                            |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | string  | 内联 JSON 配置内容                      |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | boolean | 禁用自动更新检查                        |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | boolean | 禁用旧数据清理                          |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | boolean | 禁用自动终端标题更新                    |
| `LOGINOM_AI_AGENT_PERMISSION`                 | string  | 内联 JSON 权限配置                      |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | boolean | 禁用默认插件                            |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | boolean | 禁用 LSP 服务器自动下载                 |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | boolean | 启用实验性模型                          |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | boolean | 禁用自动上下文压缩                      |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | boolean | 禁用读取 `.claude`（提示词 + 技能）     |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | 禁用读取 `~/.claude/CLAUDE.md`          |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | 禁用加载 `.claude/skills`               |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | boolean | 禁用从远程源获取模型                    |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | string  | 用于测试目的的模拟 VCS 提供商           |
| `LOGINOM_AI_AGENT_CLIENT`                     | string  | 客户端标识符（默认为 `cli`）            |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | boolean | 启用 Exa 网络搜索工具                   |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | string  | 为 `serve`/`web` 启用基本认证           |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `opencode`） |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | string  | 自定义模型配置获取 URL                  |
...
| 变量                                            | 类型    | 描述                            |
| ----------------------------------------------- | ------- | ------------------------------- |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | boolean | 启用受总开关控制的实验性功能    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | 启用图标发现                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | 禁用 TUI 中的选中即复制         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | bash 命令的默认超时时间（毫秒） |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | LLM 响应的最大输出 Token 数     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | boolean | 启用整个目录的文件监听器        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | boolean | 启用 oxfmt 格式化器             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | boolean | 启用实验性 LSP 工具             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | 禁用文件监听器                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | boolean | 启用实验性 Exa 功能             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | boolean | 为 python 文件启用 TY LSP       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | boolean | 启用计划模式                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | 启用后台子代理任务              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | 启用实验性事件系统              |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | boolean | 启用原生 LLM 请求路径           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | boolean | 启用并行 Web 搜索执行           |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | boolean | 启用 Scout 子代理               |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | boolean | 启用工作区支持                  |

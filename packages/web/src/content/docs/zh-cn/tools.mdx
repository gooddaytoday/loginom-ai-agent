...
:::note
该工具仅在设置 `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true`（或 `LOGINOM_AI_AGENT_EXPERIMENTAL=true`）时可用。
:::
...
:::note
该工具仅在使用 OpenCode 提供商时，或当 `LOGINOM_AI_AGENT_ENABLE_EXA` 环境变量设置为任意真值（例如 `true` 或 `1`）时可用。

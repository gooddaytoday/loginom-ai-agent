...
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `LOGINOM_AI_AGENT_SERVER_PASSWORD`）                   |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `LOGINOM_AI_AGENT_SERVER_USERNAME` または `opencode`） |
...
これにより、TUI インターフェイスを使用せずに opencode 機能への API アクセスを提供する HTTP サーバーが起動します。 `LOGINOM_AI_AGENT_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて OpenCode にアクセスします。 `LOGINOM_AI_AGENT_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
| 変数                                  | タイプ   | 説明                                                                |
| ------------------------------------- | -------- | ------------------------------------------------------------------- |
| `LOGINOM_AI_AGENT_AUTO_SHARE`                 | ブール値 | セッションを自動的に共有する                                        |
| `LOGINOM_AI_AGENT_GIT_BASH_PATH`              | 文字列   | Windows 上で実行可能な Git Bash へのパス                            |
| `LOGINOM_AI_AGENT_CONFIG`                     | 文字列   | 構成ファイルへのパス                                                |
| `LOGINOM_AI_AGENT_CONFIG_DIR`                 | 文字列   | config ディレクトリへのパス                                         |
| `LOGINOM_AI_AGENT_CONFIG_CONTENT`             | 文字列   | インライン JSON 構成コンテンツ                                      |
| `LOGINOM_AI_AGENT_DISABLE_AUTOUPDATE`         | ブール値 | 自動更新チェックを無効にする                                        |
| `LOGINOM_AI_AGENT_DISABLE_PRUNE`              | ブール値 | 古いデータのプルーニングを無効にする                                |
| `LOGINOM_AI_AGENT_DISABLE_TERMINAL_TITLE`     | ブール値 | ターミナルタイトルの自動更新を無効にする                            |
| `LOGINOM_AI_AGENT_PERMISSION`                 | 文字列   | インライン化された json 権限設定                                    |
| `LOGINOM_AI_AGENT_DISABLE_DEFAULT_PLUGINS`    | ブール値 | デフォルトのプラグインを無効にする                                  |
| `LOGINOM_AI_AGENT_DISABLE_LSP_DOWNLOAD`       | ブール値 | LSP サーバーの自動ダウンロードを無効にする                          |
| `LOGINOM_AI_AGENT_ENABLE_EXPERIMENTAL_MODELS` | ブール値 | 実験モデルを有効にする                                              |
| `LOGINOM_AI_AGENT_DISABLE_AUTOCOMPACT`        | ブール値 | 自動コンテキスト圧縮を無効にする                                    |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE`        | ブール値 | `.claude` からの読み取りを無効にする (プロンプト + スキル)          |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_PROMPT` | ブール値 | `~/.claude/CLAUDE.md` の読み取りを無効にする                        |
| `LOGINOM_AI_AGENT_DISABLE_CLAUDE_CODE_SKILLS` | ブール値 | `.claude/skills` のロードを無効にする                               |
| `LOGINOM_AI_AGENT_DISABLE_MODELS_FETCH`       | ブール値 | リモートソースからのモデルの取得を無効にする                        |
| `LOGINOM_AI_AGENT_FAKE_VCS`                   | 文字列   | テスト目的の偽の VCS プロバイダー                                   |
| `LOGINOM_AI_AGENT_CLIENT`                     | 文字列   | クライアント識別子 (デフォルトは `cli`)                             |
| `LOGINOM_AI_AGENT_ENABLE_EXA`                 | ブール値 | Exa Web 検索ツールを有効にする                                      |
| `LOGINOM_AI_AGENT_SERVER_PASSWORD`            | 文字列   | `serve`/`web` の基本認証を有効にする                                |
| `LOGINOM_AI_AGENT_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `opencode`) をオーバーライドします。 |
| `LOGINOM_AI_AGENT_MODELS_URL`                 | 文字列   | モデル設定を取得するためのカスタム URL                              |
...
| 変数                                            | タイプ   | 説明                                             |
| ----------------------------------------------- | -------- | ------------------------------------------------ |
| `LOGINOM_AI_AGENT_EXPERIMENTAL`                         | ブール値 | アンブレラフラグ配下の実験的機能を有効にする     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_ICON_DISCOVERY`          | ブール値 | アイコン検出を有効にする                         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | ブール値 | TUI で選択時のコピーを無効にする                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | 数値     | bash コマンドのデフォルトのタイムアウト (ミリ秒) |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | 数値     | LLM 応答の最大出力トークン                       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_FILEWATCHER`             | ブール値 | ディレクトリ全体のファイル監視を有効にする       |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_OXFMT`                   | ブール値 | oxfmt フォーマッタを有効にする                   |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL`                | ブール値 | 実験的な LSP ツールを有効にする                  |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | ブール値 | ファイルウォッチャーを無効にする                 |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EXA`                     | ブール値 | 実験的な Exa 機能を有効にする                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TY`                  | ブール値 | python ファイルの TY LSP を有効にする            |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PLAN_MODE`               | ブール値 | プランモードを有効にする                         |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | ブール値 | バックグラウンド subagent タスクを有効にする     |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_EVENT_SYSTEM`            | ブール値 | 実験的なイベントシステムを有効にする             |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_NATIVE_LLM`              | ブール値 | ネイティブ LLM リクエスト経路を有効にする        |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_PARALLEL`                | ブール値 | 並列 Web 検索実行を有効にする                    |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_SCOUT`                   | ブール値 | Scout subagent を有効にする                      |
| `LOGINOM_AI_AGENT_EXPERIMENTAL_WORKSPACES`              | ブール値 | ワークスペースサポートを有効にする               |

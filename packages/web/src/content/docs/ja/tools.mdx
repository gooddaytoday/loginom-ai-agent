...
:::note
このツールは、`LOGINOM_AI_AGENT_EXPERIMENTAL_LSP_TOOL=true` (または `LOGINOM_AI_AGENT_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、OpenCode プロバイダーを使用している場合、または `LOGINOM_AI_AGENT_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

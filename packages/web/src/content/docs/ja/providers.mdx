...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`loginom-ai-agent.json` を使用します。
...
`loginom-ai-agent.json` ファイル。セッション共有を無効にすることもお勧めします。
...
`loginom-ai-agent.json` を通じてカスタマイズします。
...
4. プロジェクトディレクトリで `loginom-ai-agent.json` ファイルを作成または更新します。

...
HTTP 基本認証でサーバーを保護するには、`LOGINOM_AI_AGENT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencode` になるか、`LOGINOM_AI_AGENT_SERVER_USERNAME` を設定してオーバーライドします。これは、`opencode serve` と `opencode web` の両方に当てはまります。

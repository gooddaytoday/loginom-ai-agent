...
- プロジェクト設定: `.loginom-ai-agent/skills/<name>/SKILL.md`
...
途中で、一致する `skills/*/SKILL.md` を `.loginom-ai-agent/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。
...
次のように `.loginom-ai-agent/skills/git-release/SKILL.md` を作成します。
...
`loginom-ai-agent.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`loginom-ai-agent.json` 内):

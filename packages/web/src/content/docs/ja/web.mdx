...
:::caution
`LOGINOM_AI_AGENT_SERVER_PASSWORD` が設定されていない場合、サーバーは保護されません。これはローカルで使用する場合には問題ありませんが、ネットワークアクセス用に設定する必要があります。
:::
:::tip[Windows ユーザー]
...
アクセスを保護するには、`LOGINOM_AI_AGENT_SERVER_PASSWORD` 環境変数を使用してパスワードを設定します。

```bash
LOGINOM_AI_AGENT_SERVER_PASSWORD=secret opencode web
```

ユーザー名のデフォルトは `opencode` ですが、`LOGINOM_AI_AGENT_SERVER_USERNAME` で変更できます。
...
`loginom-ai-agent.json` 設定ファイルでサーバー設定を構成することもできます。

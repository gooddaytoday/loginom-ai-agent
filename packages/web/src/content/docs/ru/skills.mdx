...
- Конфигурация проекта: `.loginom-ai-agent/skills/<name>/SKILL.md`
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.loginom-ai-agent/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.
...
Создайте `.loginom-ai-agent/skills/git-release/SKILL.md` следующим образом:
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `loginom-ai-agent.json`:
...
**Для встроенных агентов** (в формате `loginom-ai-agent.json`):

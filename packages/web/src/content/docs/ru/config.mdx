...
2. **Глобальная конфигурация** (`~/.config/opencode/loginom-ai-agent.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`LOGINOM_AI_AGENT_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`loginom-ai-agent.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.loginom-ai-agent`** — агенты, команды, плагины
6. **Встроенная конфигурация** (`LOGINOM_AI_AGENT_CONFIG_CONTENT` env var) – переопределяет время выполнения
...
:::note
В каталогах `.loginom-ai-agent` и `~/.config/opencode` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Поместите глобальную конфигурацию opencode в `~/.config/opencode/loginom-ai-agent.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.
...
Добавьте `loginom-ai-agent.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Укажите собственный путь к файлу конфигурации, используя переменную среды `LOGINOM_AI_AGENT_CONFIG`.
...
Укажите пользовательский каталог конфигурации, используя переменную среды `LOGINOM_AI_AGENT_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.loginom-ai-agent`), и он должен иметь ту же структуру.
...
Пользовательский каталог загружается после каталогов global config и `.loginom-ai-agent`, поэтому он **может переопределить** их настройки.
...
Используйте `LOGINOM_AI_AGENT_TUI_CONFIG`, чтобы указать на пользовательский файл конфигурации TUI.

Устаревшие ключи `theme`, `keybinds` и `tui` в `loginom-ai-agent.json` устарели и автоматически переносятся, когда это возможно.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/opencode/agents/` или `.loginom-ai-agent/agents/`. [Подробнее здесь](/docs/agents).
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/opencode/commands/` или `.loginom-ai-agent/commands/`. [Подробнее здесь](/docs/commands).
...
Поместите файлы плагина в `.loginom-ai-agent/plugins/` или `~/.config/opencode/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.

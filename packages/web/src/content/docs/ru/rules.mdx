...
Вы можете указать собственные файлы инструкций в `loginom-ai-agent.json` или в глобальном `~/.config/opencode/loginom-ai-agent.json`. Это позволит вам и вашей команде повторно использовать существующие правила вместо того, чтобы дублировать их на AGENTS.md.
...
### Использование loginom-ai-agent.json

Рекомендуемый подход — использовать поле `instructions` в `loginom-ai-agent.json`:
...
:::tip
Для монорепозиториев или проектов с общими стандартами использование `loginom-ai-agent.json` с шаблонами glob (например, `packages/*/AGENTS.md`) более удобно в обслуживании, чем инструкции вручную.
:::

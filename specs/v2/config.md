...
V2 core discovers config documents named `loginom-ai-agent.json` or `loginom-ai-agent.jsonc` in the global config directory, ancestor project directories, and `.loginom-ai-agent` config directories. The legacy `config.json` filename is not supported in V2.
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.loginom-ai-agent/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.loginom-ai-agent` policy sources remains open until `.loginom-ai-agent` configuration is reviewed.

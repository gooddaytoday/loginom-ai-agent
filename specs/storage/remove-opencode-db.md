# Remove `packages/agent/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/agent/src/storage/db.ts` module.
...
Production imports from `packages/agent/src/storage/db.ts` are concentrated in 22 source files:

- `packages/agent/src/account/repo.ts`
- `packages/agent/src/cli/cmd/db.ts`
- `packages/agent/src/cli/cmd/import.ts`
- `packages/agent/src/cli/cmd/stats.ts`
- `packages/agent/src/control-plane/workspace.ts`
- `packages/agent/src/index.ts`
- `packages/agent/src/node.ts`
- `packages/agent/src/permission/index.ts`
- `packages/agent/src/project/project.ts`
- `packages/agent/src/server/projectors.ts`
- `packages/agent/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/agent/src/server/shared/fence.ts`
- `packages/agent/src/session/message-v2.ts`
- `packages/agent/src/session/projectors.ts`
- `packages/agent/src/session/prompt.ts`
- `packages/agent/src/session/session.ts`
- `packages/agent/src/session/todo.ts`
- `packages/agent/src/share/share-next.ts`
- `packages/agent/src/storage/db.ts`
- `packages/agent/src/sync/index.ts`
- `packages/agent/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy opencode database wrapper; `packages/agent/src/storage/db.ts` has been deleted.
...
All migration groups are complete or superseded. `packages/agent/src/storage/db.ts` has been deleted.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/agent`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/agent/src`
- `bun typecheck` from `packages/agent`
- Relevant package tests from `packages/agent`, not the repo root

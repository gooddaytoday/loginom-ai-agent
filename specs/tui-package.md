...
```text
packages/agent ---\
                      > @loginom-ai-agent/tui -> @loginom-ai-agent/sdk
packages/cli --------/
```
...
`packages/cli`, or `@loginom-ai-agent/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/agent`.
- Do not replace private `packages/agent` imports with `@loginom-ai-agent/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@loginom-ai-agent/tui`.

## Ownership Boundary

### `@loginom-ai-agent/tui` Owns
...
Status: Completed. The private `@loginom-ai-agent/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@loginom-ai-agent/tui`.
...
- The package has no dependency on `opencode`, `@loginom-ai-agent/cli`, or
  `@loginom-ai-agent/core`.
...
primitives now live in `@loginom-ai-agent/tui`. The legacy host consumes explicit
...
and build inputs through `@loginom-ai-agent/tui/runtime`. Movable app, component,
...
`@loginom-ai-agent/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@loginom-ai-agent/tui`. Configured references resolve through the new
...
`@loginom-ai-agent/tui`. The legacy host injects a narrow plugin host that retains
...
public `@loginom-ai-agent/tui` root while retaining worker/server/config/plugin and
...
- Change those adapters to load config, create transport inputs, and call the
  public `@loginom-ai-agent/tui` API.
...
- Narrow `@loginom-ai-agent/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@loginom-ai-agent/core`, or either
  executable package.
...
rg '@loginom-ai-agent/core|packages/agent|packages/cli' packages/tui

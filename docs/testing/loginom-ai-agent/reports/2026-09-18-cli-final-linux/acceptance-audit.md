...
| IND-05 input/profile isolation | Final run same-name inputs have different bytes/source/package paths; host inputStore contract test | Two chats in one native TUI: original hashes, distinct runtime/artifact/path identities and separate SQLite tool records PASS; sequential, not parallel |
| IND-06 Desktop independence | CLI profile imports/process tests; final providers-list strace: no Desktop root references; install/uninstall preserved Desktop credentials | Both close orders PASS; traced setup/run/prepare/observe/close has zero Desktop references; TUI attachment/new-chat/prepare/delivery/exit trace: zero Desktop references |
...
A sequential same-process two-chat TUI exercise and its complete filesystem
trace are now also verified. This does not claim every possible command or timing. Full provider-generated

...
| Job       | Команды                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `unit`    | `setup-bun`, `setup-loginom-inputs`, кэш Turbo; `GITHUB_ACTIONS=false bun turbo test --continue` — `test` каждого workspace-пакета, включая `@loginom-ai-agent/loginom-host` с pinned Node через `LOGINOM_AI_AGENT_TEST_NODE` (`passThroughEnv` в [turbo.json](../../../turbo.json)); `--continue` показывает все упавшие пакеты за один прогон (без него turbo 2.10 падал с SIGSEGV, останавливая остальные задачи); затем `bun run check:generated` в `packages/client` и `bun run test:httpapi` в `packages/agent` |

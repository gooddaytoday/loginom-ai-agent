...
- `build (linux|windows|macos)` (`timeout-minutes: 120`, `shell: bash`):
  ...
  3. В `packages/desktop`: `bun run build`, `bun typecheck`, затем `bun run package:linux --x64 --publish never` | `package:win --x64 --publish never` | `package:mac --arm64 --publish never` с `CSC_IDENTITY_AUTO_DISCOVERY=false`.
...
- `linux-matrix` (`ubuntu-24.04`, после `build`): скачивает `loginom-linux` и выполняет [run-matrix.ts](../../../packages/desktop/test/loginom/docker/run-matrix.ts) на DEB: Ubuntu 22.04/24.04/26.04, Debian 12/13, установка в чистую ОС, offline-запуск от UID 1200 с sandbox Chromium. Отчёт `linux-matrix.json` и логи — artifact `loginom-linux-matrix` (выгружается и при падении).
...
| Job       | Команды                                                                                                                                                                                                                                                                                                                                                                    |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `unit`    | `setup-bun`, `setup-loginom-inputs`, кэш Turbo; `GITHUB_ACTIONS=false bun turbo test` — `test` каждого workspace-пакета, включая `@loginom-ai-agent/loginom-host` с pinned Node через `LOGINOM_AI_AGENT_TEST_NODE` (`passThroughEnv` в [turbo.json](../../../turbo.json)); затем `bun run check:generated` в `packages/client` и `bun run test:httpapi` в `packages/agent` |
...
LOGINOM_AI_AGENT_TEST_NODE=/absolute/cache/node-v24.19.0-linux-x64/bin/node bun turbo test
```

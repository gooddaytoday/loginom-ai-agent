...
Текущая интеграция: ветка `loginom`, подготовка версии 0.1.8. Default branch репозитория остаётся `dev`; её настройка не меняется. До завершения конкретного run этот документ описывает контракт CI, а не результат проверки. С 0.1.8 удалён импорт системного прокси, как описано в [решении](../../superpowers/specs/2026-09-22-proxy-policy.md); native gates проверяют сохранение явного proxy environment, а unit gate дополнительно выполняет реальный Node HTTP/fetch/HTTPS CONNECT тест.

...
## Ограничение ожидания отрисовки портов — source regression

В `observeGraph` обнаружено ожидание двух requestAnimationFrame без внутреннего
завершения при остановленной отрисовке фонового документа. Оно могло занимать
весь MCP timeout и мешать inspect после отказа графа. Введено ограниченное
ожидание одного цикла (100 мс); затем повторяется только чтение с исходным
общим deadline. Проверка графа и запрет повторного жеста сохранены.
Новый тест с приостановленными кадрами падает на прежнем коде и проходит после
изменения: graph tests 48 PASS, полный runtime 2276 PASS / 2 SKIP.
Это ещё не доказательство причины пропавшего порта B27; установленная `.8`
содержит расширенную диагностику, но данное последующее исправление пока
проверено только в исходниках.

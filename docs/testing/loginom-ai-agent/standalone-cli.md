...
> CLI применяет системный прокси после запуска хоста Loginom. Сбой пишется в stderr
> и показывается тостом TUI. Исторические проверки ниже относятся к прежним версиям.
> [Текущее решение](../../superpowers/specs/2026-09-23-system-proxy-v2.md).
...
[Отчёт о текущей установке](reports/2026-09-18-installed-desktop-cli/report.md).
...
По умолчанию неопределённая операция Loginom не блокирует следующий `run` и не
даёт код 4: журнал мягкий, записи прошлого процесса стираются при старте host.
`LOGINOM_AI_AGENT_STRICT_RECOVERY=1` возвращает прежний замок для CLI и
приёмочных проверок host. В этом режиме `loginom recover --acknowledge --format json`
подтверждает текущие маркеры после ручной проверки внешнего состояния; без флага
неинтерактивный вызов возвращает 4. Скрипт `packages/loginom-host/script/cli-owner-crash.ts`
сам выставляет эту переменную. Ошибки инструментов `loginom_*` не делают `run`
неуспешным: код 1 по-прежнему дают провайдер, разрешения, невалидные вызовы и
ошибки остальных инструментов.

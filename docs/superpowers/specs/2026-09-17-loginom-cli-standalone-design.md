...
`LOGINOM_RECOVERY_REQUIRED`. Журнал в мягком режиме стирает такие записи при
открытии и при `settle(false)`. Следующий вызов, чат и запуск CLI продолжаются.
Операции не повторяются. `LOGINOM_AI_AGENT_STRICT_RECOVERY=1` возвращает прежний
замок в CLI; Desktop остаётся мягким, кнопка восстановления в настройках скрыта.
Завершившийся runtime чата заменяется, но не больше двух раз за ход. Состояние,
которое runtime сам не продолжит, сбрасывается на следующем сообщении. Ошибки
`loginom_*` не определяют код выхода `run`.
...
### Актуальный чеклист — состояние на 2026-09-18

Это единственный текущий чеклист документа. Отметки относятся к техническому
Linux-scope, принятому пользователем, и к описанным в отчётах сценариям.
История ниже сохраняет статусы на даты прежних запусков; её ограничения нельзя
читать как автоматически открытые задачи текущей версии или заменять на PASS.

#### Этапы реализации

- [x] Этап 1: независимые bootstrap, product identity, профили, writer guard,
  help/version и отсутствие обращений к Desktop global state.
- [x] Этап 2: общий host, Desktop adapter, отдельный CLI Node host,
  private bridge, disconnect/cancellation/cleanup.
- [x] Этап 3: полный run, реальные Loginom CSV 55/101, save/cold readback,
  сравнение с Desktop. Настоящий Xiaomi smoke принят пользователем отдельно.
- [x] Этап 4: TUI, automation, resume, permissions, JSON/exit codes,
  вложения и локальные recovery cases.
- [x] Этап 5: Linux archive/manifest, install/uninstall, sandbox,
  установленный CLI и regression установленного Desktop после extraction host.

#### Техническая Linux-приёмка IND-01…14

Подробные evidence, версии кандидатов и границы каждой проверки:
[матрица приёмки](../../testing/loginom-ai-agent/reports/2026-09-18-cli-final-linux/acceptance-audit.md).
Исторические fault/mode/installed проверки не объявляются повторёнными на каждом
последующем packaging-only кандидате; runtime-код после edc68138d не менялся.

- [x] IND-01: standalone TUI/run с комплектными ресурсами и ограниченным PATH;
  реальный Xiaomi выполнил Dock operations и импорт CSV.
- [x] IND-02: Desktop/TUI/run с одинаковыми исходниками продукта — 34 tool schemas,
  bootstrap/prepare instructions, knowledge и runtime pins совпали.
- [x] IND-03: scripted oracle 55/101, save/close и независимый cold readback
  через Desktop/TUI/run; полный Xiaomi-generated oracle пользователем не требуется.
- [x] IND-04: TUI/run headed/headless и resume с режимом текущего запуска.
- [x] IND-05: изоляция одноимённых вложений, исходных bytes и remote paths;
  два чата одного native TUI проверены последовательно.
- [x] IND-06: Desktop global roots не затронуты в проверенных CLI/TUI путях;
  оба порядка закрытия Desktop/CLI сохранили работоспособность второго процесса.
- [x] IND-07: один writer, aliases профиля и параллельные разные профили.
- [x] IND-08: ранний выбор профиля, guarded mutating entrypoints;
  help/version без host/browser и создания профиля/БД.
- [x] IND-09: setup/check/preserve/replace/empty/pending/cancel/recover;
  неинтерактивные команды не ожидают скрытого ввода.
- [x] IND-10: проверенные SIGINT, owner/host/runtime/browser crash и network-loss
  сценарии; durable recovery и отсутствие автоматического replay.
- [x] IND-11: private attachment admission, permission boundaries и secrets;
  явный bypass тестового oracle не считается проверкой permissions.
- [x] IND-12: JSON events, ошибки и exit codes, изоляция нового benchmark profile;
  ошибки ревью a7d812e4a исправлены и проверены.
...
- [ ] Этап 6: настоящие Windows/macOS build/install/crypto/proxy/CA/GUI/sandbox
  и платформенная приёмка. Source adapters и cross-build не заменяют native PASS.
- [ ] Полный юридический third-party attribution/source/relinking audit:
  явно отложен пользователем, ориентир пользователя — 2–5 рабочих дней.
  Подготовленные материалы сохранены; WebKit build остановлен, не завершён.
- [ ] Production signing/notarization и публикация: dev-only кандидаты
  не являются подписанным production release.

Известные ограничения доказательств сохранены: отдельный live OAuth flow не
подтверждён API-key Xiaomi smoke; существующие проверки путей не доказывают
защиту от конкурентной подмены файлов тем же UID; успех другого универсального
инструмента без подтверждённой связи не снимает прежнюю ошибку. Неопределённый
business outcome после crash не объявляется успешным или отменённым.

### История реализации с 2026-09-17

Этот раздел — хронологический журнал, а не текущий список задач. Метки
«Выполнено на дату записи» и «На момент записи оставалось / не подтверждалось»
сохраняют прежние наблюдения; актуальные отметки находятся только выше.
...
- **Выполнено на дату записи:** Отдельное имя `loginom-ai-agent-cli`; Node-only resolver профилей Linux/Windows/macOS с prod/beta/dev и tmp внутри cache.
- **Выполнено на дату записи:** Атомарный directory guard с nonce, канонизацией symlink aliases и отказом повторному writer. Автоматического снятия после crash нет.
- **Выполнено на дату записи:** Marker формата/канала, отказ неизвестным непустым каталогам и symlink-подкаталогам; зарезервированные корни Desktop отклоняются.
- **Выполнено на дату записи:** Минимальный entry с help/version без backend, host, браузера и профиля.
- **Выполнено на дату записи:** Передача профильных путей в Global до его import; устранение inherited Desktop DB/config/auth overrides и неявного home config fallback.
- **Выполнено на дату записи:** Исторический shared-service дизайн помечен; актуализированы AGENTS, Linux checkpoint и отдельный CLI runbook.
- **На момент записи оставалось / не подтверждалось:** Подключить реальные команды к этому bootstrap, proxy/CA и host handshake; проверить все writer entrypoints.
- **На момент записи оставалось / не подтверждалось:** Подтвердить собранный native entry и отсутствие обращений к Desktop глобальным данным при реальной работе.
...
- **На момент записи оставалось / не подтверждалось:** Этап 2: общая фабрика host, Desktop adapter, CLI Node host/bridge/cleanup.
- **На момент записи оставалось / не подтверждалось:** Этап 3: полный run и реальный oracle с Desktop-сравнением.
- **На момент записи оставалось / не подтверждалось:** Этап 4: TUI/automation/resume/permissions/JSON/exit codes/recovery.
- **На момент записи оставалось / не подтверждалось:** Этап 5: установленный Linux archive и Desktop regression.
- **На момент записи оставалось / не подтверждалось:** Этап 6: native Windows/macOS codecs, ресурсы, упаковка и приёмка.

### Этап 2 — начат параллельно подключению entry

- **Выполнено на дату записи:** Connection store/service, credential codec и recovery store вынесены в `packages/loginom-host/src/connection` без Electron imports. Desktop сохраняет совместимые re-exports и свои тесты.
- **Выполнено на дату записи:** Общая фабрика `createLoginomHost` принимает пути, codec, snapshot окружения и browser mode; Desktop composition заменена тонким Electron adapter.
- **Выполнено на дату записи:** Host явно объявляет Schema/Product/Effect dependencies; lockfile изменён только для этих зависимостей.
- **Выполнено на дату записи:** В supervisor окружение передаётся отдельно от start IPC и фильтруется; provider credentials не проходят в runtime.
- **Выполнено на дату записи:** Общий host-port без Electron imports; leases удерживаются для call/list/admit/interrupt до фактического завершения, включая release во время запроса.
- **Выполнено на дату записи:** Live uncertainty видна в status; последующий успешный вызов не удаляет её. Убрано неявное resume recovery lease. Явный acknowledge требует idle, сбрасывает runtime чатов и сохраняет journal при shutdown до commit.
- **Выполнено на дату записи:** CLI Node entry и client, private protocol handshake, management dispatcher и штатный close после завершения принятых requests. Собрана Node dependency closure и проверен настоящий дочерний процесс.
- **Выполнено на дату записи:** Bootstrap подключён к `loginom status --format json`: профиль и guard → Node host → чистый JSON → подтверждённый exit → освобождение guard.
- **Выполнено на дату записи:** Management setup/check/status/cancel-pending/recover подключены к общей host API; stdin JSON различает preserve/replace/empty, отсутствует скрытый ввод при автоматизации, recovery требует явного подтверждения.
- **Выполнено на дату записи:** Исходная форма interactive setup со скрытыми секретами и выводом в stderr; TTY acceptance ещё не выполнена.
- **Выполнено на дату записи:** Linux proxy policy вынесена из Desktop в общий Host и применяется до CLI network clients. Комплектный Node запускается с поддерживаемым `--use-system-ca`; explicit extra CA передаётся в host/runtime.
- **На момент записи оставалось / не подтверждалось:** TUI/run и обнаружение установленного bundle.
- **На момент записи оставалось / не подтверждалось:** Подключение awaitable host-port close к owner lifecycle, отмена runtime при disconnect и полное доказательство cleanup процессов.
- **Выполнено на дату записи:** Async codec contract и собственный versioned CLI Linux secret format; Desktop records совместимы.
- **На момент записи оставалось / не подтверждалось:** Native DPAPI/Keychain implementations/acceptance — этап 6; сейчас нет plaintext fallback на Windows/macOS.
...
- **Выполнено на дату записи:** Общий resolver development bundle для management/run; приватный host подключается к существующему LoginomHost adapter.
- **Выполнено на дату записи:** Loginom preflight до импорта backend: unconfigured → 2 без создания DB; recovery → 4; неподготовленное подключение отклоняется.
- **Выполнено на дату записи:** Существующий v1 RunCommand подключён через отдельный bootstrap; прямые exits внутри handler позволяют standalone выполнить cleanup.
- **Выполнено на дату записи:** Явное закрытие event subscription и отдельного HttpApiApp scope перед AppRuntime/host; процессная регрессия зависания на ошибке модели устранена.
- **Выполнено на дату записи:** Исходный процессный тест: после fixture setup ошибка несуществующей модели выдаёт JSON error, exit 1 и освобождает guard. Проверяется отсутствие setup secret в выводе.
- **На момент записи оставалось / не подтверждалось:** Успешный provider/tool prompt, model auth commands, SIGINT и полная семантика exit codes.
- **На момент записи оставалось / не подтверждалось:** Живой Loginom CSV/save/cold-reopen oracle и Desktop-сравнение.
...
- **Выполнено на дату записи:** Успешный исходный provider → Loginom tool fixture → continuation → JSON → exit 0.
- **На момент записи оставалось / не подтверждалось:** Реальный Loginom/Chromium oracle, SIGINT standalone и platform acceptance остаются открытыми.
...
- **Выполнено на дату записи:** Неинтерактивный standalone `run`, отклонивший запрошенное разрешение,
...
- **Выполнено на дату записи:** Процессная проверка при явно заданной политике `loginom_*: ask`:
...
- **На момент записи оставалось / не подтверждалось:** Прямой policy deny без permission.asked, неисправленные/исправленные tool errors,
...
- **Выполнено на дату записи:** SIGINT во время noninteractive standalone run вызывает штатный session.abort;
...
- **Выполнено на дату записи:** Процессный тест с зависшим локальным HTTP-provider: SIGINT → JSON cancellation,
  exit 130 и освобождение guard; Agent typecheck PASS, 2 теста/59 assertions PASS.
- **На момент записи оставалось / не подтверждалось:** Отмена на раннем startup/setup, в TUI и при активном внешнем Loginom effect
...
- **Выполнено на дату записи:** Команды `providers` (`auth`) и `models` направлены в существующие backend
...
- **Выполнено на дату записи:** Backend AppRuntime закрывается до освобождения guard; command failure
  возвращает ограниченный code, ошибки cleanup сохраняют guard.
- **На момент записи оставалось / не подтверждалось:** Interactive provider login/logout и live OAuth acceptance пока не выполнены;
...
- **Выполнено на дату записи:** Worker RPC возвращает ограниченную ошибку вместо вечного pending при throw;
  client.close отклоняет текущие и будущие запросы, transport send failure не оставляет pending.
- **Выполнено на дату записи:** Worker shutdown закрывает instances, listener, загруженный HttpApiApp scope
  и AppRuntime. TUI handler включает ранние ошибки validateSession в cleanup.
- **Выполнено на дату записи:** Standalone-вариант handler не скрывает shutdown timeout/error и возвращается
  внешнему owner вместо process.exit; legacy entry сохраняет прежний final exit.
- **На момент записи оставалось / не подтверждалось:** Private Loginom bridge между главным процессом и worker, disconnect propagation,
...
- **Выполнено на дату записи:** Default invocation направлен в существующий TuiThreadCommand после Loginom
...
- **Выполнено на дату записи:** Worker подключает LoginomHost adapter через отдельные RPC request/reply frames;
  host disconnect передаётся явно и закрывает pending/future requests, без зависимости
  от Bun MessagePort.close.
- **Выполнено на дату записи:** Завершение TUI возвращается внешнему owner для host/backend cleanup. Ошибка
  worker shutdown преобразуется в LOGINOM_TUI_CLEANUP_FAILED и не освобождает guard.
- **На момент записи оставалось / не подтверждалось:** Настоящий PTY/TUI запуск, prompt с Loginom tools, выход, resume и worker crash
...
- **Выполнено на дату записи:** Реальный standalone entry запускает TUI и worker с чистым CLI-профилем,
...
- **Выполнено на дату записи:** Подтверждены exit 0, отсутствие forced termination, снятый guard и отсутствие
  живых PID всех fixture runtime и Node host. Успешный ручной запуск повторён.
- **Выполнено на дату записи:** Воспроизводимый harness сохранён в `packages/agent/test/cli/tui/standalone-pty.py`;
  требует Linux, Python3 PTY и существующий закреплённый Node в Desktop resources.
- **На момент записи оставалось / не подтверждалось:** Prompt/tool invocation из настоящего TUI, resume, worker crash, установленный
...
- **Выполнено на дату записи:** PTY harness получил режим `--prompt` с локальным OpenAI-compatible HTTP-provider.
...
- **Выполнено на дату записи:** Управляемый runtime fixture записывает факт вызова; tool result возвращается
...
- **Выполнено на дату записи:** В тесте задана policy `loginom_*: ask`; разрешение передаётся явным dangerous
  flag. Штатный TUI exit даёт 0, guard снят, PID host/runtime отсутствуют.
- **На момент записи оставалось / не подтверждалось:** Реальный Loginom/Chromium, TUI resume и installed artifact acceptance остаются открытыми.
...
- **Выполнено на дату записи:** `packages/agent/script/build.ts --standalone` собирает отдельный ранний entry
...
- **Выполнено на дату записи:** Standalone build не вызывает legacy GitHub release upload и не выдаёт bare binary
  за законченный архив с Loginom runtime.
- **Выполнено на дату записи:** Linux x64 build с Bun 1.3.14: version `0.0.0-dev-202609171623`, размер
  145758336 bytes, SHA256 `0a6029c3c299dfeeca9605a2fcd5da7fc5f18c11f78a352b34bb66779e888fe4`.
- **Выполнено на дату записи:** Native help/version в пустом cwd не создают профиль или XDG каталоги.
- **Выполнено на дату записи:** Native PTY prompt/tool acceptance с тем же управляемым Node runtime fixture:
  setup, TUI worker, Loginom tool, continuation, SQLite history и cleanup PASS.
- **На момент записи оставалось / не подтверждалось:** Общий resource staging, manifest verification, standalone archive/install/uninstall,
...
- **Выполнено на дату записи:** Desktop bundler заменён wrapper над `packages/loginom-host/script/stage-resources.ts`.
...
- **Выполнено на дату записи:** Неподдержанные native targets, относительные пути и output, перекрывающий inputs,
  отклоняются. Windows/macOS пока не получают Linux pins по умолчанию.
- **Выполнено на дату записи:** Реальный Linux staging в CLI resources: 4365 файлов; Node 24.19.0,
  Chromium revision 1243; каждый hash перечитан и подтверждён после сборки.
- **Выполнено на дату записи:** Node host dependency closure собрана рядом; native binary с этим payload выполняет
  `loginom status --format json` (unconfigured, exit0, guard снят).
- **На момент записи оставалось / не подтверждалось:** Полный CLI artifact manifest, включающий host/native binary, установщик,
...
- **Выполнено на дату записи:** Общий `cli-manifest.ts` описывает version/channel/target, source commit,
...
- **Выполнено на дату записи:** Verifier отвергает изменённые/лишние/пропущенные payload files, дубликаты путей,
...
- **Выполнено на дату записи:** Installed resolver в исходниках ищет artifact root относительно process.execPath,
  проверяет manifest и выбирает resources/loginom. Явный CLI_BUNDLE сохраняет
  development fixture режим.
- **На момент записи оставалось / не подтверждалось:** Build orchestration должна сформировать manifest с настоящим source snapshot
...
- **Выполнено на дату записи:** `packages/loginom-host/script/build-cli.ts <new-absolute-output>` собирает
...
- **Выполнено на дату записи:** Native build принимает новый абсолютный output для orchestrator, сохраняя
...
- **Выполнено на дату записи:** Новый Linux кандидат выполняет status без CLI_BUNDLE, включая symlink launcher.
  Extra payload file вызывает ограниченный отказ до host launch; guard освобождается.
- **На момент записи оставалось / не подтверждалось:** Installer/uninstaller, архив и release provenance из clean committed source
...
- **Выполнено на дату записи:** В payload включены install.sh/uninstall.sh и bundled Node installer closure.
...
- **Выполнено на дату записи:** Чужой/существующий launcher не перезаписывается; установка сериализована
  отдельным lock. Для update требуется остановка процессов, uninstall, затем install.
- **Выполнено на дату записи:** Uninstall проверяет receipt/launcher/manifest, отклоняет активные executable PID
  внутри payload и удаляет только текущую установку. Profiles/Desktop не удаляются.
- **Выполнено на дату записи:** Реальная установка native candidate в временный home → status без bundle override
  → запуск uninstall из installed payload → сохранение profile sentinel PASS.
- **На момент записи оставалось / не подтверждалось:** Windows/macOS installers, live Loginom oracle, license audit и clean release
...
- **Выполнено на дату записи:** Development Linux tar.gz создан и распакован; native status из распакованного
...
- **Выполнено на дату записи:** Supervisor требует одновременно ответ `closed: true` и завершение дочернего
...
- **Выполнено на дату записи:** Managed runtime отклоняет новые операции после начала закрытия, прерывает
...
- **Выполнено на дату записи:** Host: 16 тестов/78 assertions и typecheck PASS. Включены реальные pinned Node
  процессы без acknowledgement, с exit 1 и с зависанием после acknowledgement.
- **Выполнено на дату записи:** CLI process regression: 2 теста/59 assertions PASS. Desktop connection/host-port/
  recovery regression: 25 тестов/74 assertions и Desktop typecheck PASS.
- **Выполнено на дату записи:** Исходный managed-entry под pinned Node: close до start и close одновременно
  с ошибкой start завершаются с acknowledgement, exit 0, без сигнала. Syntax check PASS.
- **На момент записи оставалось / не подтверждалось:** Завершение настоящего Chromium, owner loss во время внешнего эффекта и
...
- **Выполнено на дату записи:** Общий tool context сохраняет `permissionDenied: true` в metadata перед
...
- **Выполнено на дату записи:** Реальный source process с fixture provider: доступный bash, policy deny для
...
- **Выполнено на дату записи:** Очередь fixture provider сбрасывается между policy-deny и cancellation
  сценариями: оставшийся после остановки текст больше не подменяет следующий вызов.
- **Выполнено на дату записи:** Standalone: 2 теста/64 assertions PASS; legacy run: 13/47 PASS; processor:
  17/80 PASS. Agent typecheck, formatting и diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Полностью скрытый политикой инструмент обрабатывается как `invalid`;
...
- **Выполнено на дату записи:** Loginom MCP result с isError=true теперь становится ошибкой инструмента,
...
- **Выполнено на дату записи:** Source process через настоящий приватный Node host и fixture runtime/provider
...
- **Выполнено на дату записи:** Standalone process suite: 2 теста/71 assertions PASS; Agent typecheck PASS;
  diff check PASS. Runtime fixture не запускает Chromium и не доказывает live oracle.
- **На момент записи оставалось / не подтверждалось:** Итоговый ненулевой exit при неустранённой tool error/invalid ещё не реализован.
...
- **Выполнено на дату записи:** build-cli.ts создаёт versioned tar.gz и отдельный .sha256 рядом с payload.
  Tar нормализует порядок, mtime, owner/group; checksum вычисляется потоково.
- **Выполнено на дату записи:** До публикации архив распаковывается во временный каталог, где проверяется
...
- **Выполнено на дату записи:** Новый native candidate собран, извлечён, установлен в временный home; native
...
- **Выполнено на дату записи:** Host typecheck теперь включает script/ вместе с src/ и test/; PASS.
- **На момент записи оставалось / не подтверждалось:** Это dirty development candidate. Live Loginom/Chromium, системные библиотеки,
...
- **Выполнено на дату записи:** Обязательные hasApiKey/ready preflight проверки ограничены командой run.
...
- **Выполнено на дату записи:** После startup prompts stdin возобновляется перед передачей TUI: readline
  закрывает prompt с приостановкой потока.
- **Выполнено на дату записи:** Source PTY unconfigured + fixture provider: setup пропущен, обычный ответ
  сохранён в SQLite, Loginom tools не advertised/called, runtime не запущен,
  exit 0 и guard снят (`/tmp/loginom-cli-tui-7gkkq2zw`).
- **Выполнено на дату записи:** Source PTY unconfigured без provider: предложение setup, provider dialog,
...
- **Выполнено на дату записи:** Настроенный source TUI prompt/tool/history regression PASS
  (`/tmp/loginom-cli-tui-i3mtntr1`). CLI process tests: 2/71 PASS, Agent typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Полный интерактивный ввод setup, ошибочные credentials, live Loginom и
...
- **Выполнено на дату записи:** PTY harness умеет --setup: выбирает настройку на старте, вводит API-ключ,
...
- **Выполнено на дату записи:** --setup --password --prompt выбирает ввод нового пароля. Оба fixture secrets
...
- **Выполнено на дату записи:** --cancel-setup посылает Ctrl+C на поле API-ключа: exit 130, guard снят,
  runtime/tool не запущены, forced=false (`/tmp/loginom-cli-tui-j_6tj84i`).
- **На момент записи оставалось / не подтверждалось:** Проверки использовали source entry и fixture runtime/provider. Они не
...
- **Выполнено на дату записи:** Добавлен script/browser-acceptance.ts: bundled Node + managed runtime +
...
- **Выполнено на дату записи:** Реальная проверка выявила race: runtime после startup error выходил до
...
- **Выполнено на дату записи:** На новом payload successful validation и wrong identity: по 10 browser PID,
  sandbox=true, alive=[]; ошибка сохраняется как LOGINOM_ACCOUNT_MISMATCH.
- **Выполнено на дату записи:** Native CLI setup rejection с копией real resource bundle и локальным
...
- **Выполнено на дату записи:** Host tests: 16/78 PASS, typecheck со script/ PASS. Исходный runtime также
  подтвердил acknowledged close после error и 100 ms задержки owner запроса.
- **Выполнено на дату записи:** Новый native binary прошёл PTY --setup --password --prompt: оба секрета
...
- **На момент записи оставалось / не подтверждалось:** Не проверены owner loss во время настоящего внешнего эффекта, startup
...
- **Выполнено на дату записи:** Runtime oracle A/B на ресурсе 0.0.0-dev-202609171710: два sales.csv,
...
- **Выполнено на дату записи:** Parent SIGKILL после readiness: 12 отслеживаемых процессов, живых потомков 0.
  parent-crash.ts принимает явный resource override и проверяет passwordless config.
- **Выполнено на дату записи:** Desktop typecheck и diff check PASS. Установленный Desktop не изменялся.
- **На момент записи оставалось / не подтверждалось:** Эти проверки вызывают managed runtime напрямую. Не закрыты одинаковый prompt
...
- **Выполнено на дату записи:** Standalone run отслеживает terminal tool parts до idle. Неустранённый
...
- **Выполнено на дату записи:** Для Loginom ошибка закрывается успешным вызовом того же инструмента с тем же
...
- **Выполнено на дату записи:** Process suite: 2 теста/82 assertions PASS. Проверены одиночный MCP isError,
...
- **Выполнено на дату записи:** Outcome tests: 4/9 PASS; legacy run regression: 13/47 PASS; Agent typecheck
  и diff check PASS. Permission/session errors сохраняют свою отдельную семантику.
- **На момент записи оставалось / не подтверждалось:** Для универсального инструмента исправление через изменённые аргументы
...
- **Выполнено на дату записи:** Добавлен ручной `script/cli-oracle-transport.ts`: локальный scripted provider
...
- **Выполнено на дату записи:** Исправлен Linux startup Chromium при длинном пути профиля: короткий private
...
- **Выполнено на дату записи:** Standalone `run --file` сохраняет snapshot байтов в исходном сообщении,
...
- **Выполнено на дату записи:** Собран и проверен Linux candidate 0.0.0-dev-202609171741:
...
- **Выполнено на дату записи:** Живой CLI run на этом candidate: setup ready; dock_prepare получил ровно
...
- **На момент записи оставалось / не подтверждалось:** Полный CLI oracle НЕ пройден. dock_node_apply(import-A) вернул `running`,
...
- **Выполнено на дату записи:** Acceptance adapter сохраняет отдельный exit receipt и различает ненулевой
...
- **Выполнено на дату записи:** Runtime сообщает отдельный private `activeWork`: выполняющийся executor,
...
- **Выполнено на дату записи:** Общий host сохраняет durable journal записей текущего владельца через
...
- **Выполнено на дату записи:** Новый процессный regression test: 5 сценариев/58 assertions PASS. Проверены
...
- **На момент записи оставалось / не подтверждалось:** Новый native artifact и живой полный CLI oracle после этого изменения
...
- **Выполнено на дату записи:** Candidate `0.0.0-dev-202609171753` собран с проверкой extracted manifest;
...
- **Выполнено на дату записи:** Настоящий CLI `run --headless --format json --file` через scripted provider
...
- **Выполнено на дату записи:** Native TUI PTY setup/password/prompt на этом candidate PASS: exit=0,
...
- **На момент записи оставалось / не подтверждалось:** Полные IND-03/04/05/10/14 остаются открытыми: нет одинакового
...
- **Выполнено на дату записи:** Standalone SessionPrompt сохраняет full-file текстовое вложение пользователя
...
- **Выполнено на дату записи:** Общий `util/file-snapshot.ts` используется также `run --file`: максимум
...
- **Выполнено на дату записи:** SessionPrompt regression 1/4 PASS: точный snapshot исходного CSV в истории
...
- **Выполнено на дату записи:** Source TUI PTY: выбор `@sales.csv`, отправка prompt, tool call, точные байты
...
- **На момент записи оставалось / не подтверждалось:** Native candidate с этим изменением и живой TUI CSV oracle ещё не проверены.
  PTY использовал fixture provider/runtime и не закрывает полный IND-03.

Native подтверждение вложения TUI:

- **Выполнено на дату записи:** Candidate `0.0.0-dev-202609171808`, artifact
...
- **Выполнено на дату записи:** PTY `--prompt --attachment` на native executable PASS: выбор CSV через
...
- **Выполнено на дату записи:** В source standalone SessionPrompt текст модели и data URL полного
...
- **Выполнено на дату записи:** Manual oracle adapter поддерживает `LOGINOM_AI_AGENT_TEST_CLI_INTERFACE=tui`.
...
- **На момент записи оставалось / не подтверждалось:** Живой TUI oracle на candidate 0.0.0-dev-202609171808 запущен в новых профилях;
...
- **Выполнено на дату записи:** Native TUI 0.0.0-dev-202609171808: два CSV выбраны через autocomplete,
...
- **Выполнено на дату записи:** Оба independent cold readback PASS: total=55/101, settingsReapplied=false,
...
- **На момент записи оставалось / не подтверждалось:** Полные IND-02/03/04/10/14 остаются открытыми: Desktop не сравнивался на
...
- **Выполнено на дату записи:** Native candidate 0.0.0-dev-202609171808: run создаёт сессию с headless=true,
...
- **Выполнено на дату записи:** Та же проверка через `--continue --no-headless` PASS; evidence
  `/tmp/loginom-cli-tui-dgad2a02`. В обоих случаях exit=0, forced=false, guard=false,
  alive=[], resume_verified=true, mode_changed=true.
- **Выполнено на дату записи:** PTY fixture теперь записывает private start envelopes для различения
...
- **На момент записи оставалось / не подтверждалось:** Использован fixture runtime/provider, без настоящего окна Chromium.
...
- **Выполнено на дату записи:** Ошибка синтаксиса ID выделена в InvalidSessionError. Standalone TUI
...
- **Выполнено на дату записи:** Source process PTY `--invalid-session`: code=2, forced=false, guard=false,
  alive=[], tool_called=false; evidence `/tmp/loginom-cli-tui-uxvmbej7`.
- **Выполнено на дату записи:** Validator test с настоящим локальным HTTP server: 1 test/5 assertions PASS.
  Неверный ID не вызывает API; 404 для корректного ID не становится
  InvalidSessionError. Agent typecheck и diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Это source изменение; candidate 0.0.0-dev-202609171808 ему предшествует.
...
- **Выполнено на дату записи:** `script/browser-acceptance.ts --headed` на ресурсах candidate
...
- **Выполнено на дату записи:** Headless с `--window-check`: ни одного mapped окна тестового Chromium,
  cleanup PASS, alive=[], sandbox=true, 10 browser processes.
  Log `/tmp/loginom-headless-window.log`.
- **Выполнено на дату записи:** Headed с ошибкой identity: окно появилось и затем закрылось; итоговый
...
- **Выполнено на дату записи:** Host typecheck и diff check PASS. Observer читает только свойства окон,
  не закрывает чужие окна и не меняет существующие приложения.
- **На момент записи оставалось / не подтверждалось:** Эти проверки вызывают настоящий managed runtime напрямую и используют
...
- **Выполнено на дату записи:** Manual oracle принимает `LOGINOM_AI_AGENT_TEST_CLI_HEADED=1` для run/TUI.
  Режим передаётся через реальный CLI флаг, а не изменением browser fixture.
- **Выполнено на дату записи:** Добавлен пассивный X11 observer: сопоставляет окна с PID Chromium по
...
- **Выполнено на дату записи:** Host/Desktop typecheck и diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Native сборка с последними source исправлениями и полный headed oracle
  ещё требуют выполнения; наличие observer само по себе не является PASS.


Живой headed run и новая native сборка (этапы 3/4/5):

- **Выполнено на дату записи:** Candidate 0.0.0-dev-202609171831 собран и проверен из dirty source snapshot
...
- **Выполнено на дату записи:** Native invalid Session ID: exit 2, guard=false, alive=[]; native TUI CSV
...
- **Выполнено на дату записи:** Живой `run --no-headless`: оба CSV построены/выполнены/сохранены/закрыты;
...
- **Выполнено на дату записи:** Исправлен EOF в acceptance adapter: TUI сохраняет control stdin, run сразу
  закрывает его. Первый attempt до исправления не выполнял tools; новый прошёл
  на новых профилях. Host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** SIGINT при ожидании stdin до dispatch: отдельный остановленный attempt
...
- **На момент записи оставалось / не подтверждалось:** Headed TUI, живой resume, одинаковая Desktop/TUI/run сборка и остальные
...
- **Выполнено на дату записи:** Standalone чтение prompt stdin допускает SIGINT без ожидания EOF. Отмена
...
- **Выполнено на дату записи:** Source process test намеренно держит пишущий конец pipe открытым до выхода
...
- **Выполнено на дату записи:** Два subprocess boundary cases проверяют SIGINT и обычный EOF с UTF-8;
...
- **На момент записи оставалось / не подтверждалось:** Исправление предшествует следующей native пересборке; candidate
...
- **Выполнено на дату записи:** Bootstrap run держит обработчик SIGINT до завершения host cleanup и снятия
...
- **Выполнено на дату записи:** При SIGINT во время запуска host команда дожидается результата запуска,
...
- **Выполнено на дату записи:** Новый subprocess test передаёт ранний настоящий SIGINT в последующее
...
- **Выполнено на дату записи:** Provider-management test первоначально достиг лимита 5 секунд; отдельно
...
- **На момент записи оставалось / не подтверждалось:** Native artifact ещё не пересобран с этим controller. Реальная отмена при
...
- **Выполнено на дату записи:** Desktop connection/credentials/recovery/host-port: 33 tests/100 assertions;
...
- **Выполнено на дату записи:** Desktop build с prod channel и комплектными ресурсами PASS; отдельный
...
- **Выполнено на дату записи:** Live GUI smoke через development Electron и packaged executable PASS:
...
- **Выполнено на дату записи:** Записаны dirty snapshot provenance, ASAR/resource hashes, evidence и
...
- **На момент записи оставалось / не подтверждалось:** DEB/AppImage install/upgrade, Desktop CSV oracle, одинаковый source build
...
- **Выполнено на дату записи:** В `packages/desktop/test/loginom/fixtures/standalone-cli` сохранены два
...
- **Выполнено на дату записи:** `runtime-acceptance.ts` использует эти файлы для direct runtime и CLI;
...
- **Выполнено на дату записи:** Исторические прогоны с B=100/1 не переписываются. Headed TUI
...
- **Выполнено на дату записи:** Новый native headless run `/tmp/loginom-linux-oracle-lO4HVe` прошёл оба
...
- **На момент записи оставалось / не подтверждалось:** Полные IND-02/03/04/05/14 остаются открытыми: нет общего source snapshot
...
- **Выполнено на дату записи:** `oracle-provider.ts` общий для CLI/Desktop acceptance. Desktop transport
...
- **Выполнено на дату записи:** Desktop candidate `/tmp/loginom-desktop-cli-regression-20260917/linux-unpacked`
...
- **Выполнено на дату записи:** Native headless TUI с общим provider прошёл оба канонических fixtures:
...
- **Выполнено на дату записи:** `compare-oracle-contracts.ts` сравнил четыре фактических model captures
...
- **На момент записи оставалось / не подтверждалось:** Полные IND-02/03/14 ещё открыты: Desktop и CLI candidates имеют разные
...
- **Выполнено на дату записи:** Попытка единой сборки 0.1.4-cli.202609171924 обнаружила настоящий отказ
...
- **Выполнено на дату записи:** Build archive-check теперь использует `tar --same-permissions`.
...
- **На момент записи оставалось / не подтверждалось:** Desktop candidate из snapshot
...
- **Выполнено на дату записи:** Desktop/CLI 0.1.4-cli.202609171930, prod, собраны из одинакового dirty
...
- **Выполнено на дату записи:** Последний SIGINT controller проверен в native binary с fixture runtime:
...
- **Выполнено на дату записи:** CLI установлен в свободный пользовательский launcher. Run/TUI запущены
...
- **Выполнено на дату записи:** Packaged Desktop `/tmp/loginom-linux-oracle-7pcbuP` и installed headless
...
- **Выполнено на дату записи:** IND-02 для этой Linux сборки: шесть model captures совпали по 34 schemas
...
- **Выполнено на дату записи:** Installed run `/tmp/loginom-linux-oracle-vtSkV0`: A cold total=55 PASS;
...
- **На момент записи оставалось / не подтверждалось:** IND-03 остаётся FAIL на общей сборке из-за run B. Требуется разобраться
...
- **Выполнено на дату записи:** Локальный regression воспроизвёл отказ выбора destination folder при
...
- **Выполнено на дату записи:** Только matched action receipt с NOT_APPLIED/preconditions,
...
- **Выполнено на дату записи:** Negative cases подтверждают отсутствие upload/retry при ambiguity,
...
- **Выполнено на дату записи:** Отдельный runtime bundle `/tmp/loginom-epoch-runtime-20260917` содержит
  точный исправленный модуль (SHA256
  `35aff53d345de3b29eb7f8212f66342a129afdae63dfcb4c24fdcb3f594e7c80`).
- **Выполнено на дату записи:** Живой direct-runtime oracle `/tmp/loginom-linux-oracle-Wd5srE` прошёл
...
- **На момент записи оставалось / не подтверждалось:** Native candidates 19:30 ещё не содержат fix; нужна пересборка и native
...
- **Выполнено на дату записи:** CLI/Desktop 0.1.4-cli.202609171958, prod, собраны из одного dirty snapshot
...
- **Выполнено на дату записи:** IND-02/03 для описанного Linux oracle: одинаковые canonical A/B,
...
- **Выполнено на дату записи:** Native headless run `/tmp/loginom-linux-oracle-JyyOe1`: 34 receipts,
...
- **Выполнено на дату записи:** У всех шести cases совпали fixture hashes, source/package paths различны.
...
- **На момент записи оставалось / не подтверждалось:** Полная mode/resume, permissions/recovery/native-platform matrix,
...
- **Выполнено на дату записи:** Добавлен повторяемый resume acceptance driver и уникальные invocation tool IDs
...
- **Выполнено на дату записи:** Native 19:58: исходная run A сессия продолжена через headed TUI `--session`,
...
- **На момент записи оставалось / не подтверждалось:** Остальные сочетания resume/mode, recovery/cancellation и native platform gates
  этим тестом не закрыты. Полная реализация плана продолжается.


Защита путей Linux installer (этап 5, source):

- **Выполнено на дату записи:** `cli-install.ts` проверяет через lstat каждую компоненту от home до
...
- **Выполнено на дату записи:** Проверены redirects `.local`, `.local/share`, `.local/bin`, самого install
...
- **На момент записи оставалось / не подтверждалось:** Конкурентная подмена компонентов другим процессом между проверкой и
...
- **Выполнено на дату записи:** Добавлен CLI DPAPI adapter через системный Windows PowerShell/.NET
...
- **Выполнено на дату записи:** Linux проверки: 4 PASS, 1 native-only SKIP, 9 assertions; host typecheck PASS.
  Добавлен Windows-only roundtrip/randomized ciphertext/tamper test.
- **На момент записи оставалось / не подтверждалось:** Настоящая Windows DPAPI приёмка, включая другую учётную запись,
...
- **Выполнено на дату записи:** Исправлен case-sensitive allowlist для Windows plain environment objects:
...
- **Выполнено на дату записи:** Тест подтверждает mixed-case OS/proxy keys, отсутствие provider secret,
...
- **На момент записи оставалось / не подтверждалось:** Это Linux real-process и platform-parameter tests. Реальный Windows host,
  DPAPI, proxy/CA и packaging acceptance остаются NOT_RUN.


Общий staging: пересечение входов и выходов (этапы 2/5/6, source):

- **Выполнено на дату записи:** Исправлена односторонняя проверка путей: destination и destination.staging
...
- **Выполнено на дату записи:** Stage-resources tests: 6 PASS, 8 assertions; host typecheck и diff check PASS.
...
- **На момент записи оставалось / не подтверждалось:** Проверка лексических путей не доказывает отсутствие symlink aliases или
...
- **Выполнено на дату записи:** К лексической проверке пересечений добавлена проверка canonical paths.
...
- **Выполнено на дату записи:** Проверены output alias на browser input, вложенный output через alias
...
- **На момент записи оставалось / не подтверждалось:** Конкурентная подмена пути после проверки остаётся открытой; полный resource
  build и новый native archive после изменения пока не запускались.


Полная сборка после staging/installer изменений (этап 5):

- **Выполнено на дату записи:** Native candidate `0.1.4-cli.202609180020`, prod: build/version smoke,
...
- **Выполнено на дату записи:** Штатные install/help/version/uninstall нового артефакта PASS;
...
- **На момент записи оставалось / не подтверждалось:** Live pipeline/resume и Desktop regression на новом candidate не выполнялись;
...
- **Выполнено на дату записи:** Добавлен повторяемый manual driver `cli-owner-crash.ts`; новый profile,
...
- **Выполнено на дату записи:** CLI exit=137; 24 отслеживаемых процесса, alive=[] после завершения.
...
- **На момент записи оставалось / не подтверждалось:** Остальной IND-10, особенно crash/network во время внешней мутации,
...
- **Выполнено на дату записи:** Driver `cli-owner-crash.ts` поддерживает отдельные сценарии SIGKILL/SIGINT
  с разными ожидаемыми exit/guard/retry результатами.
- **Выполнено на дату записи:** На candidate 0.1.4-cli.202609180020 SIGINT при ожидании модели после успешного
  ...
  [Отчёт](../../testing/loginom-ai-agent/reports/2026-09-17-cli-owner-crash/report.md).
- **На момент записи оставалось / не подтверждалось:** Отмена активной внешней мутации и остальные crash/network случаи остаются
...
- **Выполнено на дату записи:** Полный `bun test` из packages/loginom-host с pinned Node 24.19.0:
...
- **Выполнено на дату записи:** Agent run-outcome: 4 PASS, 9 assertions; package-local Agent typecheck PASS.
  Несвязанный успех не стирает ошибку; exact args и Loginom operation_id
  подтверждают поддерживаемую связь исправления.
- **На момент записи оставалось / не подтверждалось:** Исправление универсального инструмента через другие аргументы по-прежнему
...
- **Выполнено на дату записи:** Driver resume-oracle получил явный `--inverse`: headless TUI --session,
...
- **Выполнено на дату записи:** TUI: exit=0/guard=false/forced=false/visible=[]; run:
...
- **На момент записи оставалось / не подтверждалось:** Все browser modes/resume на едином release artifact, mutation recovery
...
- **Выполнено на дату записи:** Candidate 0.1.4-cli.202609180020 дополнительно прошёл headed TUI --session
...
- **Выполнено на дату записи:** Вместе с inverse-прогоном `/tmp/loginom-live-resume-nPuhW5` оба интерфейса
...
- **На момент записи оставалось / не подтверждалось:** Это не закрывает Windows/macOS и оставшиеся mutation/recovery/release gates.


Подготовка macOS Keychain codec (этап 6):

- **Выполнено на дату записи:** Реализован versioned keychain envelope: AES-256-GCM, новый 96-bit nonce
...
- **Выполнено на дату записи:** Реальный crypto test без mock: UTF-8 roundtrip, новые ciphertext, неправильный
...
- **На момент записи оставалось / не подтверждалось:** Envelope пока не подключён к darwin CLI: он продолжает fail closed.
...
- **Выполнено на дату записи:** Добавлен Objective-C helper `native/keychain.m` с Security/Foundation:
...
- **Выполнено на дату записи:** `cli-keychain.ts` запускает только абсолютный bundled helper, с пустым env,
...
- **Выполнено на дату записи:** Добавлен native-only `build-keychain.ts`: macOS arm64, deployment target 14,
...
- **На момент записи оставалось / не подтверждалось:** Objective-C source не компилировался на этой Linux-машине. Native helper
...
- **Выполнено на дату записи:** Добавлен `script/keychain-acceptance.ts`: missing read, crypto roundtrip,
...
- **Выполнено на дату записи:** Host typecheck/diff check PASS. На Linux driver проверенно завершился
  MACOS_ARM64_REQUIRED до создания test profiles. Runbook содержит native команды,
  service/account naming и границы проверки.
- **На момент записи оставалось / не подтверждалось:** Реальная macOS execution остаётся NOT_RUN; Linux отказ не считается Keychain
  PASS. Locked/denied Keychain, signing/update ACL и packaged host требуют native ОС.


Границы native credential payload (этап 6):

- **Выполнено на дату записи:** DPAPI Unprotect допускает служебное расширение ciphertext относительно
...
- **Выполнено на дату записи:** Keychain и DPAPI отклоняют oversized envelopes до вызова OS helper.
...
- **На момент записи оставалось / не подтверждалось:** Native boundary roundtrip остаётся NOT_RUN. Запрошена информация о доступных
...
- **Выполнено на дату записи:** Artifact manifest требует `resources/loginom/bin/loginom-keychain` для darwin;
...
- **Выполнено на дату записи:** Standalone resolver проверяет наличие helper и в dev bundle override;
  missing helper не откладывается до операции сохранения credentials.
- **Выполнено на дату записи:** Manifest tests: 3 PASS, 14 assertions, включая missing/tampered helper;
  Host и Agent package-local typecheck, diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Это fixture проверка состава, не компиляция/исполнение Mach-O helper и
...
- **Выполнено на дату записи:** Новый manual driver запускает настоящий candidate 0.1.4-cli.202609180020
...
- **Выполнено на дату записи:** Ранний выход после permission denial корректно учитывается драйвером;
...
- **На момент записи оставалось / не подтверждалось:** Остальная permissions/attachment/recovery матрица этим run не закрыта.


TUI permission rejection через PTY (этап 4, IND-11):

- **Выполнено на дату записи:** Native candidate 0.1.4-cli.202609180020, отдельный profile: TUI показывает
...
- **Выполнено на дату записи:** Автоматический exit через Ctrl+D: code=0/guard=false/forced=false,
...
- **На момент записи оставалось / не подтверждалось:** Allow once/always, private attachment и оставшаяся permissions matrix
  ещё не закрыты. Нулевой TUI exit не интерпретируется как успешный tool result.


TUI Allow once (этап 4, IND-11):

- **Выполнено на дату записи:** Driver подтверждает видимый permission prompt через Enter, без skip-permissions.
...
- **Выполнено на дату записи:** approved=true/rejected=false/forced=false/exit=0/guard=false;
  host typecheck/Python AST/diff check PASS. [Отчёт](../../testing/loginom-ai-agent/reports/2026-09-17-cli-permissions/report.md).
- **На момент записи оставалось / не подтверждалось:** Следующий tool call после Allow once, Allow always и остальные границы
  permissions/attachments остаются открытыми.


Срок действия Allow once (этап 4, IND-11):

- **Выполнено на дату записи:** Native TUI 0.1.4-cli.202609180020: первый dock_prepare одобрен Allow once
...
- **Выполнено на дату записи:** Evidence `/tmp/loginom-cli-permission-80hod8`: approved=true/rejected=true,
...
- **На момент записи оставалось / не подтверждалось:** Allow always, межсессионные/после-перезапуска границы и оставшиеся
  attachments/recovery gates ещё открыты.


TUI Allow always в одной сессии (этап 4, IND-11):

- **Выполнено на дату записи:** Native 0.1.4-cli.202609180020: видимые Allow always + Confirm;
...
- **Выполнено на дату записи:** Evidence `/tmp/loginom-cli-permission-JljE8Q`: alwaysConfirmed=true,
...
- **На момент записи оставалось / не подтверждалось:** Межсессионная граница и перезапуск после Always отдельно не проверены;
  полный IND-11 этим сценарием не объявляется завершённым.


Always после restart/resume (этап 4, IND-11):

- **Выполнено на дату записи:** Новый native run --continue на profile/workspace предыдущего Always-прогона,
...
- **Выполнено на дату записи:** Evidence `/tmp/loginom-cli-permission-Kpsr3d`; host typecheck/diff check PASS.
  [Отчёт](../../testing/loginom-ai-agent/reports/2026-09-17-cli-permissions/report.md).
- **На момент записи оставалось / не подтверждалось:** Два чата одного живого процесса и оставшиеся attachment/recovery границы
  по-прежнему требуют отдельной проверки.


Выбор standalone target и Windows native binary (этап 6):

- **Выполнено на дату записи:** Agent build поддерживает --target=linux-x64/darwin-arm64/win32-x64
...
- **Выполнено на дату записи:** С Linux собран Windows CLI/TUI 0.1.4-cli.202609180110:
...
- **Выполнено на дату записи:** Недостающие optional build packages core-win32-x64@0.4.5 и
...
- **На момент записи оставалось / не подтверждалось:** Это только cross-compiled binary, не complete Windows distribution:
...
- **Выполнено на дату записи:** Pinned Bun собрал CLI/TUI target darwin-arm64, version 0.1.4-cli.202609180110,
...
- **Выполнено на дату записи:** core-darwin-arm64@0.4.5 и fff-bin-darwin-arm64@0.9.4 добавлены только
...
- **На момент записи оставалось / не подтверждалось:** Mach-O не исполнялся на Linux. Это не полный darwin archive: native
...
- **Выполнено на дату записи:** Получены официальные Node v24.19.0 win-x64.zip и darwin-arm64.tar.gz;
...
- **На момент записи оставалось / не подтверждалось:** Это verified archive inventory, не native execution/release PASS.
...
- **Выполнено на дату записи:** Из закреплённого Playwright получены download URLs Chromium 1243 / Chrome
...
- **На момент записи оставалось / не подтверждалось:** Это resource inventory, не native execution: подписи и независимый
...
- **Выполнено на дату записи:** Добавлен script/dpapi-acceptance.ts: synthetic roundtrip, fresh ciphertext,
...
- **Выполнено на дату записи:** Host typecheck PASS; на Linux driver отклонён с WINDOWS_X64_REQUIRED
  до создания evidence. Это проверка платформенного ограничения, не DPAPI PASS.
- **На момент записи оставалось / не подтверждалось:** Native выполнение двумя Windows accounts и проверка ACL ещё требуются.


Windows profile ACL source integration (этапы 1/6):

- **Выполнено на дату записи:** До создания writer guard подключён Windows ACL adapter. Пустой root
...
- **Выполнено на дату записи:** Linux profile regression: 4 PASS / 10 assertions. Добавлены platform rejection
...
- **На момент записи оставалось / не подтверждалось:** Нативные PowerShell/NTFS проверки, два Windows accounts, race acceptance
...
- **Выполнено на дату записи:** Существующий Linux/macOS profile root отклоняется до записи guard, если
...
- **Выполнено на дату записи:** Linux: 6 profile tests PASS, 31 assertions. Проверены root 0755 без новых
...
- **На момент записи оставалось / не подтверждалось:** Нативный macOS запуск не выполнен. Проверка охватывает root и основные
...
- **Выполнено на дату записи:** Полный Host набор с pinned Node: 36 PASS / 2 native-only SKIP,
...
- **Выполнено на дату записи:** Уточнена обязательная LOGINOM_AI_AGENT_TEST_NODE в инструкциях Host;
...
- **На момент записи оставалось / не подтверждалось:** Source regression не заменяет новый installed build, реальные Windows/macOS
  и external mutation/crash acceptance. Эти gates остаются открытыми.


Durable recovery reopen без replay (этапы 2/5):

- **Выполнено на дату записи:** Host-port integration теперь повторно создаёт host из того же durable
...
- **Выполнено на дату записи:** Неполное подтверждение отклоняется с RECOVERY_CONFLICT и сохраняет журнал;
...
- **Выполнено на дату записи:** Pinned Node + package-local host-port suite: 5 PASS, 94 assertions;
...
- **На момент записи оставалось / не подтверждалось:** Это повторное создание Host с новыми реальными Node runtime processes,
...
- **Выполнено на дату записи:** Добавлен integration case kill: настоящий supervised Node runtime
...
- **Выполнено на дату записи:** Проверяется точная последовательность принятых runtime calls; она остаётся
...
- **На момент записи оставалось / не подтверждалось:** Fixture не исполняет внешний Loginom mutation. Потеря всего CLI/host во
  время внешней операции и её последующий business read-back остаются открытыми.


Живой runtime после разрыва IPC (этап 2):

- **Выполнено на дату записи:** Исправлен supervisor: событие disconnect немедленно отклоняет ожидающие
...
- **Выполнено на дату записи:** Реальный Node fixture разрывает IPC и остаётся живым через interval.
...
- **Выполнено на дату записи:** Process/transport/host-port regression: 11 PASS, 136 assertions; Host
  typecheck и diff check PASS. Native Windows исполнение ещё не проверено.


Ограниченное завершение private Node host (этап 2):

- **Выполнено на дату записи:** После close reply/разрыва IPC клиент ждёт exit не более пяти секунд,
...
- **Выполнено на дату записи:** Реальные Node fixtures: ack-without-exit, disconnect-without-exit и bad-ack,
...
- **На момент записи оставалось / не подтверждалось:** Принудительное завершение host не доказывает закрытие всех его браузеров;
...
- **Выполнено на дату записи:** Собран 0.1.4-cli.202609180038, prod, из стабильного dirty snapshot.
...
- **Выполнено на дату записи:** Настоящие install/uninstall PASS. Installed launcher с PATH без глобальных
...
- **На момент записи оставалось / не подтверждалось:** На этом кандидате не повторялись live Loginom/TUI/CSV/crash и Desktop
...
- **Выполнено на дату записи:** После реального открытия существующего тестового пакета SIGKILL CLI:
...
- **Выполнено на дату записи:** Отдельный profile, SIGINT: exit130, 24 tracked, alive=[], guard освобождён,
  status retry exit0. Evidence `/tmp/loginom-cli-owner-crash-0Yz6bX`.
- **Выполнено на дату записи:** Оба driver runs завершились PASS; stdout/stderr/retry проверены на отсутствие
  API key. [Сводка](../../testing/loginom-ai-agent/reports/2026-09-18-cli-cleanup-build/report.md).
- **На момент записи оставалось / не подтверждалось:** Сигналы отправлялись после готового prepare receipt, без mutation. Active
  external mutation crash, native Windows/macOS и общие release gates открыты.


Реальный active import + SIGKILL (этапы 2/5):

- **Выполнено на дату записи:** Driver расширен режимом active-import: исходный test CSV передаётся как
...
- **Выполнено на дату записи:** Native 00:38 PASS: crash-import running, exit137, tracked24, alive=[],
...
- **Выполнено на дату записи:** Host typecheck и diff check PASS. Test driver не меняет installed binary;
  существующие пакеты не сохранялись, guard/recovery не удалялись.
- **На момент записи оставалось / не подтверждалось:** Точная серверная фаза на момент сигнала и business outcome не доказаны;
...
- **Выполнено на дату записи:** Расширен active-import driver: SIGINT с неопределённой операцией требует
...
- **Выполнено на дату записи:** Native 00:38 полный повтор PASS, evidence /tmp/loginom-cli-owner-crash-ytSmta:
...
- **Выполнено на дату записи:** Первоначальное неверное driver expectation130 и корректный повтор отражены
...
- **Выполнено на дату записи:** Новый native run на профиле active-import SIGINT возвращает4 и единственное
  RECOVERY_REQUIRED error event, освобождает guard и сохраняет recovery hashes.
- **Выполнено на дату записи:** Отдельный прогон с HTTP counter вместо test model endpoint подтвердил
...
- **На момент записи оставалось / не подтверждалось:** Business outcome неопределённой операции и оставшиеся platform/release
  gates не считаются закрытыми этой admission-проверкой.


Windows shared-host Node path (этап 6):

- **Выполнено на дату записи:** Исправлен hardcoded bin/node в createLoginomHost: Windows выбирает
...
- **Выполнено на дату записи:** Linux host/host-port: 7 PASS, 124 assertions; Host typecheck и diff check
  PASS. Runtime resource verifier берёт node/browser paths из manifest.
- **На момент записи оставалось / не подтверждалось:** Исправление ещё не входит в исторический native 00:38 artifact и не
  подтверждает Windows execution. Полные Windows staging/installer/acceptance открыты.


Windows user installer source (этап 6):

- **Выполнено на дату записи:** Добавлен cli-install-windows.ts и подключён в общий install entrypoint.
...
- **Выполнено на дату записи:** Uninstall проверяет свои receipt/launcher/manifest и отсутствие процессов
...
- **Выполнено на дату записи:** Host typecheck, Linux installer regression и Windows platform rejection:
  2 PASS / 1 Windows-only SKIP, 38 assertions; diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Windows fixture install/uninstall, cmd argument propagation, busy-process
...
- **Выполнено на дату записи:** Выделен build-cli-installer.ts: bundled install.mjs, install/uninstall.sh
...
- **Выполнено на дату записи:** Linux build-cli использует общий helper. Оба installer target скомпилированы
...
- **На момент записи оставалось / не подтверждалось:** Windows cmd execution, полный ZIP/resource staging и нативная установка
  по-прежнему не подтверждены; генерация wrappers не считается Windows PASS.


CLI manifest: macOS directory symlinks (этап 6):

- **Выполнено на дату записи:** Inventory поддерживает ссылки на каталоги app frameworks: сохраняет
...
- **Выполнено на дату записи:** Добавлен реальный filesystem fixture Framework.framework/Versions/A и
...
- **Выполнено на дату записи:** Manifest/install regression: 5 PASS / 1 Windows-only SKIP, 57 assertions;
...
- **На момент записи оставалось / не подтверждалось:** Нативная macOS упаковка/исполнение и resource staging остаются открытыми;
  эта проверка выполнена на Linux и покрывает CLI artifact manifest.


macOS Unix installer source (этап 6):

- **Выполнено на дату записи:** Unix install/uninstall допускает darwin с теми же отдельными versioned
...
- **Выполнено на дату записи:** Генератор выпускает macOS install/uninstall.sh и INSTALL.md (распаковка
...
- **Выполнено на дату записи:** Linux installer/manifest: 4 PASS, 54 assertions; Host typecheck и diff
...
- **На момент записи оставалось / не подтверждалось:** Сам lsof на macOS, runtime install/launch/uninstall, signing/Keychain и
  полный darwin archive остаются непроверенными до доступа к нативной машине.


Native resource staging source (этап 6):

- **Выполнено на дату записи:** Общий staging выбирает Linux/Windows/macOS layout: node.exe/npm/LICENSE
...
- **Выполнено на дату записи:** Native archive inventory hashes вынесены в build-only
...
- **Выполнено на дату записи:** Копирование сохраняет symlink text; resource manifest/verifier поддерживают
...
- **Выполнено на дату записи:** Полный Linux staging /tmp/loginom-native-staging-ACeyRl/resources завершился;
...
- **На момент записи оставалось / не подтверждалось:** Windows/macOS ветки staging ещё не исполнены; полный build-cli пока
...
- **Выполнено на дату записи:** Сборщик подключает native resources/installer для linux-x64, win32-x64,
...
- **Выполнено на дату записи:** Полный Linux candidate 0.1.4-cli.202609180107 PASS: native version,
...
- **Выполнено на дату записи:** Host typecheck и diff check PASS.
- **На момент записи оставалось / не подтверждалось:** Windows ZIP/macOS tar execution, installation, signing и native acceptance
...
- **Выполнено на дату записи:** Актуализирован основной CLI checkpoint и migration checkpoint; начальная
...
- **На момент записи оставалось / не подтверждалось:** Настоящий model provider в live oracle обязателен по §13: scripted provider
...
- **На момент записи оставалось / не подтверждалось:** Остаются варианты IND-10 host/browser crash/network loss, установленный
...
- **Выполнено на дату записи:** Зафиксирован FAIL артефакта 01:07: после SIGKILL host CLI не завершался
...
- **Выполнено на дату записи:** Исправлен верхний entrypoint: flush/exit выполняются также после ошибки
...
- **Выполнено на дату записи:** В crash driver добавлен явный deadlineExceeded: спасательный timeout
  всегда означает FAIL, независимо от итогового кода.
- **На момент записи оставалось / не подтверждалось:** Повторный live host crash новым native executable и полная упаковка:
  см. reports/2026-09-18-cli-host-crash/report.md; исходный IND-10 пока не закрыт.

- **Выполнено на дату записи:** Повторный live host crash кандидатом `0.1.4-cli.202609180130` PASS:
...
- **Выполнено на дату записи:** Полный standalone-status suite: 3 PASS, 89 assertions (45.08 s).
- **На момент записи оставалось / не подтверждалось:** Полная упаковка и installed acceptance исправленного entrypoint,
...
- **Выполнено на дату записи:** Добавлен режим browser в ручной crash driver: выбор ровно одного главного
...
- **Выполнено на дату записи:** Package-local typecheck loginom-host PASS. Подробности и ограничения:
  reports/2026-09-18-cli-browser-crash/report.md.
- **На момент записи оставалось / не подтверждалось:** Live результат browser crash и оставшиеся IND-10 gates пока не закрыты.

- **Выполнено на дату записи:** Live browser crash кандидатом 01:30 с development bundle 01:07 PASS:
...
- **На момент записи оставалось / не подтверждалось:** IND-10 остаётся частичным: live runtime crash/network loss и установленный
...
- **Выполнено на дату записи:** Ручной драйвер получил режим runtime: PID родителя Chromium активного
...
- **На момент записи оставалось / не подтверждалось:** Live результат записывается в reports/2026-09-18-cli-runtime-crash/report.md;
  installed acceptance, network loss и остальные gates остаются открыты.

- **Выполнено на дату записи:** Live runtime crash кандидатом 01:30 с development bundle 01:07 PASS:
...
- **На момент записи оставалось / не подтверждалось:** Полный IND-10 всё ещё не закрыт: network loss после dispatch и проверка
  полного установленного артефакта остаются обязательными.

Полная Linux упаковка исправленного CLI (2026-09-18):

- **Выполнено на дату записи:** Артефакт 0.1.4-cli.202609180135 собран; native smoke, source stability,
...
- **Выполнено на дату записи:** Реальный install.sh установил пользовательский payload/launcher; --help
  и --version с ограниченным PATH без внешних runtime PASS.
- **На момент записи оставалось / не подтверждалось:** Installed host-crash acceptance выполняется без development bundle;
...
- **Выполнено на дату записи:** Installed host crash 01:35 PASS без bundle override: code 1 без timeout,
...
- **Выполнено на дату записи:** Реальный uninstall PASS: launcher/payload отсутствуют, все 3965 хешей
  файлов профиля совпали до/после удаления. Guard/recovery не удалялись.
- **Выполнено на дату записи:** Архив сохранён на диск в /tmp/loginom-ai-agent-cli-0.1.4-cli.202609180135-linux-x64.tar.gz;
  повторная проверка SHA256 совпала. Полный план остаётся открытым.

IND-10 network loss, изолированный стенд (2026-09-18):

- **Выполнено на дату записи:** Добавлен TCP relay для HTTP Loginom только временного тестового профиля:
  разрыв established connections и отказ reconnect без изменения системной сети.
- **Выполнено на дату записи:** Real-socket тест PASS (4 assertions), upstream остаётся доступным напрямую;
  typecheck host PASS. Live запуск полного артефакта 01:35 начат без bundle override.
- **На момент записи оставалось / не подтверждалось:** Результат и границы проверки фиксируются в
  docs/testing/loginom-ai-agent/reports/2026-09-18-cli-network-loss/report.md.

- **Выполнено на дату записи:** Диагностировано ограничение network-loss стенда: два запуска остановились
...
- **На момент записи оставалось / не подтверждалось:** Следующий шаг: определить ограничение browser login/origin либо применить
  изолированный разрыв с сохранением исходного origin. Gate остаётся открытым.

- **Выполнено на дату записи:** Network стенд: контрольная авторизация по исходному адресу PASS; через
...
- **На момент записи оставалось / не подтверждалось:** Продолжить диагностику WebSocket/origin; реальный разрыв после dispatch
  ещё не выполнен и не засчитывается в IND-10.

Исправление авторизации без пароля, выявленное network стендом (2026-09-18):

- **Выполнено на дату записи:** Найдена гонка: Loginom делает пустое поле пароля readonly после выбора
...
- **Выполнено на дату записи:** loginPage пропускает fill только при пустом supplied password и уже пустом
...
- **Выполнено на дату записи:** Pinned Node tests: 4 PASS. Live Chromium с исправленным loginPage source
  авторизовался через исходный relay (16 соединений, без отказов). Переписывание
  Host/Origin не требуется для этого случая.
- **На момент записи оставалось / не подтверждалось:** Пересобрать runtime в полном артефакте и продолжить network loss после
  dispatch. Source login PASS не заменяет проверку полного сценария.

- **Выполнено на дату записи:** Полный артефакт 0.1.4-cli.202609180210 с passwordless fix собран:
...
- **Выполнено на дату записи:** Network driver теперь требует фактически оборванные активные сокеты;
  real-socket тест 5 assertions PASS, host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Live network-loss запуск нового полного артефакта выполняется без bundle
  override; итог и ограничения будут записаны в network-loss отчёте.

- **Выполнено на дату записи:** Новый live запуск прошёл passwordless setup и подготовку Loginom, но
...
- **На момент записи оставалось / не подтверждалось:** Исследовать проверку байтов доставки через relay; network loss остаётся
  непроверенным, исходный критерий не ослабляется.

- **Выполнено на дату записи:** Уточнена причина AMBIGUOUS в журнале: DOWNLOAD_EVENT_MISSING после
...
- **Выполнено на дату записи:** Добавлен вариант сетевого стенда на собственном IPv4 с отказом входящих
...
- **На момент записи оставалось / не подтверждалось:** Новый отдельный live профиль проверяет этот вариант; результат ожидается.

- **Выполнено на дату записи:** Live network loss полного артефакта 02:10 PASS для ограниченного случая:
...
- **Выполнено на дату записи:** Recovery сохранён; acknowledgement/явный replay не отправлялись. Точная
...
- **Выполнено на дату записи:** Финальные socket tests: 2 PASS, 6 assertions; host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Этот результат не заменяет installed acceptance версии 02:10, настоящий
  model provider и оставшиеся native/release/desktop gates исходного плана.

- **Выполнено на дату записи:** Версия 02:10 установлена реальным install.sh. Для места удалён только
  промежуточный resource staging ACeyRl этой задачи; архивы/профили сохранены.
- **На момент записи оставалось / не подтверждалось:** Installed network-loss acceptance запущен через ~/.local/bin launcher
  без bundle override, с restricted PATH. Результат будет в network-loss отчёте.

- **Выполнено на дату записи:** Installed network loss версии 02:10 PASS: реальный launcher/restricted PATH,
...
- **Выполнено на дату записи:** Реальный uninstall PASS: launcher/payload отсутствуют, 4113 хешей файлов
...
- **Выполнено на дату записи:** Desktop packaging/static-verifier tests: 5 PASS, 47 assertions.
  Полный Desktop build с текущим host/runtime и passwordless fix PASS.
- **Выполнено на дату записи:** AppImage кандидата 0.1.4-cli.202609180220 создан. Первый DEB build
  остановился на ENOSPC во временном FPM-каталоге; ничего не устанавливалось.
- **На момент записи оставалось / не подтверждалось:** DEB повторно упаковывается из уже собранного payload с tmpfs TMPDIR.
...
- **Выполнено на дату записи:** AppImage распакован; все 4365 runtime entries сверены с manifest исходного
  payload, включая архитектуру ELF. Такая же проверка linux-unpacked PASS.
- **На момент записи оставалось / не подтверждалось:** DEB retry ещё выполняется (command session 65381, активный xz); перед
...
- **Выполнено на дату записи:** DEB retry завершён, версия 0.1.4~cli.202609180220/amd64. Распакованные
  4365 ресурсов и ASAR совпали с проверенным payload; SHA256 обоих артефактов
  записаны в Desktop regression report.
- **Выполнено на дату записи:** Пакет возврата 0.1.4 сверён с установленным ASAR до установки кандидата.
- **На момент записи оставалось / не подтверждалось:** Установка кандидата выполняется; затем изолированная приёмка и возврат
  исходной версии 0.1.4. Пользовательский процесс не завершается.

- **Выполнено на дату записи:** Installed Desktop 02:20 выявил FAIL: root-owned ASAR/desktop entry получили
...
- **Выполнено на дату записи:** Исправлена Linux упаковка: публичные права payload, umask 022 для FPM metadata,
...
- **Выполнено на дату записи:** 6 tests/53 assertions и Desktop typecheck PASS; новый unpacked payload прошёл
  permissions check. Проверен запуск config из umask 077.
- **На момент записи оставалось / не подтверждалось:** Кандидат 02:25 пакуется (session 60769); требуется installed GUI/runtime
  acceptance и последующий возврат стабильной версии. IND-14 пока не закрыт.

- **Выполнено на дату записи:** Новый AppImage 02:25 распакован; public permissions и все 4365 runtime
  entries PASS. DEB проверяется отдельно после завершения сжатия, исходная
  установленная версия по-прежнему 0.1.4.

- **Выполнено на дату записи:** DEB 02:25 завершён; public permissions, 4365 runtime entries и ASAR PASS.
...
- **Выполнено на дату записи:** Установленные public permissions и все 4365 runtime entries проверены.
- **На момент записи оставалось / не подтверждалось:** Полный installed Desktop CSV oracle выполняется (session 8442, evidence
...
- **Выполнено на дату записи:** Installed Desktop 02:25: CSV A import/group/save PASS (35 + 20 = 55),
  независимый cold reopen/readback PASS (55, settingsReapplied=false).
- **На момент записи оставалось / не подтверждалось:** CSV B и завершение обёртки возврата 0.1.4 ещё ожидаются; session 8442
  продолжает работу, повторный запуск не нужен.

- **Выполнено на дату записи:** Installed Desktop 02:25: CSV B import/group/save PASS (100 + 1 = 101),
...
- **Выполнено на дату записи:** Обёртка завершилась: oracleCode=0, restoreCode=0, restoredAsarMatches=true.
...
- **На момент записи оставалось / не подтверждалось:** Остались настоящий model provider, native OS и release/signing gates;
  scripted provider и unsigned Linux developer packages их не заменяют.

Native proxy adapters, source checkpoint (2026-09-18):

- **Выполнено на дату записи:** Добавлены read-only Windows CLI collector через WinHTTP CurrentUser API
...
- **Выполнено на дату записи:** Manual HTTP/HTTPS, IPv6, domain/loopback bypass и precedence разбираются;
...
- **Выполнено на дату записи:** Известная ошибка policy до host startup возвращает exit 2 и освобождает guard;
  повторный запуск реального CLI не получает ложный PROFILE_BUSY.
- **Выполнено на дату записи:** Native parser tests 5/34, CLI proxy 1/8, standalone integration 3/89,
  Linux/Desktop proxy 5/19 — PASS; host/agent/Desktop typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Native collectors и routing на реальных Windows/macOS не исполнены,
...
- **Выполнено на дату записи:** Новый полный CLI 02:40 собран: native smoke/source stability/manifest/
  extracted archive PASS. Архив сохранён в /tmp; SHA256
  8bb96171d31ffbe92434595cd03046992afdb9a9bbfe8f81cafada2d3ad7f45d.
- **Выполнено на дату записи:** Installed launcher/restricted PATH: help/version без создания профиля,
...
- **Выполнено на дату записи:** Uninstall PASS: launcher удалён, все 3676 хешей файлов профиля совпали.
...
- **Выполнено на дату записи:** Для места удалена только распакованная копия CLI 01:07 после проверки
...
- **Выполнено на дату записи:** Native standalone и private Node host сохраняют Bun build-inputs.json;
  отсутствие metadata завершает сборку ошибкой. Старые артефакты не изменялись.
- **Выполнено на дату записи:** Реальный Linux compile/version smoke и host build PASS; typecheck обоих
  пакетов PASS. CLI: 3793 emitted inputs / 467 npm package versions; host: 100 / 3.
- **Выполнено на дату записи:** Инвентарь package metadata и root notice filenames сохранён в
  docs/testing/loginom-ai-agent/reports/2026-09-18-cli-license-inputs/.
- **На момент записи оставалось / не подтверждалось:** У 26 CLI package versions root license file не найден; требуется проверка
...
- **Выполнено на дату записи:** Build-cli подключает collect-build-notices: emitted CLI/host npm inputs,
...
- **Выполнено на дату записи:** На реальном графе скопированы 442 текста для 467 package versions;
  независимая сверка множества пакетов и всех SHA256 PASS. Host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Новый полный архив с notices ещё проверяется; 26 missing package texts,
  вложенные notices и Bun/native/resources audit остаются открытыми.

- **Выполнено на дату записи:** Полный CLI 03:20 с 442 notice texts собран: native smoke/source stability/
...
- **Выполнено на дату записи:** Исправлена обнаруженная полной сборкой база относительных host metafile
  paths (caller cwd); первая попытка завершилась до публикации, повтор PASS.
- **На момент записи оставалось / не подтверждалось:** Этот candidate не устанавливался; notices audit остаётся incomplete
  (26 missing + nested/Bun/native exclusions). Подробности в отчёте license-inputs.

Pinned upstream notices (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Для четырёх package versions сохранены upstream license texts по gitHead
...
- **Выполнено на дату записи:** Collector на реальном графе: 467 пакетов / 446 файлов / 22 missing.
  Независимая проверка всех SHA256 и четырёх gitHead mappings PASS; typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Новый полный архив после supplements не собирался; 03:20 остаётся прежним.
...
- **Выполнено на дату записи:** Для 12 версий provider-utils проверены exact version tags → commit и
  package.json name/version; upstream copyright/license notices сохранены с SHA256.
- **Выполнено на дату записи:** Collector сохраняет README отдельно от license files, не скрывая missing.
  На реальном графе: 458 notices + 464 README; все 922 хеша PASS, typecheck PASS.
- **Выполнено на дату записи:** Drizzle tag с несовпадающей package version не принят; test fixture license
  npm/agent не подставлена вместо собственной. Источники/пробелы записаны в отчёте.
- **На момент записи оставалось / не подтверждалось:** Остались 10 missing package entries, полные standard license texts,
  Bun/native/nested audit и новый полный архив. License gate ещё открыт.

Bun notices (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Закреплён upstream LICENSE.md Bun 1.3.14 по commit
  0d9b296af33f2b851fcbf4df3e9ec89751734ba4; Bun.revision локального toolchain совпал.
- **Выполнено на дату записи:** Build-cli проверяет версию/revision и SHA256, включает LICENSE.md и source.json
  со статусом incomplete. Host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Native linked-library/polyfill тексты, corresponding source/relinking audit
...
- **Выполнено на дату записи:** Полный candidate 03:50 включает 458 npm notices, 464 README и pinned Bun
...
- **Выполнено на дату записи:** Архив сохранён в /tmp; SHA256
  b20eeb8f0a509253d9f3046459e86a35047f7ab0d15d246b07738941caca61a5.
- **На момент записи оставалось / не подтверждалось:** Candidate 03:50 не устанавливался. License gate остаётся incomplete:
...
- **Выполнено на дату записи:** По build scripts точного Bun commit закреплены 21 dependency source pin;
...
- **Выполнено на дату записи:** Build-cli включает native notices и проверяет каждый hash; все 29 локальных
  SHA256 независимо сверены. Host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Это default source inventory для разных платформ, не exact binary linkage.
...
- **Выполнено на дату записи:** Тот же libwebp commit найден в официальном Chromium gitiles; COPYING,
  AUTHORS/PATENTS добавлены с SHA256. Native collection: 32 files / 21 components.
- **Выполнено на дату записи:** Из фактического pinned Linux Chromium получен chrome://credits (757 секций),
  headless с включённой Chromium sandbox; тестовый browser штатно закрыт.
- **Выполнено на дату записи:** Проверены binary hash и gzip/plain credits hashes. Linux build-cli экспортирует
  readable credits с проверкой pin/hash; shared notices wording исправлен.
  Host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Полный архив после этих additions не собран; другие платформы, nested/native
  и corresponding-source/relinking compliance по-прежнему не подтверждены.

Полный notices archive 04:10 (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Native smoke/source stability/manifest/extracted archive PASS. Отдельно
  проверены все 32 native hashes и Chromium credits; архивные файлы совпали.
- **Выполнено на дату записи:** Архив сохранён в /tmp с независимой SHA256 проверкой:
  e275ca545348d5316e34c1669efbe1da756b56c7150cc50ae3a769aee4cb831e.
- **Выполнено на дату записи:** В начало раздела 16 добавлена актуальная сводка этапов и ограничений;
  исторические evidence сохранены, старые PASS не перенесены на этот artifact.
- **На момент записи оставалось / не подтверждалось:** Candidate 04:10 не устанавливался; license/release/native/provider gates открыты.

Регрессия сборщика notices (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Реальный Bun.build fixture проверяет scoped root против nested package.json,
  точное копирование текста и сохранение missing при README с названием лицензии.
- **Выполнено на дату записи:** Symlink LICENSE за пределы package root отклоняется; успешный inventory
  не записан, внешний файл не изменён. Проверены реальные файловые операции.
- **Выполнено на дату записи:** Package-local test 1/12 assertions и host typecheck PASS. Windows symlink
  fixture не запускался; общие release/acceptance gates остаются открытыми.

Drizzle source provenance (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Опубликованный npm provenance указывает build commit Drizzle; registry
...
- **Выполнено на дату записи:** Source collector: 459 notices + 464 README / 9 missing; все 923 hashes PASS,
  host typecheck PASS. Неподтверждённые commit для AWS не подставлялись.
- **На момент записи оставалось / не подтверждалось:** Новый полный архив после добавления Drizzle не собирался; остальные audit
  gaps и native/provider/signing gates остаются открытыми.

Embedded Bun source notices (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Из точного Bun commit сохранены целиком assert.ts/events.ts/url.ts с полными
...
- **На момент записи оставалось / не подтверждалось:** Полнота остальных polyfills и nested sources не утверждается. Новый архив
...
- **Выполнено на дату записи:** Штатные install.sh/uninstall.sh в обычном home: launcher в versioned payload,
  restricted PATH help/version PASS, выбранный профиль не создан этим запуском.
- **Выполнено на дату записи:** Проверены installed hashes 32 native files и Chromium credits (757 секций).
...
- **Выполнено на дату записи:** Для места удалены только распакованные 00:20/00:38 после manifest/archive
  SHA256/matching manifest и /proc проверки; архивы, профили и journals сохранены.
- **На момент записи оставалось / не подтверждалось:** Drizzle/3 JS source additions ещё требуют нового архива; оставшиеся
...
- **Выполнено на дату записи:** Drizzle/3 JS additions вошли в новый полный архив. Native smoke, source
  stability, manifest/extracted archive PASS.
- **Выполнено на дату записи:** Штатные install/uninstall PASS. Restricted PATH help/version без профиля;
...
- **Выполнено на дату записи:** Архив сохранён в /tmp, SHA256 независимо проверен:
...
- **На момент записи оставалось / не подтверждалось:** 9 npm collection gaps, прочие license/source/relinking и native/provider/
...
- **Выполнено на дату записи:** Реальный install.sh + installed launcher без bundle override; отдельные
  профили, original CSV, verified upload/running import/durable recovery до SIGKILL.
- **Выполнено на дату записи:** Browser crash: code 4, deadline=false, 24 PID завершены, guard снят,
  status recoverable-error; recovery сохранён (OQYQvV).
- **Выполнено на дату записи:** Runtime crash: code 1, deadline=false, 24 PID завершены, guard сохранён,
  status code 3/PROFILE_BUSY; recovery сохранён (gj6jFM). Acknowledge/replay не делались.
- **Выполнено на дату записи:** Uninstall exit 0, launcher/payload отсутствуют; профили/journals сохранены.
...
- **На момент записи оставалось / не подтверждалось:** Provider scripted; остальные timing/native OS/real provider/release gates
  этим не закрыты. Полная цель остаётся активной.

Author-linked MIT supplement (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Для abstract-logging сохранён derived текст из MIT template и явного
...
- **Выполнено на дату записи:** Год/авторство не выдуманы. Collector: 460 notices + 464 README / 8 missing;
  все 924 output hashes и host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Архив 04:30 сохраняет прежний состав; source addition ещё не пересобран.
  Полный license/native/provider/release audit остаётся открытым.

Declared standard license texts (этап 5, 2026-09-18):

- **Выполнено на дату записи:** Для SPDX-пакетов сохранены verbatim README + полные заявленные CC0/CC-BY
  тексты по immutable SPDX source; attribution/URLs/hashes/преобразование записаны.
- **Выполнено на дату записи:** Для трёх AWS packages tarball SHA512 совпал с registry, package.json/README
...
- **Выполнено на дату записи:** Collector: 465 notices + 464 README, все 929 hashes и host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Остались 3 missing text entries (npmcli/agent, poe-auth, opentui-spinner),
...
- **Выполнено на дату записи:** Все текущие supplements вошли в архив. Build/native smoke/source stability/
  manifest/extracted archive PASS; штатные install/uninstall PASS.
- **Выполнено на дату записи:** Installed hashes 465 notices, 464 README, 35 native/JS файлов и Chromium
  credits проверены; help/version без создания профиля, profile fixture сохранён.
- **Выполнено на дату записи:** Архив сохранён в /tmp и SHA256 проверен:
...
- **На момент записи оставалось / не подтверждалось:** Остались 3 text gaps и общий compliance/release audit. В стандартных CLI
...
- **Выполнено на дату записи:** Ошибки Dock распознаются в первом receipt/structuredContent, включая
...
- **Выполнено на дату записи:** Ctrl+C для providers/auth/models передаётся в Effect и HTTP-запрос metadata.
...
- **Выполнено на дату записи:** Текстовые вложения file/data URL сохраняют полный исходный snapshot для
...
- **Выполнено на дату записи:** Ошибки run `--fork` без `--continue`/`--session` и отсутствующего сообщения
...
- **Выполнено на дату записи:** Новый полный Linux archive из чистого `1657a6c07`, version
  `0.1.4-cli.20260918review`: build, native version, source stability,
  manifest и extracted archive verification PASS.
- **На момент записи оставалось / не подтверждалось:** Установленная сборка с настоящим Xiaomi provider: CSV 55/101,
  save/cold reopen и независимый readback; затем сверка остальных Linux gates.
- **На момент записи оставалось / не подтверждалось:** Оставшиеся Linux notices/source-distribution gaps и доступные проверки
...
- **Выполнено на дату записи:** Installed Xiaomi MiMo вызвал реальные Loginom tools, доставил исходный CSV
...
- **Выполнено на дату записи:** Честно сохранён исход первого широкого запроса: ошибка схемы и неоднозначный
...
- **Выполнено на дату записи:** Дополнительный живой прогон остановлен SIGINT: exit 130, guard освобождён;
  тестовые runtime/browser процессы завершились. Desktop не изменён.
- **На момент записи оставалось / не подтверждалось:** Следующий этап: оставшиеся Linux packaging/notices/source gaps и итоговая
...
- **Выполнено на дату записи:** Для npmcli/agent, poe-auth и opentui-spinner сверены registry tarball integrity
...
- **Выполнено на дату записи:** Добавлены неизменённые SPDX ISC/MIT тексты с metadata/README, hashes и явным
  attributionReview. Авторство/год не выдумывались; placeholders сохранены.
- **Выполнено на дату записи:** На фактическом build graph: 467 package records, 468 notices + 464 README,
  все 932 hashes PASS; missing-text list пуст. Collector test 1/12 и host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Новый полный архив с этими supplements и установленная сверка inventory.
...
- **Выполнено на дату записи:** Полный clean archive `0.1.4-cli.20260918linux` из `edc68138d` включает
...
- **Выполнено на дату записи:** Штатная установка; help/version без создания profile; status через
  bundled Node host; guard освобождён. Все 932 installed npm notice/README hashes PASS.
- **Выполнено на дату записи:** Штатное удаление сохранило побайтово 8093 файла трёх тестовых профилей и
  проверенные Desktop credential files; launcher/payload удалены.
- **Выполнено на дату записи:** Offline non-root Ubuntu 22.04 и Debian 12 containers: CLI help/version,
...
- **На момент записи оставалось / не подтверждалось:** Итоговая сверка требований Linux с точным разделением текущих и исторических
...
- **Выполнено на дату записи:** Native providers list: strace file-only, 412 syscall lines, 0 обращений к
  проверяемым Desktop roots; exit 0/guard=false. Scope — именно эта команда.
- **Выполнено на дату записи:** Общий permission service, guards/aliases и standalone proxy policy:
...
- **Выполнено на дату записи:** Node из final archive: HTTP proxy, HTTPS CONNECT + extra CA, node:http,
  loopback bypass/no-direct-fallback PASS на реальных локальных сокетах.
- **Выполнено на дату записи:** Точный project source archive из binary commit edc68138d создан, SHA256
  записан в final report; это не утверждение о полном third-party relinking audit.
- **На момент записи оставалось / не подтверждалось:** Final archive CSV 55/101/cold readback запущен штатным deterministic harness
...
- **Выполнено на дату записи:** `0.1.4-cli.20260918linux` headless run: A=55 и B=101, реальные import/group/save,
...
- **Выполнено на дату записи:** Source archive: 11903 entries совпали с git tree точного binary commit,
  missing/extra=0.
- **На момент записи оставалось / не подтверждалось:** Установленный TUI final artifact с видимым Chromium проверяется тем же
...
- **Выполнено на дату записи:** Разобран отказ первого final TUI: A cold readback 55 PASS; B сохранён с 101,
  но close отклонён до dispatch (`UI_EPOCH_CHANGED`, effect_possible=false).
  Полный PASS этому прогону не присвоен.
- **Выполнено на дату записи:** Драйвер принимает только доказанный pre-dispatch refusal, обновляет observation,
...
- **На момент записи оставалось / не подтверждалось:** Новый installed headed TUI oracle: `/tmp/loginom-linux-oracle-uwsU99`,
...
- **Выполнено на дату записи:** Новый прогон `/tmp/loginom-linux-oracle-uwsU99`: A=55/B=101,
  save/close и независимый cold readback, settingsReapplied=false; driver exit 0.
- **Выполнено на дату записи:** Оба CLI процесса: exit 0, guard=false, PTY forced=false; по одному видимому
  окну Chromium, remaining=[] после закрытия. Прежний failed run сохранён.
- **Выполнено на дату записи:** Итог записан в final Linux report и `tui-oracle-summary.json`.
- **На момент записи оставалось / не подтверждалось:** Отдельный Desktop dev-кандидат с теми же исходниками продукта собран:
...
- **Выполнено на дату записи:** Отдельный Desktop dev-кандидат: A=55/B=101, save/close, независимый cold
...
- **Выполнено на дату записи:** Шесть Desktop/TUI/run captures: 34 tool schemas/descriptions и bootstrap
  совпали; шесть prepare replies содержат одинаковые full instructions/knowledge.
- **Выполнено на дату записи:** Новый повторяемый `desktop-cli-independence.ts`: оба порядка закрытия,
...
- **Выполнено на дату записи:** CLI setup/run/prepare/observe/close file trace: 959201 syscall lines,
  0 references к четырём Desktop global roots. Полный TUI trace не заявляется.
- **Выполнено на дату записи:** Тестовые процессы завершены; штатный uninstall удалил CLI launcher/payload,
...
- **Выполнено на дату записи:** Сборщик сохраняет вложенные LICENSE/NOTICE/COPYING/COPYRIGHT с относительными
...
- **Выполнено на дату записи:** Реальный build graph edc68138d: 467 packages, 483 notices (15 nested),
  464 README; проверены все 947 hashes, missing own-package texts=0.
- **Выполнено на дату записи:** Actual-build fixture test: 1 PASS/16 assertions; host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Новый полный архив ещё не собран; attribution/native/source/relinking audit
  не объявляется завершённым. Старый final artifact сохранён без изменений.

### Два чата в одном TUI и файловая изоляция — PASS

- **Выполнено на дату записи:** Один native TUI/profile: первый sales.csv → /new → другой sales.csv.
...
- **Выполнено на дату записи:** Exit 0, forced=false, guard=false. Предыдущие два неудачных перехода /new
  сохранены отдельно; повторной доставки старых операций не было.
- **Выполнено на дату записи:** Драйвер ждёт onFinish и наблюдаемое меню New session перед отдельным Enter;
  host typecheck PASS. Новая проверка не подменяет permission acceptance.
- **Выполнено на дату записи:** File trace всех трёх попыток: 1362347 syscall lines, 0 references к четырём
...
- **Выполнено на дату записи:** Чистая сборка `0.1.4-cli.20260918nested` из c6eceef90: native version,
  source stability, manifest и extracted archive PASS.
- **Выполнено на дату записи:** Установлены и проверены 947 notice/README hashes, включая 15 вложенных
  notices. Help/version без global runtime в PATH; bundled host status PASS.
- **Выполнено на дату записи:** Uninstall удалил launcher/payload, сохранил созданный профиль.
- **Выполнено на дату записи:** Source archive точно соответствует 11918 git-tree files; hashes и пути
  записаны в отчёте. Это project source, не полный third-party source bundle.
- **На момент записи оставалось / не подтверждалось:** Attribution/native/per-file/source/relinking audit остаётся incomplete;
...
- **Выполнено на дату записи:** Получен официальный source archive точного Bun commit; LICENSE.md совпал
  с закреплённым hash. Полный src/js сохранён: 188 файлов и одна внутренняя ссылка.
- **Выполнено на дату записи:** Все 189 entries/hash сверены с исходным архивом; 37 native/source notice
  input hashes и host typecheck PASS. В inventory записаны origin/hash/transform.
- **Выполнено на дату записи:** Build включает компактный src-js.tar.gz и inventory; README описывает его
  и вложенные npm notices. Прежние individual JS supplements сохранены.
- **На момент записи оставалось / не подтверждалось:** Новый архив с src/js ещё не собран. Native dependency source closure и
  relinking остаются отдельными неподтверждёнными требованиями; это не release PASS.

### Bun: SQLite и Node compatibility sources

- **Выполнено на дату записи:** SQLite: сохранены все 6 файлов amalgamation/bindings; версия 3.53.0 и
  sqlite_source_id совпали между точным source archive и закреплённым Bun runtime.
- **Выполнено на дату записи:** Node headers 24.3.0 и source archive проверены по официальным SHA256.
  Взяты 111 файлов с штатными exclusions openssl/uv и точным LICENSE.
- **Выполнено на дату записи:** Все 23 default dependency recipes Bun имеют component records;
  проверены 41 native/source input hashes и host typecheck.
- **На момент записи оставалось / не подтверждалось:** Это source recipe coverage, не binary linker map. WebKit per-file и
...
- **Выполнено на дату записи:** Чистый `0.1.4-cli.20260918sources` из d391443be: native version,
  source stability, manifest и extracted archive PASS.
- **Выполнено на дату записи:** Установленная проверка: 41 native/source hashes, 306 entries внутри
  source archives, 947 npm notice/README hashes совпали. Help/version/status PASS.
- **Выполнено на дату записи:** Uninstall сохранил профиль и удалил launcher/payload; source archive точно
  соответствует 11926 git-tree entries. Runtime-код продукта не изменялся.
- **На момент записи оставалось / не подтверждалось:** Конкретный остаток: Bun native src за пределами JS/SQLite (527 файлов с
...
- **Выполнено на дату записи:** Добавлены все 2899 оставшихся src entries вне JS/SQLite, без отбора только
  по copyright markers. Содержимое и внутренняя ссылка сохранены без изменений.
- **Выполнено на дату записи:** Объединение трёх архивов точно совпало со всеми 3094 src entries исходного
  Bun commit: missing/extra/duplicates=0. Проверены 43 input hashes и typecheck.
- **Выполнено на дату записи:** README описывает полное покрытие src этими тремя архивами.
- **На момент записи оставалось / не подтверждалось:** External dependency sources, WebKit и relinking остаются открыты;
  archive rebuild с этим дополнением ещё не выполнялся.

### ICU и фактические runtime revisions

- **Выполнено на дату записи:** Добавлены неизменённые LICENSE/license.html ICU 75.1 из официального
  source release; версия совпала с Bun, LICENSE — с закреплённым release tag.
- **Выполнено на дату записи:** Двенадцать native commit IDs из process.versions совпали с source records;
  host typecheck PASS. Compatibility versions не объявляются linker map.
- **Выполнено на дату записи:** Полный WebKit checkout получен через Git; commit/tree совпали с API.
  Отказ codeload не изменил выбранную версию.
- **На момент записи оставалось / не подтверждалось:** Отдельный полный WebKit source archive генерируется; его целостность,
  source-distribution integration и relinking ещё не заявляются проверенными.

### TinyCC source и полный WebKit archive

- **Выполнено на дату записи:** Добавлен полный неизменённый TinyCC source для фактического Bun revision;
  проверены 523 файла, все 47 native/source input hashes и host typecheck.
- **Выполнено на дату записи:** Полный WebKit archive проверен против 464897 Git blob entries, включая
...
- **На момент записи оставалось / не подтверждалось:** WebKit archive пока хранится отдельно в /tmp: включение в source distribution,
...
- **Выполнено на дату записи:** Добавлен сборщик source companion: закреплённые commit/tree/hash/size,
  потоковая проверка исходника и копии, запрет замены существующего каталога.
- **Выполнено на дату записи:** Реальный комплект `/tmp/loginom-cli-webkit-source-companion` собран;
  независимый sha256sum, отклонение неверного размера/hash и сохранение
  существующего каталога PASS. Host typecheck PASS.
- **Выполнено на дату записи:** CLI build включает WebKit source metadata и ссылку на companion в notices.
- **На момент записи оставалось / не подтверждалось:** Новый Linux binary archive с полным набором последних source additions
...
- **Выполнено на дату записи:** Чистый `0.1.4-cli.20260918native-src` из `1feb99775` собран;
  source stability, native version smoke и extracted manifest PASS.
- **Выполнено на дату записи:** Все 47 доставленных native/source hashes совпали; WebKit companion pin
  совпал. Help/version: exit 0, профиль не создаётся.
- **На момент записи оставалось / не подтверждалось:** Corresponding-source build recipes/patches и external dependencies,
...
- **Выполнено на дату записи:** Собран отдельный комплект из 24 закреплённых архивов: полный Bun с 53
  build modules/15 patch files, WebKit, ICU, Node headers и 20 native dependencies.
- **Выполнено на дату записи:** Все 27 собранных external notices совпали с файлами полных архивов;
  source/copy SHA256 и независимый sha256sum для 24 архивов PASS.
- **Выполнено на дату записи:** Добавлен воспроизводимый сборщик комплекта с отказом при missing/corrupt
  input и запретом замены каталога; missing-input check и host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Транзитивные Rust crates, toolchain prerequisites и фактическая
...
- **Выполнено на дату записи:** Получены все 43 внешних crate archives из lol-html c-api/Cargo.lock;
  SHA256 совпали с lock. Два других пакета уже входят в lol-html source.
- **Выполнено на дату записи:** Добавлен детерминированный crate bundle и URL/hash inventory;
  новый companion из 25 архивов собран, входные/доставленные hashes проверены.
- **Выполнено на дату записи:** Независимая проверка вложенных 43 crate hashes и оригинального lock PASS;
  host typecheck PASS.
- **На момент записи оставалось / не подтверждалось:** Rust std build dependencies, toolchain и actual rebuild/relink остаются
...
- **Выполнено на дату записи:** Собран Docker image с LLVM 21.1.8 и закреплённым Rust nightly-2025-12-10
  с rust-src. Версии проверены; пакеты хоста/Desktop не изменены.
- **Выполнено на дату записи:** Bun release-local configure без сети PASS: 22 dependencies/1130 objects.
- **Выполнено на дату записи:** TinyCC по настоящему Bun recipe собран без сети из сохранённого source,
  с upstream patch и десятью object files; exit 0.
- **Выполнено на дату записи:** Ограничения native rebuild: non-root, 2 CPU, 12 GiB, без дополнительного swap.
- **На момент записи оставалось / не подтверждалось:** WebKit CMake configure/generate PASS, компиляция запущена и ещё выполняется;
...
- **Выполнено на дату записи:** Этапы 1–5 технической Linux-реализации и приёмки завершены в описанном
  scope; матрица IND-01…14 проверена по текущим и явно версионированным отчётам.
- **Выполнено на дату записи:** Реальный Xiaomi smoke принят пользователем; повторный model-generated
  oracle не требуется. Runtime diff от проверенного edc68138d отсутствует.
- **Выполнено на дату записи:** Юридический third-party/source/relinking audit исключён пользователем
  из текущей цели и отложен отдельно; уже собранные материалы сохранены.
- **Выполнено на дату записи:** WebKit build остановлен по запросу (exit 137, не PASS); Cargo vendor завершился
  до остановки (exit 0). Контейнеров аудита не осталось. Toolchain/логи сохранены.
- **Выполнено на дату записи:** Последний checkpoint source companion: 27 архивов, включая rust-src и

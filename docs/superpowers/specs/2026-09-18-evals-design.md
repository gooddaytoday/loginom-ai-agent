...
- Коды выхода: 0 — сессия дошла до idle без неустранённых ошибок; 1 — runtime/provider/tool/permission (в том числе `CLI_PERMISSION_REJECTED`, `CLI_TOOL_FAILED`, ошибки провайдера); 2 — аргументы или отсутствующая настройка; 3 — `PROFILE_BUSY` и конфликты состояния (только stderr); 4 — неопределённая операция Loginom (`LOGINOM_RECOVERY_REQUIRED`, `LOGINOM_CALL_UNCERTAIN`) только при `LOGINOM_AI_AGENT_STRICT_RECOVERY=1`; 130 — отмена. Дизайн CLI прямо фиксирует: код 0 не доказывает правильность сценария. По умолчанию журнал мягкий: неподтверждённые записи не блокируют следующий `run` и не дают код 4. Ошибки инструментов `loginom_*` тоже не дают код 1; попытка без `.lgp` остаётся `no_artifact`. В строгом режиме preflight `run` возвращает 4, пока в профиле есть `recoveries`; их снимает `loginom recover --acknowledge --format json`. Harness `recoverIfNeeded` в мягком режиме безвреден.
...
- 2026-09-18: план реализации записан: [часть 1 — фундамент](../plans/2026-09-18-evals-mvp.md) (задачи 1–5: скелет, конфиг, задачи, CLI, артефакт, отчёт) и [часть 2 — оркестратор, судья, режимы, приёмка](../plans/2026-09-18-evals-mvp-part2.md) (задачи 6–11). Разработка по `/tdd`.

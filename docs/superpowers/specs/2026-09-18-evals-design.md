...
Разработка — TDD (skill `tdd`): каждая проверка ниже появляется как один падающий тест поведения через публичный интерфейс модуля, затем минимальная реализация; горизонтальные срезы «все тесты, потом весь код» запрещены. `cd evals && bun test` (из корня репозитория тесты не запускаются по правилу `do-not-run-tests-from-root`):

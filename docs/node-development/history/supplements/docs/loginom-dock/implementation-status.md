> Исторический документ, адаптированный для навигации в Loginom AI Agent. Даты, версии, SHA и результаты относятся к прежним проверкам. Пути к коду указаны относительно нового репозитория; это не доказательство проверки текущих файлов. Исходник `source-b660d25b9099` и изменения: [происхождение](../../../../provenance.json). `unavailable:artifact-*` означает [неперенесённый материал](../../../unavailable.md). Актуальная работа: [регламент](../../../../README.md).
...
> 17 сентября, MiMo resumption: общий table reader и first read без preview проверены live;
...
> 23×6 полный CSV audit,27полей partial preview,unknownDone и защита intermediate-save PASS.
> Совокупная регрессия2118PASS/1SKIP (9sandbox socket failures адресно перепроверены).
> Общая приёмка38запусков ещё не начата; [checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/mimo-session-handoff-2026-09-17.md).

> **Пауза по просьбе пользователя, 17 сентября 2026.** [Передача MiMo в новую сессию](../../../../../../services/loginom-ai/docs/loginom-dock/mimo-session-handoff-2026-09-17.md). Последний run19 остановлен; финальные38 запусков не начаты. Новая правка mapping-table-view — непроверенный черновик. Этот checkpoint заменяет прежние статусы active для MiMo.
...
## MiMo: задача02 прошла полный диагностический аудит — 17 сентября 2026

Source9283894b, run20260917-140005-59459432: сохранение/выход и независимые
8таблиц/6276ячеек, настройки/связи/рейтинги PASS;10негативных проверок аудитора
отвергнуты. Текущая задача47 выполняется на той же версии. Полная серия38 ещё
не начата; смысловые ошибки сложных задач остаются открытыми. Подробности и
точка продолжения: [отчёт MiMo](../../../../../../services/loginom-ai/docs/loginom-dock/mimo-stability-report.md).

---

## MiMo: технические исправления проверены, полная приёмка открыта — 17 сентября 2026

В отдельной ветке исправлены восстановление соединения и подтверждённого Input
Done, точность компактного предпросмотра, фактический профиль normal Hermes CLI,
пояснения об округлении/нулях и ответ ожидания занятой операции. Модельные
прогоны13–15 сохраняли пакет и завершали Loginom-сеанс, однако полное бизнес-задание
38 не принято из-за пропусков и ошибочных выводов. Сейчас выполняется диагностика
задачи02; итоговая серия38 запусков ещё не начата. Main/общий plugin/server не
менялись. Актуальные ревизии, проверки и точка продолжения — в
[mimo-stability-report.md](../../../../../../services/loginom-ai/docs/loginom-dock/mimo-stability-report.md). Исторические итоги ниже.
...
## MiMo: масштабирование, восстановление и закрытие — 17 сентября 2026

Отдельная ветка `codex/mimo-scenario-stability`. Исправлены привязка мини-карты
к текущему сценарию, возврат после подтверждённой ошибки синтаксиса калькулятора
и сообщение о повторном dirty-state после сохранения. Живые проверки прошли,
включая исправление на том же узле и закрытие/logout без discard.
Автономный task38 diagnostic3 выявил дополнительный крайний случай координат8/8;
прогон остановлен, пакет не сохранён. Опубликованная схема и исполнитель теперь
отклоняют координаты менее64 до эффектов. Исправленный запрос без position
прошёл вживую с ровно1новым узлом/1связью. Клиент1963PASS/1SKIP,0FAIL.
По подсказке пользователя отдельно проверено автоупорядочивание и оптимальный
масштаб; полезны для геометрии, но ещё не интегрированы в обычное восстановление.
Diagnostic4 остановлен на повторном отказе соединения; воспроизведена перерисовка
при уходе указателя. Исправление соединения и расширение чтения до100 строк
проходят итоговую живую проверку. Быстрый просмотр порта проверен отдельно:72
строки в счётчике,57 отрисованы; ускорение ещё не измерено.
Полная аналитическая успешность и итоговая серия
ещё не подтверждены. [Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/mimo-stability-report.md),
[план](../../../../../../services/loginom-ai/docs/plans/loginom-dock/mimo-scenario-stability.md).
Main, VPS и общий плагин не менялись.

---

## MiMo: первый диагностический сценарий прошёл независимый аудит — 16 сентября 2026

Только ветка `codex/mimo-scenario-stability`, без слияния и обновления плагина.
Задача02 на `xiaomi / mimo-v2.5 / medium`, аккаунт `mimo` без пароля:
модель самостоятельно сохранила8аналитических узлов/7связей. Независимый reopen
проверил все9портов, настройки, полные таблицы и рейтинги:366строк2024года,
выручка365345.55; исходные500 и отклонённые134строки также сверены по всем полям.
Evidence: `unavailable:artifact-83a19d274d11e0f6`.
Runtime этого PASS: `5ba385249a27b3224772ac6daf8f9c751753ac372a4a80ecbab77c049136b0cf`.

После него уточнены schema имён/дат и wait-подсказка для работающего узла.
Полный набор1871:1861PASS/9отказов из-за запрета localhost sockets/1SKIP;
все затронутые14тестов прошли адресный повтор с доступом. Совокупно1870PASS/1SKIP.
Задача47 (`20260916-161114-dac3a8bc`) остановлена с FAIL: второй экспорт
выполнен, но чтение файла столкнулось с UI_ROOT_STALE, пакет не сохранён.
Прямой повтор установил восстановление конечной подпапки через прежнюю вкладку
Files: ожидание только промежуточного /mimo неверно. Исправления прошли
живой парный экспорт: оба файла по500строк полностью сверены. Дополнительно
устранён подтверждённый epoch-only отказ до скачивания; неизвестные эффекты
не повторяются. Полный клиент1880PASS/1SKIP/0FAIL. Новый диагностический
MiMo-прогон47 `20260916-164351-7b84b185` завершён FAIL: первые два экспорта
прошли, третий выполнен, но ожидание папки не завершилось. Пакет не сохранён.
Прямой тройной экспорт после исправления возврата во вкладку Files прошёл:
все3CSV по500строк сверены полностью, вкладка повторно используется.
Параметры нового экспорта по умолчанию проверены живым прогоном.

MiMo47fix3 (`20260916-171627-d7d7c0f8`) остановлен с FAIL после22вызовов:
два одноимённых узла вызвали расхождение ключей графа `@1` и навигации `-3`.
Последовательные прямые проверки выявили это в завершении входного мастера,
открытии настроек, проверке владельца и возврате из карточки выполненного узла.
Исправления используют проверенный GUID и привязанный к нему путь навигации;
полный живой цикл двух одноимённых узлов прошёл (fix5). Дополнительно исправлено ожидание
маски перед connect всех типов. Пользовательский resume теперь принимает
исходный ID и берёт неизменяемый запрос из локального runtime; unresolved
эффекты по-прежнему блокируются. Полный набор актуальной версии:1891PASS/1SKIP/0FAIL; адресные проверки
навигации и повторного resume также прошли.

MiMo47fix4 сохранил пакет без технической блокировки:20вызовов/487с.
Независимый reopen проверил7таблиц/186строк, настройки и граф — PASS.
Полное задание — FAIL: итоговый текст содержал неподтверждённый топ экономий,
смешение среднего по строкам со средним групп и противоречивую трактовку R&D.
Добавлены явное предупреждение о неполной выборке и общие требования к
подтверждению отчётных показателей; эталон и формулы модели не передаются.
Актуальный полный набор:1892PASS/1SKIP/0FAIL. Повтор47fix5 запускается в
`/mimo/MiMo-task47-fix5-20260916`; Hermes-слот занят.

Итоговая серия корпуса, полное восстановление выходных мастеров и живой
fault-injection доставки остаются незавершёнными. Задача47 пока не прошла
приёмку полного бизнес-задания. В8завершённых изолированных SQLite-архивах
очищены reasoning-поля; видимые ответы и tool evidence сохранены, новые
прогоны очищаются после экспорта. Активная native история не меняется.
[План и checkpoints](../../../../../../services/loginom-ai/docs/plans/loginom-dock/mimo-scenario-stability.md).

---

## MiMo: диагностика в отдельной ветке — 16 сентября 2026

Ветка `codex/mimo-scenario-stability`, база `9b46c85f`; изменения ещё не выпущены.
Исправлены согласование user schema/runtime, автоматическая позиция, технические
defaults импорта и чтения, загрузка preview и ожидание переходов мастера.
Добавлены ранняя проверка полного выхода нового калькулятора и output
`mappings.changes`, сохранён строгий legacy список. Live импорт и калькулятор
500строк, отказ до создания, исправление новымID, rename/exclude и replay PASS.
Полный клиент:1868PASS/1SKIP/0FAIL; native attachment tests11PASS.

Hermes проверяется только на Xiaomi `mimo-v2.5/medium`, Loginom `mimo` без пароля,
новый стенд `10.200.11.224`. Итоговая серия корпуса и независимый save/reopen аудит
ещё не завершены; восстановление output-мастера и широкий полный reader остаются
в работе. Main, сервер и общий плагин не изменялись.
[План, evidence и checkpoint](../../../../../../services/loginom-ai/docs/plans/loginom-dock/mimo-scenario-stability.md).
...
[Проверки, точные ревизии и оставшиеся ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/releases/rc6-release-2026-09-14.md).
...
По запросу пользователя создана [единая инструкция](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node-workflow-runbook.md)
...
[Подробные подтверждения](../../../../../../services/loginom-ai/docs/loginom-dock/memory-routing-activation-2026-09-13.json).
...
[Пакет переноса и focused test-2 план](../../../../../../services/loginom-ai/docs/plans/loginom-dock/11-saveas-integration-package-2026-09-13.md).
...
[Отчёт](../../../../../../services/loginom-ai/docs/plans/loginom-dock/11-development-audit.md),
[сводка](../../../../../../services/loginom-ai/docs/loginom-dock/replacement-development-2026-09-13.json),
[checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/replacement-progress-2026-09-12.md).
...
[Подробный checkpoint и следующий шаг](../../../../../../services/loginom-ai/docs/loginom-dock/duplicates-progress-2026-09-12.md).
...
[Итог, доказательства и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-hermes-full-3.md).
...
Runtime d5fd1ba7. [Исправление и доказательства](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-overflow-owner-fix.md).
...
[Итог, диагностика и план повтора](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-hermes-full-1.md).
...
[Результат, доказательства и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-resume-frozen-v4-1.md).
...
[Текущий отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-candidate-live-preflight-1.md). Candidate db38f7f2 и
...
[Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-r2-integration-1.md), [доказательства](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-r2-integration-evidence.json).
...
[Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-manual-output-followup-1.md), [доказательства](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-manual-output-evidence.json).
...
[Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-configure-recovery-1.md), [план R2](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-r2-integration-plan.md).
...
варианта. [Отчёт и новые pins](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-acceptance-input-limit-fix-1.md).
...
[Отчёт и admission](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-acceptance-input-preparation-1.md).
...
[Отчёт исправления](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-fix-1.md), [ревью](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-review-1.md),
[checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-checkpoint.md).
...
[Итоговый отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-development-report.md), [checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/node-13-checkpoint.md).
...
[Итог, хеши и границы](../../../../../../services/loginom-ai/docs/loginom-dock/node16-final-acceptance-2026-09-14.md).
...
[Причина, доказательства и точный checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/node16-source-replacement-result-2026-09-14.md).
...
[Доказательства, verifier delta и следующий checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/node16-hash-fix-result-2026-09-13.md).
...
[Точный статус и evidence](../../../../../../services/loginom-ai/docs/loginom-dock/node16-native-gates-progress-2026-09-13.md).
...
Никакие prepared CASE_PASS не считаются приёмкой. [Отчёт и интерфейсы](../../../../../../packages/loginom-runtime/tools/loginom-acceptance/collapse/runner-integration/README.md),
[машинный итог](../../../../../../packages/loginom-runtime/tools/loginom-acceptance/collapse/runner-integration/result.json).
...
[Kit и bounded diagnostic план](../../../../../../packages/loginom-runtime/tools/loginom-acceptance/collapse/acceptance-kit/README.md),
[машинный checkpoint](../../../../../../packages/loginom-runtime/tools/loginom-acceptance/collapse/acceptance-kit/preparation-result.json).
...
остаётся незавершённой. [Результат и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/node16-review-fix-result-2026-09-13.md).
...
[Итог и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/node16-exact-wiring-result-2026-09-13.md).
...
[Контракт, module owners и acceptance mapping](../../../../../../services/loginom-ai/docs/loginom-dock/node16-variant-contract-design-2026-09-13.md).
...
и shared plugin не менялись. Подробности: [hardening report](../../../../../../services/loginom-ai/docs/loginom-dock/collapse-variant-hardening-result-2026-09-13.md).
...
[Результат, provenance и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/collapse-variant-prototype-result-2026-09-13.md).
...
[Follow-up и доказательства](../../../../../../services/loginom-ai/docs/loginom-dock/collapse-empty-persistence-2026-09-13.md),
[план ограниченного read-прототипа для отдельного решения](../../../../../../services/loginom-ai/docs/loginom-dock/collapse-variant-prototype-plan-2026-09-13.md).
...
[Run7, исправление, актуальные component/pins](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-wizard-point-fix.md).
...
в [отчёте исправлений](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-save-reopen-fix.md).
...
[Диагноз, pins и evidence](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-account-diagnosis.md).
...
[Checkpoint и исходное evidence](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-account-preflight.md).
...
[Полный checkpoint, диагноз и frozen launch plan](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-final-admission-preflight.md).
...
[Отчёт и pins](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-navigation-origin-fix.md).
...
[Checkpoint, pins и два конкретных блокера](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-native-observer-smoke.md).
...
[Текущий checkpoint, pins и ресурсный trigger](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-read-observer-v2.md).
...
[Checkpoint и ограниченный план дальнейшей проверки](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-read-observer.md).

# Text export: разработка кандидата завершена — 13 сентября 2026

Текущий acceptance blocker: [reject-baseline read](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-reject-baseline-gate.md).
...
[План, доказательства и ограничения](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export.md).
...
[Отчёт исправлений](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-fix.md).
[Пакет подготовки автономной приёмки](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-acceptance-preparation.md)
...
[Отчёт и оставшиеся gates](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-preparation-2026-09-13.md).
...
[Отчёт разработки recovery](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-recovery-2026-09-13.md) содержит
...
[Отчёт единственного fix round](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-fix-r1-2026-09-13.md).
Ниже — исторические статусы соответствующих ревизий.
...
[План потоков](../../../../../../services/loginom-ai/docs/plans/loginom-dock/three-stream-workflow.md) фиксирует
...
Все три разработчика подтвердили в `unavailable:artifact-22950d6e897487a2` source runtime,
...
снимок изменений с SHA находится в `unavailable:artifact-39efa2599ab52315`.
...
затем подплан09 Слияние. [Checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
[Наработки, ограничения и продолжение](../../../../../../services/loginom-ai/docs/loginom-dock/parallel-node-pilot.md).
...
[Порядок, ограничения и продолжение](../../../../../../services/loginom-ai/docs/loginom-dock/parallel-node-pilot.md).
...
Добавлены [подпланы11–16 и общие условия](../../../../../../services/loginom-ai/docs/plans/loginom-dock/next-wave.md):
...
> сохраняется pending; не повторять отправку. [Контрольная точка](../../../../../../services/loginom-ai/docs/loginom-dock/text-import-node.md).
...
> [Контрольная точка](../../../../../../services/loginom-ai/docs/loginom-dock/text-import-node.md).
...
`git diff --check` прошёл. Логи: `unavailable:artifact-cfa013bbe15c5b02`.
...
резервной копии `unavailable:artifact-1d57fe2460b43f32`.

Резервная копия наработок, логи проверок и `unavailable:artifact-7d3cddcf270d9f0b` сохранены:
SHA256 всех 21 228 проверенных файлов совпали до и после очистки. Подробный
локальный manifest — `unavailable:artifact-6bb54c6fcb482e06`.
...
До работы сохранена локальная резервная копия `unavailable:artifact-1d57fe2460b43f32`:
...
[Контракт и source pins](../../../../../../services/loginom-ai/docs/loginom-dock/workspace-preparation.md). Сервер, публичные артефакты
...
Действующий план разделён на [отдельные документы](../../../../../../services/loginom-ai/docs/plans/loginom-dock/README.md):
...
сохранён [побайтно в истории](../../../../../../services/loginom-ai/docs/plans/2026-09-02-loginom-dock-implementation-plan-history-through-2026-09-07.md),
SHA256 6a5370e3516f2045a3188671b2c3dc4ab09da7c74c25d95e6981a5393133b3dd.
[Действующий план](../../../../../../services/loginom-ai/docs/plans/2026-09-02-loginom-dock-implementation-plan.md) задаёт
...
Доказательства: `unavailable:artifact-3239d30bca09e18c`.
...
Точная точка продолжения и SHA: [resume-checkpoint.md](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
P3 не принят. Текущий run и frozen audit — в [resume-checkpoint.md](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
SHA и дальнейшая работа: [resume-checkpoint.md](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
Подробности и SHA: [resume-checkpoint.md](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
описанные ограничения. Подробности и продолжение: [windows-agent-install.md](../../../../../../services/loginom-ai/docs/loginom-dock/windows-agent-install.md).
...
в [актуальном checkpoint](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md). Production unchanged.
...
`unavailable:artifact-6f724dc4e3d635b1`; текущее состояние и следующий шаг — в
[resume-checkpoint.md](../../../../../../services/loginom-ai/docs/loginom-dock/resume-checkpoint.md).
...
Hermes tool schemas. Подробности: [UI review](../../../../../../services/loginom-ai/docs/loginom-dock/p3-import-viewport-review.md).
...
[P4 ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/p4-managed-shutdown.md). P4 целиком не закрыт.
...
Private copy `unavailable:artifact-f2be7d216b36e4e4`.
...
Private evidence: unavailable:artifact-a59544be230cdfe4.
...
[p8-source-dependency-review.md](../../../../../../services/loginom-ai/docs/loginom-dock/p8-source-dependency-review.md).18/18 tests PASS;
...
recomputed types при format commit без schema acceptance. 295 unavailable:artifact-4b8ff1fae63e9c7b Python/10 packaging PASS. UI оставлен в TF-1 ImportTextFileParamsWizard,
...
связывают значения. Screenshot unavailable:artifact-fe4fb69dafd58ea3.
...
output-apply-{client,python,packaging,unit}-tests.txt в unavailable:artifact-f1ba7b56a9e97f13.
...
Private evidence unavailable:artifact-874cb21b1399eb60 и
...
Private evidence unavailable:artifact-c96bf64b22f0f9e2 и
...
Private evidence unavailable:artifact-5f17acf26a27dbf4; test logs
...
Private evidence: unavailable:artifact-94b2beeeec8abb43, grouping-done.png.
...
unavailable:artifact-f00261dc965dcda9{client,python,packaging}-tests.txt.
...
Private evidence unavailable:artifact-714e2e56fb47e986.
...
Private evidence unavailable:artifact-d8360b1c43f6e79a и
...
Private evidence: unavailable:artifact-e184bdb3aee614b0;
...
Private evidence: unavailable:artifact-c8684816928ceeba,
...
Добавлен cancel_expression_parameters через тот же UI unavailable:artifact-81f599275a674287.
...
и unavailable:artifact-54979b04d11f3d4f. Active Hermes нет.
...
Evidence unavailable:artifact-c6d1ffea9043de01, expression-apply-*-tests.txt.
...
unavailable:artifact-483481fb2bdb9c4b; тесты expression-field-*-tests.txt.
...
Evidence unavailable:artifact-0c5d4486d2092eb3 и expression-params-live-result.txt.
...
Live evidence: unavailable:artifact-c2c24aa76476972d (Codex diagnosis,
...
Evidence: unavailable:artifact-fe079cc8256496f0; UI оставлен открытым
...
Evidence: unavailable:artifact-1682b95bf6c08617 и navigation-live-result.txt.
...
allowed_roots=['/test/packages']. Отчёт unavailable:artifact-9c0b3eb81ffd7f8f.
...
package.save_as в packages/loginom-runtime/executor/catalog/actions.json разрешает только
...
в unavailable:artifact-f1ba7b56a9e97f13. негативные проверки покрывают изменённое исходное
...
Отчёты: unavailable:artifact-b463189902b3de5e. Это проверка исходников
...
Отчёты unavailable:artifact-7e2c2f9832397d98. Следующий полный run — явно
...
unavailable:artifact-28af9322835f8088{client,pages-client,audit-python,packaging}-tests.txt.
...
Отчёты unavailable:artifact-e19d8c5dc58400c2. Runtime inputs остаются 46.
...
workspace-ui PASS; 106 Python и 10 packaging PASS. Отчёты unavailable:artifact-f1ba7b56a9e97f13
...
unavailable:artifact-d5cf08e4e3dc24e0 PASS. Upload tests исполняют сериализованный production
...
unavailable:artifact-c2601226fbe6a1b4, SHA
...
Operator check packages/loginom-runtime/tools/loginom-acceptance/artifact-transport-check.mjs запускает
...
Последний native-local report unavailable:artifact-92f1b04a620fb136:
...
184 client / 81 Python PASS. Первый unavailable:artifact-677f3a536e3a35f1 run выявил mock session
...
В `unavailable:artifact-eca9075bb5cffc09` добавлены sales.csv,
...
Добавлен `packages/loginom-runtime/tools/loginom-acceptance/checked_state.py`: привязка ответа к одной
...
Runtime после f9e4fe83 не менялся, 164 unavailable:artifact-ae99e53bf671aae5 packaging актуальны.
...
Curated inventory `packages/loginom-runtime/executor/inventory/palette-2026-09-05.json`: 12 групп,
...
Добавлен `packages/loginom-runtime/client/lib/observation-pages.mjs`: public output до 12000 байт,
...
`unavailable:artifact-702793804cc454b8`.
...
Audit `unavailable:artifact-e8d79d3232453016`, SHA
`d80639df64ce113b89a0efcc718f3b927137fdeff2b9dc006fd5215ba50f2e5b`.
Индекс `unavailable:artifact-a223e59c768259bb` — 26 attempts /
...
место хранения `unavailable:artifact-eaea778c5b115b49`.
...
Индекс `unavailable:artifact-d7e02b49aa800bf6`:
...
`unavailable:artifact-18d04be4ce989b22`: 22 попытки / 8 PASS,
...
`unavailable:artifact-c981c205e9abd7f4`: 18 попыток / 7 PASS,
...
`unavailable:artifact-337d3ecaeab986ea`, 17 попыток / 6 PASS,
...
`unavailable:artifact-5c3608c29fa773b1`: 13 попыток / 6 PASS;
...
`git diff --check` чистый. Индекс: `unavailable:artifact-751409c9548bf849`.
...
Индекс сохранён в `unavailable:artifact-4e83c5b488546b2b`.
...
`packages/loginom-runtime/client/lib/recovery-context.mjs`. Он читает pinned E2E-helper через штатный MCP
...
Индекс `unavailable:artifact-a81424a287694c6d`: 8 попыток,
...
`unavailable:artifact-d315bdf32b717522` подтвердил точный граф до/после
...
Индекс `unavailable:artifact-dd863cc28a966615`: 7 попыток, 1 прежний PASS,
...
По запросу пользователя в [план](../../../../../../services/loginom-ai/docs/plans/2026-09-02-loginom-dock-implementation-plan.md#post-mvp-scope)
...
Добавлена [карта E2E-источников](../../../../../../services/loginom-ai/docs/loginom-dock/e2e-source-map.md) по 14 областям. 46 выбранных
...
В `unavailable:artifact-6c9840df3f4b6129` перенесены четыре операторских wrapper:
...
`unavailable:artifact-ce2f7b3675b4251a`, SHA
...
`unavailable:artifact-435ffbf1ab46bc9b`, SHA
...
`unavailable:artifact-50f515332a69e72b`, SHA
...
| Артефакт | SHA-256 |
| --- | --- |
| `unavailable:artifact-95e18a8c56f1879f` | `80b00db646b2bd2cb1fe641a27d4a70a5c121c9f1afad0af28bc590ab66e857e` |
| `unavailable:artifact-05d0310be6086b27` | `5db3dc1e64eed577ca1868f8491364e041ae5b05e2aa0521f14387ade16855ab` |
...
Перенесены в `unavailable:artifact-6c9840df3f4b6129`:
...
| Артефакт | SHA-256 |
| --- | --- |
| `unavailable:artifact-8b1a74d5c9817dd8` | `9a30c47bb615f31269ce098a0bd2fcde4d0af1c759725105c3492480999a808c` |
| `unavailable:artifact-0189623b798bd032` — 1 попытка, 1 PASS | `d1e73499143d963c8a9240bf5efdac2d1bcd4fd59f10578050327c657ce1a9f8` |
| `unavailable:artifact-14c762f37323bdd6` — 46 E2E / 3 UI | `80055b6f866cbf93f7de25c487d3b4310f2611f94a6208da24e12c4951046fc0` |
...
Добавлен операторский `packages/loginom-runtime/tools/loginom-acceptance/preflight.py` и его README.
...
`ca4b9f4cdbf6d46358235147d445a45428b224046cf0074cae16a241d3bdc016`.
Отчёт: `unavailable:artifact-9c14c56ff56b395b`, SHA-256
...
приведены в [отдельной справке](../../../../../../services/loginom-ai/docs/loginom-dock/pinned-ui-probes.md).
...
Доказательства — в `unavailable:artifact-797458c758e8c823`: `positive-audit.json`,
...
Приватные доказательства находятся в `unavailable:artifact-c0162febc155ea5f`.
...
сохранён в защищённом серверном `client.json` и локальном `unavailable:artifact-f9c3d022b98ca5f7`.
...
попытка обнаружила исключение `unavailable:artifact-47995fe672cb178a` общим Git-ignore правилом `lib/`;
...
Внешняя копия получена на Mac: `unavailable:artifact-9e0bf1b03d11d17d`, 5195182080 байт,
...
MCP. Для этой временной ошибки добавлен один повтор с новым unavailable:artifact-870e4748fb173d79 и
...
Добавлены пользовательская инструкция `packages/loginom-runtime/client/INSTALL.md` и порядок выпуска
`services/loginom-ai/docs/loginom-dock/releasing.md`. Архивы, `SHA256SUMS`, инструкция и отчёт проверки
подготовлены в `unavailable:artifact-6f39a011f8aaa699`.
...
commit и обоих архивах; 19 файлов `unavailable:artifact-ce7a9ad6c2e41947` и `unavailable:artifact-47995fe672cb178a` совпадают с текущей
...
Итоговый скриншот просмотрен; полный отчёт — `unavailable:artifact-343a2bd4bffac840`.
...
Отчёт проверки — `unavailable:artifact-72544d9b5afe8f40`.
...
Отчёт — `unavailable:artifact-c1cd16bedee8df91`.
...
прошла, ошибок консоли нет. Скриншот — `unavailable:artifact-a3e0ee45f7637c61`.
...
Отчёты и просмотренные скриншоты — `unavailable:artifact-ea2039e8ad752319`; сборочная ревизия
...
Выявлен дефект для следующего клиентского выпуска: `packages/loginom-runtime/client/test/landing.test.mjs`
...
Доказательства сохранены в `unavailable:artifact-38ea3ca4fe35557a`:
...
Подробности и точные доказательства — [checkpoint node12](../../../../../../services/loginom-ai/docs/loginom-dock/duplicates-progress-2026-09-12.md).
...
[Комплект и границы приёмки](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node12-acceptance-preparation-1/README.md).
...
[Сводка](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node12-acceptance-preparation-1/candidate-preflight-summary.json).
...
[Отчёт](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node12-acceptance-preparation-1/attempt-20260913-060042.md).
...
[Полный отчёт](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node12-acceptance-preparation-1/attempt-20260913-060514.md).
...
[Отчёт, pins и комплект node12.2](../../../../../../services/loginom-ai/docs/plans/loginom-dock/node12-acceptance-preparation-2/README.md).
...
Итог и ограничения: [отчёт разработки](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-development-report-2026-09-13.md).
...
[Отчёт native-save/startup](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-native-candidate-preflight-2026-09-13.md):
...
[Статус попытки](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-1-blocked-2026-09-13.md):
...
[Отчёт единственного Sol/low run](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-2-2026-09-13.md):
...
[Отчёт, pins и ограничения полного аудитора](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-target-recovery-2026-09-13.md).
...
[Отчёт и свежий пакет кандидата](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-terminal-refusal-verifier-2026-09-13.md).
...
[Отчёт и pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-candidate2-preflight-2026-09-13.md).
...
[Отчёт и pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-3-2026-09-13.md).
...
[Отчёт и предложение по import refusal](../../../../../../services/loginom-ai/docs/loginom-dock/node14-import-configure-followup-2026-09-13.md).
...
[Отчёт и pins](../../../../../../services/loginom-ai/docs/loginom-dock/node14-import-refusal-verifier-2026-09-13.md).
...
[Отчёт, ограничения и38 evidence receipts](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-4-2026-09-13.md).
...
[Диагностика, границы и возобновление](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-remove-link-diagnosis-2026-09-13.md),
[проверки и контрольные суммы](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-remove-link-diagnosis-pins-2026-09-13.json).
...
[Подготовка, ограничения и план запуска](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-acceptance5-preparation-2026-09-13.md),
[квитанции](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-acceptance5-preparation-pins-2026-09-13.json).
...
[Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-5-2026-09-13.md),
[pins и квитанции](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-5-pins-2026-09-13.json).
...
[Диагностика и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-stale-target-diagnosis-2026-09-13.md),
[pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-stale-target-diagnosis-pins-2026-09-13.json).
...
[Отчёт и scope возможного продолжения](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-stale-full-context-diagnosis-2026-09-13.md),
[pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-stale-full-context-diagnosis-pins-2026-09-13.json).
...
[Исправление, ограничения и точный план запуска](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-source-selection-fix-2026-09-13.md),
[pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-source-selection-fix-pins-2026-09-13.json).
...
[Отчёт](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-6-2026-09-13.md),
[pins](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-6-pins-2026-09-13.json).
...
[Исправление, pins и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-target-selection-fix-2026-09-13.md).
...
[Отчёт и доказательства](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-7-2026-09-14.md).
...
[Исправление, доказательства и команда запуска](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-import-layout-fix-2026-09-14.md).
...
[Результат и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-autonomous-acceptance-8-2026-09-14.md).
...
[Исправление, границы и комплект запуска](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-existing-import-audit-fix-2026-09-14.md).
...
[Итог, версии и ограничения](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-run8-reevaluation-v2-2026-09-14.md),
[машинные квитанции](../../../../../../services/loginom-ai/docs/loginom-dock/missing-values-run8-reevaluation-v2-pins-2026-09-14.json).
...
[Подробности и ограничения](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-import-editor-fix.md).
...
[Доказательства, уточнение и ограничения](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-binding-fix.md).
...
[Ограничения и доказательства](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-return-fix.md).
...
[Доказательства и ограничения](../../../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export-reveal-fix.md).
...
`services/loginom-ai/docs/loginom-dock/mimo-stability-pause-2026-09-16.md`.

## MiMo — работа возобновлена 16 сентября 2026

Пользователь дал команду продолжить. Работа идёт в `codex/mimo-scenario-stability`,
только Xiaomi / mimo-v2.5 / medium, Loginom `mimo`, отдельные браузеры и папки.
Task21fix1 построил6узлов, но полное задание FAIL: нет анализа риска по доходу,
общие средние и часть выводов неверны; штатный teardown отказал UNSAVED_CHANGES.

На отдельной копии независимо сверены7таблиц/1005строк и настройки6узлов.
Ошибка заключительного чтения графа воспроизведена одним Cancel выходного мастера:
два SVG одного native порта, атрибут data-tid на одном, shape.node на другом.
Исправлена проверенная привязка по прежнему порту/ячейке/GUID и native hit.
Адресный live5узлов и полный copy4-аудит PASS_DIAGNOSTIC, оригинал не принят.
Runtime исправления портов: `ba6d64730f291da6ecd6747e324922b952d8affb71a73192b534dce23e5ebdb2`.

Следующий этап — ограниченное восстановление подтверждённого output Done
калькулятора. Код и64адресных теста готовы, полный набор1904PASS/1SKIP;
живое испытание потери ответа ещё не принято. Финальная серия38не начата.
Канонический checkpoint: `services/loginom-ai/docs/plans/loginom-dock/mimo-scenario-stability.md`.
Main, deployment и общий плагин не изменялись.

### Дополнение MiMo: output recovery PASS, task19 сохранён

Ограниченное восстановление output Done калькулятора прошло живую потерю ответа:
одно Done/один узел/одна связь, исходныйID; клиент1905PASS/1SKIP.
Task19 завершился и закрыл сеанс штатно, но business FAIL; экспорт не создан
из-за недоступной явной позиции. Независимое открытие оригинала заблокировано
пустым HTTP-ответом стенда до входа. Runtime сохранён неизменным; подробный
checkpoint и порядок продолжения находятся в плане MiMo. Финальная серия не начата.
...
### 2026-09-17 — MiMo: существующий калькулятор после отказа

На `codex/mimo-scenario-stability` исправлено восстановление после синхронного
отказа patch существующего калькулятора до первого изменения выражений.
Двукратное закрытие и независимое повторное открытие/сверка сохранённых
настроек предшествуют снятию блокировки. Живой повтор на диагностической
копии task38: отказ → исправленный новый ID → тот же узел и72строки, без
дублей; cleanup/logout PASS. Клиентская регрессия1936PASS/1SKIP.
Контракты finish/read и ошибки сортировки уточнены. Task38 модельный прогон
до исправления FAIL, оригинал остался заблокированным; на копии проверены
все 72 строки, удержание верно, числовой месяц NULL. Новый zoom-отказ остаётся
не воспроизведённым. Итоговая серия корпуса не выполнена; main и установленный
плагин не изменены. Подробности: `services/loginom-ai/docs/plans/loginom-dock/mimo-scenario-stability.md`.
...
До завершения не редактировать unavailable:artifact-d1813c5ebde5a28e и не открывать второй сеанс mimo.
...
разрыва, сохранение и широкие ответы. Подробности: services/loginom-ai/docs/loginom-dock/mimo-stability-report.md.
...
Полная серия38прогонов не начата. Точка продолжения: services/loginom-ai/docs/loginom-dock/mimo-stability-report.md.

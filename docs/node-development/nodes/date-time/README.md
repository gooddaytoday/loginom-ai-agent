...
12 преобразований: year/quarter/month/day_of_month/hour, start/end года/квартала/месяца и date.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях B02, V27.
...
- Вход уже datetime; строковый разбор и изменение часового пояса не выполняются.
- Нет ISO-недель и произвольных строковых форматов; несколько преобразований одного/разных полей допускаются.
...
[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/date-time-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/date-time-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Конец месяца/года/квартала, високосный день, время суток, NULL и пустая таблица.
- Несколько преобразований/полей, точные типы/границы периодов, независимые ожидаемые значения.
- Input/output mappings и сохранённые настройки/порядок после отдельного reopen.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/13-date-time.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/loginom-dock/node13-branch-acceptance-2026-09-14.json) — доказательство только закреплённой там ревизии.

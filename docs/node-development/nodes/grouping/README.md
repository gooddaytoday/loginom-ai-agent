...
Упорядоченные ключи и меры sum/count/avg/min/max; согласованная схема результата.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях B02, B18, B27, B37, B47, B65, V02, V27, V65.
...
- Непустые ключи и меры; не более 128 ключей/256 мер. Одно поле не может быть и ключом, и мерой.
- Нет медианы, distinct count и произвольных статистических тестов в этом контракте.
- SUM/AVG только integer/real; аналитически правильный выбор агрегации остаётся ответственностью модели и независимого oracle.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/grouping-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/grouping-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Несколько ключей и несколько функций поля; NULL/all-null/пустой вход/дроби.
- Имена/labels/исключение ключей/output mapping и полный multiset результата.
- Временный preview upstream, cleanup и сохранённая схема после нового открытия.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/07-grouping.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/plans/loginom-dock/07-completion-audit.md) — доказательство только закреплённой там ревизии.

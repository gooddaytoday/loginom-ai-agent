...
Информационные и упорядоченные транспонируемые поля; ignore_empty=true/false; scalar/variant-выход.
...
- Это unpivot столбцов, а не разбиение строки со списком. Входной variant не поддержан.
- Нативный путь полного точного чтения variant ограничен 50 строками × 8 полями и проверенным происхождением данных; тип и точность нельзя выводить по выборке.
- Оба списка ролей не пересекаются; служебные names/displaynames/values/datatypes запрещены в информационных именах.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/collapse-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/collapse-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Пять скалярных типов/NULL, однородные integer и смешанные поля без принудительного string.
- Оба ignore_empty, all-null/header-only, mapping и DataTypes-off.
- Независимая exact проверка значений/типов и 10 малых случаев после save/reopen в пределах контракта чтения.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/16-collapse-columns.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/loginom-dock/node16-branch-acceptance-2026-09-14.json) — доказательство только закреплённой там ревизии.

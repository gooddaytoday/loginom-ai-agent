...
Изменение имени/метки/типа/вида/назначения/исключения; явный порядок полей.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях REFORM.
...
- Пять scalar-типов; циклические переименования и конверсии из неподдержанных типов не входят в контракт.
- Для continuous → string/boolean требуется явно задать дискретный вид.
- Выбор поля по точному имени/identity, не по потенциально одинаковой метке; максимум 128 изменений.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/reform-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/reform-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Конверсии пяти типов, NULL и некорректные значения с нативной семантикой Loginom.
- Одинаковые labels, rename/type/usage/exclude, полный порядок и сохранение неуказанных свойств.
- Done/Close и выполнение; независимая схема/значения после save/reopen.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/05-field-parameters.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/plans/loginom-dock/05-completion-audit.md) — доказательство только закреплённой там ревизии.

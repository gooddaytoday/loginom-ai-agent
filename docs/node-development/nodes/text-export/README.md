...
Устойчивый ID: `component.exports.Text`. Runtime: `exports.text`, режим `delimited`. Исторический подплан 17.
...
Один табличный вход → проверенный CSV/TSV UTF-8; назначение в разрешённом хранилище, явный формат и overwrite.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях V65.
...
- Нет табличного выхода: mappings=[], read.ports=[], sample_rows=0; результат — file_artifacts.
- UTF-8; разделители «;», «,» или табуляция; заголовок none/names/labels, BOM, LF/CRLF, десятичная точка/запятая, двойная кавычка. Форматы NULL, даты, времени и Boolean ограничены валидатором.
- Файл не больше 16 MiB; replace требует явный destination в каждом запросе. Старое ограничение /test-2 не является текущей политикой: путь проверяет storage-policy.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/text-export-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/text-export-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Кириллица, кавычки/разделитель/перенос внутри поля, NULL/empty, типизированные значения.
- TSV, BOM, CRLF, метки, пустой файл и широкая таблица; запрет перезаписи и явная замена существующего файла.
- Байты/SHA файла и его принадлежность свежему исполнению; сохранение .lgp проверяется отдельно.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/17-text-export.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/loginom-dock/node17-branch-acceptance-2026-09-14.json) — доказательство только закреплённой там ревизии.

...
Конкатенация 2–15 таблиц с сохранением дублей, полной картой полей и явными префиксами.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях V27.
...
- Главный вход 0 и присоединённые 1–14; tables обязаны идти последовательно и покрывать каждое поле входов.
- Типы сопоставляемых полей должны совпадать; отдельное поле задаётся main:null. DISTINCT не поддержан.
- Контракт 15 входов шире исторически проверенных live8; предел не повышает подтверждённое покрытие.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/union-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/union-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Два/три/восемь самостоятельных входов, дубли/NULL/пустые таблицы, несопоставленные поля.
- Добавление входа существующему узлу без повторной связи; префиксы/порядок/исключение.
- Неверный тип/неполная карта до эффекта; save/reopen и полный ordered output.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/10-union.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/plans/loginom-dock/10-completion-audit.md) — доказательство только закреплённой там ревизии.

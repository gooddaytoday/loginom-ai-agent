...
Упорядоченные выражения Loginom; новые поля и явная замена существующих; scalar-выход.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях B18, B27, B47, B65, V65.
...
- Язык выражений Loginom; JavaScript-режим не реализован.
- Типы integer/real/string/boolean/datetime; до 128 выражений, формула до 2048 символов.
- Поддержка выражений не доказывает правильность аналитической формулы; деление на ноль/NULL и агрегации проверяются независимо.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/calculator-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/calculator-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Зависимые выражения, добавление и replace; порядок вычисления и порядок выходных полей.
- Integer/real/string/boolean/datetime, NULL, дроби; ошибки формул без ложного успеха.
- Повторная настройка того же узла, save/reopen и независимые ожидаемые значения.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/04-calculator.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/plans/loginom-dock/04-completion-audit.md) — доказательство только закреплённой там ревизии.

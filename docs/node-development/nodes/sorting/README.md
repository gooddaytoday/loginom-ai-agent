...
Полный упорядоченный список ASC/DESC ключей и настройки сравнения строк.
...
- Desktop технически создал, выполнил и сохранил этот тип в случаях B02, B27, B37, B47, B65, V27, V65.
...
- Непустой список до 128 ключей. Для string/variant обязательный case_sensitive, для прочих типов он опускается.
- compare_with_locale — отдельный явный параметр; одинаковые ключи не гарантируют стабильность порядка строк.
- Переменные и непроверенные локали/variant-комбинации не считать принятыми по наличию схемы.

[Текущий обработчик](../../../../packages/loginom-runtime/client/lib/sorting-node.mjs); [параметры/процедура](../../../../packages/loginom-runtime/client/lib/sorting-parameters.mjs). Публичную схему и валидатор читать перед изменением; скопированный пример не заменяет проверку фактической версии.
...
- Несколько ASC/DESC ключей, регистр и locale; NULL и одинаковые значения.
- Полный порядок строк по независимому comparator без обещания порядка равных ключей.
- Изменение списка ключей/порядка выходных полей; save/reopen.
...
- [Исторический подплан](../../../../services/loginom-ai/docs/plans/loginom-dock/08-sorting.md) — объём и прошлые наблюдения; прежние статусы и исполнители исторические.
- [Историческое принятие](../../../../services/loginom-ai/docs/plans/loginom-dock/08-completion-audit.md) — доказательство только закреплённой там ревизии.

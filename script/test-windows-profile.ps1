...
try {
  ...
  $test = Join-Path $repo 'packages/agent/test/cli/profile-windows.test.ts'
  $command = @"
`$ErrorActionPreference = 'Stop'
`$env:PSModulePath = [IO.Path]::Combine(`$PSHOME, 'Modules')
`$identity = [Security.Principal.WindowsIdentity]::GetCurrent()
if (([Security.Principal.WindowsPrincipal]::new(`$identity)).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Expected a standard user' }
...
"@
  ...
  $child = Start-Process -FilePath (Join-Path $env:SystemRoot 'System32/WindowsPowerShell/v1.0/powershell.exe') `
    -ArgumentList @('-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', [Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($command))) `

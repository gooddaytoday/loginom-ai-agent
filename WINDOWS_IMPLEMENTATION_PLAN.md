...
- Сквозные проверки используют существующее подключение Dock, стенд `logi-test-plan.bg.local` и ChatGPT.
...
- [x] Согласовать Desktop + CLI, локальные сборки и проверку через ChatGPT.
...
- [ ] Подтвердить действительность существующих доступов Dock без вывода секретов.
...
- [ ] Повторить сборку из чистого checkout без неучтённых локальных файлов.
...
- [ ] Проверить разделение профилей Desktop/CLI и каналов приложения.
- [ ] Проверить Electron safeStorage для Desktop.
- [x] Проверить DPAPI CurrentUser для CLI.
- [ ] Проверить сохранение и чтение секретов после перезапуска.
- [ ] Проверить отказ при повреждённых данных или недоступной защите без перехода к plaintext.
- [ ] Проверить ACL профиля CLI и блокировку одновременного доступа.

### Системный прокси

- [ ] Подключить чтение пользовательских настроек прокси Windows к обоим продуктам до запуска backend/runtime.
- [ ] Реализовать общий выбор прямого соединения или прокси для URL.
- [ ] Поддержать отдельные HTTP/HTTPS-прокси, доменные и IP-шаблоны, `<local>` и loopback.
- [ ] Подключить маршрутизацию к используемым HTTP, fetch и WebSocket-вызовам Node/Bun.
- [ ] Проверить внутренний Loginom, внешний Dock, ChatGPT и OAuth callback.
- [ ] Для неподдерживаемых режимов выдавать явную ошибку; системные настройки компьютера не изменять.

### Жизненный цикл

- [ ] Проверить штатное закрытие через IPC и подтверждение очистки.
- [ ] Проверить Ctrl+C и повторное прерывание CLI.
- [ ] Проверить потерю родительского процесса и аварии runtime/Chromium.
- [ ] Ограничить принудительную очистку процессами конкретного запуска.
- [ ] Сохранять блокировку и состояние восстановления при неподтверждённой очистке.
...
- [ ] Расширить Desktop-манифест целью `win32-x64`, пользовательской установкой, NSIS-артефактом и Windows-путями. NSIS и resource manifest готовы; release manifest ещё Linux-only.
- [ ] Добавить проверку PE/AMD64 и комплектных ресурсов. CLI-проверка готова; Desktop release verifier ещё нужно расширить.
- [ ] Сохранить совместимость проверки с существующими Linux-манифестами.
...
- [ ] Добавить проверки Windows-путей, прокси, манифестов и завершения процессов.
- [ ] Перенести наблюдение за процессами и окнами на Windows API.
- [ ] Проверить интерактивный CLI через ConPTY.
- [x] Проверить пути с пробелами и кириллицей.
- [ ] Проверить запуск установленных продуктов без Node/Bun/Python в PATH.
- [x] Проверить автономное открытие мастера настройки.
- [ ] Пройти настройку Loginom и повторный запуск Desktop/CLI.
- [ ] Пройти вход ChatGPT в отдельных профилях; при необходимости привлечь пользователя только для интерактивной авторизации.
- [ ] Выполнить CSV-сценарий через Desktop: результаты 55 и 101, сохранение и независимое повторное открытие.
- [ ] Повторить CSV-сценарий через CLI `run`.
- [ ] Повторить CSV-сценарий через TUI.
- [ ] Подтвердить реальные вызовы Loginom-инструментов через ChatGPT в Desktop и CLI.
...
- [ ] Добавить Windows-сборку и пакетные тесты в CI без публикации.
- [ ] Обновить устаревшие инструкции Windows и сведения о готовности платформы.
- [ ] Описать подготовку окружения, сборку, установку, запуск и отладку.
- [ ] Подготовить отчёт: ОС, commit, команды, результаты, SHA256 и оставшиеся ограничения.
- [ ] Отдельно отметить проверки, которые не проводились на чистой Windows или под другим Windows-пользователем.
- [ ] Передать EXE Desktop, ZIP CLI, исходники и контрольные суммы.
- [ ] Проверить отсутствие секретов в передаваемых материалах.
- [ ] Обновить итоговый статус плана.

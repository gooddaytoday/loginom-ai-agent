...
### 2026-09-19 — понятные настройки Loginom, Desktop 0.1.5

- [x] Показывать фиксированную маску и подпись о сохранённом ключе без возврата секрета в renderer.
- [x] «Сохранить и закрыть»: проверить данные, сохранить, закрыть окно и показать результат; при ошибке сохранить введённый черновик.
- [x] «Проверить подключение»: проверить текущие значения без сохранения и закрытия.
- [x] Удалить постоянную кнопку загрузки настроек; автоматически читать текущее состояние, не затирая черновик; обновление по кнопке оставлено только при конфликте версии.
- [x] Добавить понятное закрытие кнопкой и крестиком; защитить несохранённые изменения также для Escape и щелчка вне окна.
- [x] Различать успешное и отложенное применение; показывать позднюю ошибку после закрытия формы.
- [x] Собрать и установить Desktop 0.1.5.0 из product commit `30a22ff8905ff9f3d0b2dafe3605de17b23ffda2`, проверить совпадение установленного ASAR с packaged payload.
- [x] Проверки: 19 app-тестов, 7 packaging-тестов, 4 package typecheck, 4371 resource files; packaged GUI и реальные Dock/Loginom check/save/reopen через установленное приложение — PASS.
- Artifact SHA256: `9c003ea3a91fc20e2bb741ecc79573886b0b53a14d8dad2de3884fbf6116c9ca`. CLI не изменялся. Все GUI-тесты использовали отдельные профили.
- [Отчёт, evidence, команды и ограничения](docs/testing/loginom-ai-agent/reports/2026-09-19-settings-ux/report.md). Это целевая UI-регрессия, не повтор полной Windows/Xiaomi/CSV-приёмки ниже.
